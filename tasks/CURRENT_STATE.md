...
- **Phase X-05-1: CI Stage 1 — 実装完了（判定 GO・記録: doc64）／push・GitHub Actions 実走確認は未実施（commit-only）**。`.github/workflows/ci.yml` 新規1ファイルのみ（app code/package/lock 無変更・secrets不使用・DB不要）: push/PR 時に install→db:generate→test→typecheck→lint を自動実行。ローカル検証 green（test 211・typecheck 0・lint 0・actionlint は未インストールのため未実施＝成功扱いしない）。**GitHub Actions 実走確認は push 後に利用者が Actions タブで確認**（doc65 候補）。Stage 2 build・Stage 3 smoke・否定系テスト（X-05-2）は別承認。
...
- Phase 2-B-5 基準 commit（本番確認 GO 済み基準）: `83d35bc`（※現在 HEAD ではなく基準 commit。現在位置は git を参照）
...
1. **CI Stage 1 commit（doc64＋ci.yml）の push（push-only・別承認。feature＋main）→ GitHub Actions 実走確認（利用者・doc65 候補）**。
2. **X-05-2（否定系テスト第一弾）/ Stage 2（build の CI 追加）の承認判断**。isHumanUser の A/B 方式も人間判断。

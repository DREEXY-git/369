...
1. **Phase 2-B-5 実装 commit の push（push-only・別承認。feature＋main）**。
2. **Phase 2-B-5 の本番確認（doc49 の型・利用者実測・doc61 候補）**: 本番に実在する営業プレイブックで検索→「参照した会社の頭脳」表示→ai_reference ログ→既存検索無回帰。
3. **本番確認 GO 後に Phase 2-B 全体クローズ判定**。改善候補（任意・別承認）: CI 導入・否定系テスト・doc49 script化。

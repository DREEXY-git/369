...
1. **Phase 2-C-5-ENTRY 記録（doc78）の push（push-only・別承認。feature＋main）**。
2. **Phase 2-C-5 実装（company-brain-reference 4テーブル目・doc79 候補・別承認）の承認判断**（doc78 §9 に実装プロンプト案）。
3. または **ConsentRecord 連携設計 / Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・並行可）**。

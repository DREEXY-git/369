...
| **2-G MCP/API internal scope design** | 369 MCP/API Gateway の内部設計（scope 体系・rate limit・audit・sandbox 方針）。Company Brain API 等の read-only internal 設計 | docs-only 設計 | 公開なし・エンドポイント実装なし |

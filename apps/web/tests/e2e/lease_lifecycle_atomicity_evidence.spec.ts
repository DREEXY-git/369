...
async function countAll(tenantId: string, reservationId: string, assetIds: string[]): Promise<LeaseCounts> {
  ...
  // movement 由来 event は本 spec の movement id に限定（他テストの資産行と混ざらないよう key で照合）。
  const movementEvents = events.filter((e) => movementIds.some((id) => e.idempotencyKey === `LEASE_MOVEMENT:mov:${encodeURIComponent(id)}`));
...
test('High-1 barrier: lock前quantity=1がlock待機中に0へ変更されると、lock後の同tx再読込で conflict（ライン/Movement 0）', async () => {
  const actor = await getActor();
  const asset = await makeAsset(actor.tenantId, 1);
  const reservation = await makeReservation(actor.tenantId);
  try {
    // add 呼び出しは事前読取（quantity=1 の snapshot）後、tx 開始直前の gate で停止する。
    // その間に別 transaction が在庫を 0 へ変更して commit（adjust は FOR UPDATE＋単一 tx）。
    // 再開後の add は lock 取得**後**に同 tx で quantity を再読込するため、0 を見て conflict になる。
    let releaseGate!: () => void;
    const gate = new Promise<void>((res) => (releaseGate = res));
    const pAdd = addAssetToLeaseReservation(
      actor,
      { reservationId: reservation.id, assetId: asset.id, quantity: 1 },
      { __gateBeforeTxForTest: () => gate },
    );
    ...
    const r2 = await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: asset.id, quantity: 1 });
...
test('P2-2 原子性: ライン作成後・Movement前のfaultで全rollback（ライン0）→ retryがちょうど1組へ収束', async () => {
  ...
  try {
    ...
    const r = await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: asset.id, quantity: 1 });
...
test('lifecycle CAS: reserved→confirmed→dispatched→returned が各1回だけ成立し、replayは already で不増', async () => {
  ...
  const reservation = await makeReservation(actor.tenantId);
  try {
    const add = await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: asset.id, quantity: 2 });
...
test('順序違反はfail-closed: reserved からの dispatch/return・returned からの再dispatch は invalid-state（書き込み0）', async () => {
  ...
  const reservation = await makeReservation(actor.tenantId);
  try {
    await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: asset.id, quantity: 1 });
    ...
    const addLate = await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: asset.id, quantity: 1 });
...
test('High-3 多明細fault: 2件目lineのfaultでdispatch全rollback（status=confirmed維持・Movement/usageCount不増）→解除後retryが1回だけ成功', async () => {
  ...
  const reservation = await makeReservation(actor.tenantId);
  try {
    await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: assetA.id, quantity: 1 });
    await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: assetB.id, quantity: 1 });
    await confirmLeaseReservation(actor, reservation.id);
...
test('High-3 並行replay: 同一予約への並列4 dispatch は勝者1本（Movement=line数・usageCount+1・イベント/Audit 各1）', async () => {
  ...
  const reservation = await makeReservation(actor.tenantId);
  try {
    await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: asset.id, quantity: 2 });
    await confirmLeaseReservation(actor, reservation.id);
    const before = await prisma.productAsset.findUnique({ where: { id: asset.id }, select: { usageCount: true } });
...
test('境界の否定: 別tenant・不存在Asset/予約は notfound で書き込み0', async () => {
  ...
  try {
    ...
    const addForeign = await addAssetToLeaseReservation(foreignActor, { reservationId: reservation.id, assetId: asset.id, quantity: 1 });
    ...
    // 不存在 Asset / 予約。
    const addNoAsset = await addAssetToLeaseReservation(actor, { reservationId: reservation.id, assetId: 'asset_does_not_exist_0', quantity: 1 });
    expect(addNoAsset.ok).toBe(false);
    if (!addNoAsset.ok) expect(addNoAsset.reason).toBe('notfound');
    const addNoRsv = await addAssetToLeaseReservation(actor, { reservationId: 'rsv_does_not_exist_0000', assetId: asset.id, quantity: 1 });

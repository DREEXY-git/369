...
1. **CaseStudyConsent 保存条件接続 本番確認記録（doc93）の push（push-only・別承認。feature＋main）**。
2. **anonymized=false の本格扱い判断（運用・表示・閲覧範囲・doc91 §6 段階7・別承認。いきなり解禁しない）／Customer Pain の扱い判断（高機密ラベル対応が先・別承認）**。
3. **AI参照条件の扱い判断（ai_reference purpose の使用・doc91 §6 段階8・別承認）**。
4. **公開活用判断（ApprovalRequest・表現審査・doc91 §6 段階9・別承認。今は進めない）**。

...
describe('getGoldenPathActionForReason — reason ごとの是正アクション', () => {
  it('未送信は請求書へ・延滞/未回収は督促セクション(#dunning)へ・requiresFinance=true', () => {
    expect(getGoldenPathActionForReason('unsent_invoice', ctx).href).toBe('/invoices/inv1');
    for (const r of ['overdue_receivable', 'unpaid'] as AttentionReasonCode[]) {
      const a = getGoldenPathActionForReason(r, ctx);
      expect(a.href).toBe('/invoices/inv1#dunning'); // Phase 1-15: 督促セクションへ deep link
...
describe('actionLabel は実行性を含む（Phase 1-14 インライン是正）', () => {
  it('解消・完了・申請・記録・進む など対処につながる文言', () => {
    ...
    expect(getGoldenPathActionForReason('overdue_receivable', ctx).actionLabel).toContain('督促');
    expect(getGoldenPathActionForReason('unpaid', ctx).actionLabel).toContain('督促');
...
describe('未回収/延滞は督促セクションへ deep link（Phase 1-15）', () => {
  it('延滞/未回収の href は /invoices/{id}#dunning・label は「督促」を含む', () => {
    for (const r of ['overdue_receivable', 'unpaid'] as AttentionReasonCode[]) {
      const a = getGoldenPathActionForReason(r, ctx);
      expect(a.href).toBe('/invoices/inv1#dunning');
      expect(a.actionLabel).toContain('督促');
      expect(a.requiresFinance).toBe(true);
    }
  });

  it('finance 権限なしでは督促アクション（延滞/未回収）が非表示', () => {
    const visible = visibleGoldenPathActions(buildGoldenPathActionLinks({ id: 'e1', invoiceId: 'inv1', attentionReasons: ['overdue_receivable', 'unpaid'] }), false);

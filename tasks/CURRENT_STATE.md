...
1. **CaseStudyConsent 保存条件接続 実装（doc92）の push（push-only・別承認。feature＋main）**。
2. **保存条件接続の本番確認（利用者実測・§0・別承認。拒否→台帳登録→通る→取り消し→拒否の1周＋片付け）**。
3. **anonymized=false の本格扱い判断（運用・表示・閲覧範囲・別承認。いきなり解禁しない）／Customer Pain の扱い判断（高機密ラベル対応が先・別承認）**。
4. **AI参照条件の扱い判断（ai_reference purpose の使用・別承認）**。

...
// v6.4 WIP-4: モバイルドロワーの受入（69導線の DOM 描画・overlay クリックで閉じる・背景スクロール復元・
// focus/aria・role=dialog）。desktop 非退行は上の Sidebar テスト群が担保。
test('v6.4 モバイルドロワー受入: 69導線 DOM 描画・overlay close・背景復元・focus/aria', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await login(page, 'ceo@ikezaki.local');
  await page.goto('/dashboard');
  ...
  // OWNER は 69 導線すべてが DOM に描画される（表示は overflow scroll 内・スクロールで到達可能）。

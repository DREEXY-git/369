...
- **CRM/SFA（LeadMap/Customer/Deal/Account/`/leadmap`）は既存実装済み**（doc121 案A・requireUser/hasPermission('leadmap')/tenantId/writeAudit/OutreachApproval）。

### 21-3. 現在のPhaseで未完了のこと

- CRM/SFA の **Phase 2 完了正式記録が未作成**。
- **高機密ラベル runtime 統制は設計のみ**（doc108〜114・Customer 既定 CUSTOMER_CONFIDENTIAL）。
- **CRM 不足補強（archivedAt/internalNote）未着手**（roadmap22 §15）。
- 回帰ゲート（E2E smoke）最新 green の docs 固定が未整備。

### 21-4. 次に進むPhase

- 事業 Phase 3（AI Growth Engine）／ PDF Phase 3（承認・監査基盤）。

### 21-5. 次のPhaseへ進むために必ず完了すべきこと

1. Phase 2 完了の正式記録（CRM/SFA 含む本番GO/回帰ゲート/不足補強の扱い）。
2. 高機密ラベル runtime 統制の現状監査。
3. 外部送信 Human Certification Gate 運用確認（`leadmap:external_send`＋`OutreachApproval` PENDING）。
4. Consent・SuppressionList 運用確認。
5. AI 境界再確認（AIロール mutation 禁止・外部送信なし・実LLM/AIコスト承認制）。
6. 回帰ゲート（E2E smoke）green 確認。

### 21-6. 次のPhaseへ進んでよいかの判定（GO / HOLD）

- **HOLD（Phase 3 進入は保留）**。

### 21-7. GO / HOLD の理由

- HOLD 理由: Phase 2 完了の正式記録が無い／高機密ラベル runtime 統制が設計のみ／Phase 3 は外部発信リスク（広告/PR/SEO/MA/メール）を持ち Consent・Human Certification Gate の運用確認が前提／回帰ゲート最新 green が docs 上未固定。
- GO 化条件: §21-5 の 1〜6 を満たし、人間 Phase Gate が Phase 2 完了と Phase 3 進入を承認した時点。

### 21-8. 人間承認が必要な判断

- Phase 2 を正式完了と宣言してよいか／Phase 3 進入可否／高機密ラベル runtime 解禁の可否（doc108〜114 の重い承認）／CRM 不足補強を実装するか。

### 21-9. 次Phaseに進む前にやってはいけないこと

- Phase 2 完了記録なしの Phase 3 実装／外部送信・広告自動運用・SNS投稿・PR配信・SEO公開・LINE・DM送信・実LLM・AIコストの解禁／高機密ラベル runtime を Phase Gate 承認なしに解禁／新規 Lead モデル・`/crm/leads`・`crm:*` 新設・schema変更・migration（案A に反する）。

### 21-10. CURRENT_STATE / PROGRESS / roadmap / audit / Obsidian Dashboard にどう反映したか

- **roadmap**: 本書 `docs/roadmap/23`（Phase移行条件本体）。**audit**: `docs/audit/122`（監査記録）。
- **CURRENT_STATE**: doc122 要約1段落（Phase 2 部分完了・Phase 3 HOLD・移行条件6項目）を追記。
- **PROGRESS**: 箇条書き＋見出し「## Phase 2 完了判定・Phase 3 移行条件整理 …」を追記。
- **Obsidian Dashboard**: `OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md` に入口1行（GitHub docs 側のみ）。369-vault は非編集。

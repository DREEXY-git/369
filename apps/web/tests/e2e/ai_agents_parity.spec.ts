...
test('AI社員プロフィール要素の証拠 screenshot（desktop/mobile・切れなし）', async ({ page }) => {
  ...
  for (const vp of [
    ...
    // 詳細のプロフィールカード要素全体（評価コメントまで可視＝切れなし）。
    ...
    await card.screenshot({ path: `test-results/ai-profile-detail-${vp.name}.png` });
    // 3D 側プロフィール要素全体。
    ...
    await officeProfile.screenshot({ path: `test-results/ai-profile-office-${vp.name}.png` });
  }
});

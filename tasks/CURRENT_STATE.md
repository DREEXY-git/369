...
1. **CaseStudyConsent 保存条件接続設計 記録（doc91）の push（push-only・別承認。feature＋main）**。
2. **保存条件接続の §0 人間決定（接続だけ先行 / anonymized=false 方針とセット / 封印寄りのいずれか・別承認）**。
3. **保存条件接続の実装承認判断（validateCaseStudyConsentReconciliation の接続・否定系テスト・安全ゲート・別承認）**。
4. **anonymized=false の扱い判断（突合判定接続後・別承認。いきなり解禁しない）／Customer Pain の扱い判断（高機密ラベル対応が先・別承認）**。

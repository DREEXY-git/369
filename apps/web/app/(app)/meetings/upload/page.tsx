...
export default async function MeetingUploadPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  // upload 1回分の冪等トークン。同一フォームの再送信（二重click/リトライ/戻る→再送）は
  // 同一 Meeting へ収束し、重複会議を作らない。新しい取込はページ再表示で新トークンになる。
      ...
      <Card>
        ...
        <CardContent>
          {sp.error && ERROR_MESSAGES[sp.error] ? (
            <div className="mb-3 rounded bg-red-50 px-3 py-2 text-xs text-red-700">{ERROR_MESSAGES[sp.error]}</div>
          ) : null}

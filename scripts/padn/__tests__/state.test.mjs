...
// fixtures（L1 実データ模擬）は owner が投稿するため、production と同じく policy 相当の
// 信頼リストを渡して fold する（discover.buildSnapshot が行うのと同じ合成）
const TRUSTED = { trustedVerdictAuthors: ['DREEXY-git', 'github-actions[bot]'] };

test('WIP fold: B2 実ライフサイクル（dispatch→claim→freeze→PASS→human gate）', () => {
  const world = standardWorld();
  const fold = foldWipState(world.commentsByIssue[68], TRUSTED);
  assert.equal(fold.state, 'READY_FOR_HUMAN_GATE');
  assert.equal(fold.frozenHead, world.B2_HEAD);
  assert.equal(fold.reworkCount, 0);
  const b1fold = foldWipState(world.commentsByIssue[67], TRUSTED);
  assert.equal(b1fold.state, 'DISPATCHED'); // 未クレーム
});

test('WIP fold: 信頼リスト外の投稿者による状態遷移 marker は無視される（Codex R11 P2）', () => {
  const mk = (at, body) => ({ id: 1, created_at: at, user: { login: 'github-actions[bot]' }, body });
  ...
  // 第三者の散文 READY_FOR_HUMAN_GATE ではレーンが進まない（監査スキップ偽装の防止）
  const spoofed = foldWipState([
    ...base,
    { id: 2, created_at: '2026-07-17T01:30:00Z', user: { login: 'third-party-user' }, body: 'READY_FOR_HUMAN_GATE にすべきだ' },
  ]);
  assert.equal(spoofed.state, 'FROZEN_FOR_REVIEW');
  // 信頼リスト内（Director/owner）の READY_FOR_HUMAN_GATE は従来どおり有効
  const legit = foldWipState(
    [...base, { id: 3, created_at: '2026-07-17T01:30:00Z', user: { login: 'DREEXY-git' }, body: 'READY_FOR_HUMAN_GATE' }],
    TRUSTED,
  );
  assert.equal(legit.state, 'READY_FOR_HUMAN_GATE');
});

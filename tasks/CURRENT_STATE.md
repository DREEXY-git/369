...
1. **ConsentRecord連携器選択 記録（doc83）の push（push-only・別承認。feature＋main）**。
2. **CaseStudyConsent schema 実装判断（doc83 §12 Unknowns 4点の人間確定 → schema-only・追加のみ migration・段階承認・いきなり実装しない）**。

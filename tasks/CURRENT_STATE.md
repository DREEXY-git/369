...
- **Phase 2-B-3: SalesPlaybookEntry read-only 可視化 — main反映済み（`a2bb2b6`）・本番確認 HOLD（実装記録: doc54／HOLD記録: doc55＋doc14 §47・2026-07-05）**。最新の詳細実測で**症状はナビ表示のみに絞られた**: ナビに「営業プレイブック」が出ない=NG（唯一のNG）。一方 **/brain/playbooks 直打ちは開き・空一覧（本番 seed 未実行のため正常）・ボタンなし・既存画面・エラーなし・外部送信なし=すべてOK**。GO記録は main 反映前に差し止めて HOLD 訂正済み。**原因は未特定・断定しない**（有力観点: ナビ=共通部品のキャッシュ/反映タイミング=doc37前例と同型／repo側 nav 表示条件の read-only 確認）。**次は read-only 原因調査**（コード修正・DB・認証・RBAC・本番環境・Vercel環境変数は変更しない）。書き込み（2-B-4）は HOLD 解消まで進まない。seed デモデータ6件（playbookType 4種網羅・PII/実価格/口コミ/顧客の声ゼロ・全件 externalAiAllowed=false・NORMAL/INTERNAL のみ）＋read-only 一覧 `/brain/playbooks`（knowledge:read＋tenantId・作成/編集/削除/Server Action なし）＋ナビ1行＋**smoke 16/16 green（既存15本回帰なし）**。書き込み（2-B-4）・AI参照（2-B-5）は別承認。本番は seed 未実行のため一覧が空で正常（doc49 原則で本番確認条件を事前定義済み）。
...
- 注記: **Phase 2-B-3（read-only 可視化・`a2bb2b6`）は main反映済みだが本番確認 HOLD**（症状はナビ表示のみ: ナビに「営業プレイブック」が出ない。/brain/playbooks 直打ちは正常・doc55＋doc14 §47）。**HOLD 解消（再実測GO）まで基準は Phase 2-B-2 のまま**。
...
2. **Phase 2-B-3 の read-only 原因調査**（別ミッション。症状は「ナビに営業プレイブックが出ない」に特定済み。本番再実測=ハードリロード・別ブラウザ・ログアウト/ログイン後のナビ確認＋repo側=nav.ts の該当1行の main 反映内容と表示条件の read-only 確認。**コード修正・DB・認証・RBAC・本番環境・Vercel環境変数の変更はしない**）→ 原因特定 → 再実測 → GO記録（doc56 候補）。

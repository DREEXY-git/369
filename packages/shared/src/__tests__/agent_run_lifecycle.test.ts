...
describe('maskRunError v6.3（escaped-quoted 値の内部 delimiter・改行の完全封鎖）', () => {
  ...
  it('quoted 値の後は malformed の可能性を優先して末尾まで fail-closed にする', () => {
    const out = maskRunError('config password: "sec-01" and then connected to host ok', 500);
    expect(out).not.toContain('connected to host ok');
...
// v6.5: 引用符 depth を推測しない fail-closed 回帰。開始 depth 0..4、偽 closer は開始と同 depth、
// malformed terminal は別 depth 0..6、区切り6種を総当たりする（180ケース）。
describe('maskRunError v6.5（一般化 malformed quote-depth matrix）', () => {
  it('全 depth・delimiter 組合せで sentinel を一文字列として残さない', () => {
    const delimiters = [',', ' ', ';', '\n', '}', ']'];
    let checked = 0;
    for (let openDepth = 0; openDepth <= 4; openDepth++) {
      for (let terminalDepth = 0; terminalDepth <= 6; terminalDepth++) {
        ...
        for (let d = 0; d < delimiters.length; d++) {
          const sentinel = `DEPTH_${openDepth}_${terminalDepth}_${d}_SECRET`;
          ...
          const out = maskRunError(input, 500);
          expect(out, `leaked for open=${openDepth} terminal=${terminalDepth} delimiter=${JSON.stringify(delimiters[d])}`)

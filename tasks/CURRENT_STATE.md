...
1. **CaseStudyConsent 突合判定設計 記録（doc89）の push（push-only・別承認。feature＋main）**。
2. **突合判定の実装承認判断（doc89 準拠・validateCaseStudyConsentReconciliation 案・否定系テスト・安全ゲート・別承認）**。
3. **anonymized=false の扱い判断（突合判定後・別承認。いきなり解禁しない）**。

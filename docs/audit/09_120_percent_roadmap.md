...
## Phase 1-9 進捗（2026-06-24）— 候補→正式化 ＋ CashFlow接続 ＋ 保守リファクタ

- **候補→正式化**: 承認済み JournalCandidate→正式 JournalEntry/JournalEntryLine（Account自動解決）、InvoiceCandidate→正式 Invoice(ISSUED)/InvoiceLineItem/Receivable。executeApprovedAction（冪等・二重実行防止）。外部送信なし。
- **CashFlow接続**: /finance/cashflow に FinanceEvent(cashflow_expected) の入金/支払予定を非破壊追加。
- **保守リファクタ第一歩**: lib/domains/operations/procurement.ts・stocktake.ts を新設、対応 action を薄く（behavior不変）。**新規DBモデルなし**。
- 機密は finance 権限ゲート＋writeConfidentialViewLog。DomainEvent 3種・GrowthEvent 3種追加。
- unit 151 / integration 58、lint/typecheck/build green。e2e finance_formalize.spec.ts 追加。
- これで **現場実行 → Finance Bridge → 候補 → 承認 → 正式仕訳/正式請求書 → 資金繰り** が接続。
- 次: Phase 1-10（請求の外部送信ゲート＋入金消込、operations/actions.ts の継続分割）または Phase 2（試算表/決算書・OCR→仕訳）。

## Phase 1-10 進捗（2026-06-24）— 請求送信ゲート＋入金消込＋資金繰り統合

- **請求送信ゲート**: 正式 Invoice(ISSUED)→承認(invoice_send)→`executeApprovedAction`で SENT。送信前 `prepareExternalPayload`(PIIマスク)＋`assertAiToolAllowed`(AI送信禁止)。EXTERNAL_SEND_ENABLED時のみ実送信。二重送信防止。
- **入金消込**: `recordInvoicePayment` で Payment/Invoice(status)/Receivable(status)/FinanceEvent(payment_received posted)を連動。全額入金で関連入金予定を実績化。
- **資金繰り統合**: /finance/cashflow に 予定 vs 実績（入金/支払・差引・直近30日実績・支払超過警告）を非破壊追加。
- **保守リファクタ**: lib/domains/operations/events.ts へ移設、operations/actions.ts 713→626行。
- **新規DBモデルなし**。unit 160 / integration 64、6コマンド green。e2e invoice_payment.spec.ts。
- これで **正式請求書→承認後送信→入金記録→売掛金回収→資金繰り実績反映** が接続。
- 次: Phase 1-11（支払実行ゲート＝買掛の支払、operations/actions.ts 継続分割）または Phase 2（試算表/決算書・OCR→仕訳）。

## Phase 1-11（2026-06-24）— Golden Path Hardening

- 達成: プランニングホッコー中心フロー20ステップを監査し、継ぎ目（特に案件→Finance Bridge）をUIで接続。18/20がUIから一本で通る状態に。
- 新規DBモデル: なし。新機能の横展開なし（既存の縦スライスを実用レベルへ）。
- 残（次の山）: 経営ダッシュボードへの案件横断KPI集約（ステップ20）、リース予約統合（ステップ4）、承認種別 invoice_finalize/invoice_send の分離。

## Phase 1-12（2026-06-24）— Golden Path KPI Executive Dashboard

- 達成: 案件横断の Golden Path を経営 KPI に集約し **ステップ20を完成（△→✅）**。社長が「危ない案件・次の一手・未回収/粗利/資金繰り影響・承認待ち」を一目で把握。「今すぐ見るべき案件」を優先度付けで提示。
- 新規DBモデル: なし。新機能の横展開なし（既存縦スライスの経営可視化）。finance 機密は lib 段階で redact（STAFF不可視）＋ planning-hokko の金額無ゲートを是正。
- 残（次の山）: `completedAt` 導入で「今月完了/リードタイム」精緻化、是正アクションへのディープリンク、規模拡大時の KPI キャッシュ化、リース予約統合（ステップ4）、承認種別分離。

## Phase 1-13（2026-06-24）— Golden Path Action Deep Links + completedAt + Approval 分離

- 達成: 「危ない案件」を見つけた瞬間に正しい是正アクションへ進める導線を整備（見える→気づく→**すぐ対処する**）。reason ごとの deep link、event detail アンカー、AttentionList の「対処」ボタン。
- 精緻化: `EventProject.completedAt`（最小フィールド）で「今月完了」を正確化（completedAt 優先・fallback）。Approval を `invoice_finalize`/`invoice_send` に意味分離（後方互換）。
- 新規DBモデル: なし（completedAt の1列のみ）。横展開なし。finance 系アクションは STAFF 非表示（深層防御）。
- 残: 是正アクションのインライン実行、リードタイム時系列、関数名リネーム。

## Phase 1-14（2026-06-24）— Golden Path Inline Corrective Actions

- 達成: 是正アクションの一部を**その場で実行/申請**（物流完了・リスク解消・Finance Bridge・送信承認申請・入金記録）。「見える→気づく→飛ぶ→対処する」まで到達。
- 既存 action 再利用中心・**新規 server action ゼロ・新規DBモデルゼロ**。外部送信は承認ゲート不変、督促メールは未実装。
- 位置づけ: IKEZAKI OS 全体は初期段階。プランニングホッコー向け実用MVPとして Golden Path の実務性が一段向上。
- 残: 督促の承認付きドラフト、リードタイム時系列、関数名リネーム。

...
test('承認実行 R5: 同一 approval の 2 並列実行は claim CAS で直列化し、PO/Audit/Domain/Outbox/Growth 各 1・Approval exactly-once executed（Codex R5 #4）', async () => {
  const t = await tenantId();
  const uid = await ceoUserId();
  const { poId, assetId, approvalId } = await confirmApprovePendingPo(t, uid);
  const human = { tenantId: t, userId: uid };
  try {
    // 同一 approval を 2 本並列実行。Approval claim の updateMany（executedAt=null 限定）行ロックで直列化され、
    // 勝者 1 本のみが PO CAS→Evidence→終端まで到達し、敗者は先行 commit 後に executedAt!=null を見て already-executed。
    const [a, b] = await Promise.all([
      executeApprovedPurchaseOrderIssue(human, approvalId, poId),
      executeApprovedPurchaseOrderIssue(human, approvalId, poId),
    ]);
    const executed = [a, b].filter((r) => r.executed);
    const rejected = [a, b].filter((r) => !r.executed);
    expect(executed.length, '実行成功はちょうど 1（exactly-once）').toBe(1);
    expect(rejected.length, '敗者はちょうど 1').toBe(1);
    expect(rejected[0]?.reason, '敗者は already-executed で fail-closed').toBe('already-executed');
    // PO/Audit/Domain/Outbox/Growth 各 1・Approval executed（二重 lineage を作らない）。
    await assertPoExecutedOnce(t, poId, approvalId);
  } finally {
    await cleanup(poId, assetId);
  }

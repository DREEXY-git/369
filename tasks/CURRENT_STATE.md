...
1. **Phase 2-B-5-PROD GO記録（doc61＋doc14 §50）の push（push-only・別承認。feature＋main）**。
2. **Phase 2-B 全体クローズ判定（doc48 の 2-A-CLOSE と同じ型・doc62 候補・別承認）**。
3. 改善候補（任意・別承認）: 頭脳2画面→プレイブックのタブ導線・アーカイブ文言/視認性・実LLMキー設定（外部送信解禁とセット）・CI 導入・否定系テスト・doc49 script化。

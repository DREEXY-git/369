...
1. **Phase 2-C-2 記録 commit（doc72・CaseStudy schema）の push（push-only・別承認。feature＋main）**。
2. **push 後の本番確認（利用者実測・doc49 の型。build 成功＝migration 反映と既存画面無回帰の確認）**。
3. その後 **Phase 2-C-3（read-only 画面＋架空 seed デモデータ・doc73 候補）の承認判断**。または **Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・並行可）**。

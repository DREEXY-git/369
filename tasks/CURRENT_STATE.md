...
1. **CaseStudyConsent 突合判定実装 記録（doc90）の push（push-only・別承認。feature＋main）**。
2. **保存条件への接続の承認判断（doc89 §13 段階3・anonymized=false の扱い（段階6）とセット設計か接続先行かの人間選択・別承認）**。
3. **anonymized=false の扱い判断（突合判定接続後・別承認。いきなり解禁しない）**。

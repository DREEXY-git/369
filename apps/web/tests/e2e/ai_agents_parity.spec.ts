...
// CI の artifact は `apps/web/test-results/**/*.png` を集めるため、明示 path で書き出す（visual_evidence と同方式）。
test('AI社員プロフィール要素の証拠 screenshot（desktop/mobile・切れなし）', async ({ page }) => {
  test.setTimeout(120_000);
  await login(page, 'ceo@ikezaki.local');
  await page.goto('/ai-agents');
  ...
  for (const vp of [
    ...
    // 詳細のプロフィールカード要素全体。v6.9 WIP-3: stitching の header 重なりで上端/評価/下端が
    // 欠ける問題を captureFullElement（viewport 拡大の単一フレーム撮影＋PNG 実寸 assert）で解消。
    ...
    await captureFullElement(page, card, `test-results/ai-profile-detail-${vp.name}.png`);
    // 3D 側プロフィール要素全体（上端の epithet/名前〜評価コメント〜下端まで切れなし）。
    ...
    await expect(officeProfile.getByTestId('ai-office-profile-epithet')).toBeVisible();
    await expect(officeProfile.getByTestId('ai-office-profile-eval')).toBeVisible();
    await captureFullElement(page, officeProfile, `test-results/ai-profile-office-${vp.name}.png`);
  }
});
...
test.describe('cross-tenant child isolation（agentId 一致・tenantId 別の子は非表示）', () => {
  ...
  test.beforeAll(async () => {
    ...
    const own = ceo
      ...
      : null;
    if (!ceo || !own) return; // seed 未整備なら ownAgentId/ownTenantId 未設定のまま → 下の test 内 fail-fast assert で検出（skip はしない）
    ownAgentId = own.id;
  ...
  test('別テナントの run/action/memory は自テナント agent 詳細に漏れない', async ({ page }) => {
    // silent skip 禁止（Codex #4965087729 P2-2）: fixture（自テナント agent＝seed／別テナント＝beforeAll 作成）を fail-fast 検証。
    expect(Boolean(ownAgentId && foreignTenantId), 'fixture 未整備（seed AI agent または foreign tenant 作成失敗）').toBe(true);
    await login(page, 'ceo@ikezaki.local');
  ...
  test('v6.7: 自テナントの run でも参照先が別テナント agent なら一覧に出ない（agent 名も漏れない）', async ({ page }) => {
    // silent skip 禁止（Codex #4965087729 P2-2）: fixture（別テナント agent＝beforeAll 作成／自テナント＝ceo）を fail-fast 検証。
    expect(Boolean(foreignAgentId && ownTenantId), 'fixture 未整備（foreign agent または own tenant 取得失敗）').toBe(true);
    await login(page, 'ceo@ikezaki.local');
    await page.goto('/ai-agents');

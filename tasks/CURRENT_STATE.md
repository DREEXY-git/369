...
1. **CaseStudyConsent UI設計 記録（doc86）の push（push-only・別承認。feature＋main）**。
2. **CaseStudyConsent UI 実装承認判断（doc86 準拠・人間のみ・writeAudit・安全ゲート・smoke・doc87 候補・別承認）**。
3. **突合判定の設計・実装判断（doc83 §9 段階3・validateCaseStudyConsent 拡張・否定系テスト・安全ゲート・別承認）**。

...
1. **Phase 2-C-3-PROD 記録（doc75）の push（push-only・別承認。feature＋main）**。
2. **Phase 2-C-4（CaseStudy 人間書き込み＋writeAudit＋許諾/匿名化制御＋静的ゲート拡張・doc76 候補）の承認判断**（doc75 §5 に推奨プロンプト案）。
3. または **Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・2-C-4 と並行可）**。

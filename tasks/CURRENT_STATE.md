...
1. **X-05-3 実装 commit（doc68＋script＋CI step）の push（push-only・別承認。feature＋main）→ CI 実走確認（新 step 込み・利用者・doc69 候補）**。
2. **Stage 2（build の CI 追加）/ ★2 権限拒否テストの承認判断**。

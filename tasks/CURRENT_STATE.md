...
1. **CaseStudyConsent anonymized=false 本格扱い設計 記録（doc94）の push（push-only・別承認。feature＋main）**。
2. **anonymized=false 本格扱いの §0 人間決定（運用・表示・閲覧範囲・別承認。いきなり解禁しない）**。
3. **UI/表示方針の最小実装判断（別承認）**。
4. **AI参照条件の扱い判断（ai_reference purpose の使用・別承認）／公開活用判断（ApprovalRequest・表現審査・別承認）**。
5. **Customer Pain / Stage 2・3・★2・UX / CI / Test / Release Governance 等の品質基盤強化（別承認）**。

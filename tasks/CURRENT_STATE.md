...
1. **CaseStudyConsent AI参照条件設計 記録（doc97）の push（push-only・別承認。feature＋main）**。
2. **AI参照条件の §0 人間決定（KEEP_ANONYMIZED_TRUE_ONLY 維持または別案・別承認）**。
3. **AI参照条件の実装判断（実装する場合も別承認。company-brain-reference 変更は個別承認）**。
4. **公開活用判断（ApprovalRequest・表現審査・別承認。今は進めない）／Customer Pain の扱い判断（高機密ラベル対応が先・別承認）**。
5. **Stage 2 / Stage 3 / ★2 / UX改善／CI / Test / Release Governance 等の品質基盤強化（別承認）**。

...
1. **Phase 2-C-4-PROD 記録（doc77）の push（push-only・別承認。feature＋main）**。
2. **Phase 2-C-5（CaseStudy AI参照・doc78 候補・別承認）の承認判断**（doc77 §5 に骨子: 匿名化済みのみ参照等の条件設計から）。または **ConsentRecord 連携設計（許諾運用フロー・別承認）**。
3. または **Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・並行可）**。

...
| `C03-022` | `CI_VERIFIED` | ContentAssetの下書き・却下状態から内部`ApprovalRequest(content_review)`を1件だけ作る承認依頼 | PR #18 `dd54ce9`、`content-review-bridge.ts`、独立state oracle 11/11、CI `29186179985` | Draft限定。公開・CMS投稿・外部送信・実LLM・課金は未接続。実DB並行再現はEvidence Gap |
| `C03-025` | `CI_VERIFIED` | `PENDING`限定CASと対象ContentAssetの状態を単一transactionで承認・却下へ遷移 | PR #18 `dd54ce9`、unit 472、E2E 131、独立state oracle | main・Preview・Production未確認。実DB並行approve/rejectは未実測 |
| `C03-026` | `CI_VERIFIED` | 人間の決定noteを`decisionNote`へ記録 | PR #18 `dd54ce9`、`decideContentReviewCore`、CI `29186179985` | コメント履歴・編集・複数往復の完成ではない |
| `C03-028` | `CI_VERIFIED` | 人間却下でApprovalRequestとContentAssetを同一transaction内の`REJECTED/rejected`へ遷移 | PR #18 `dd54ce9`、契約test 12/12、独立state oracle | 汎用Approval全typeの却下原子性を意味しない |
| `C03-032` | `CI_VERIFIED` | コンテンツreview-only申請を`riskLevel=LOW`として人間承認へ送る限定経路 | PR #18 `dd54ce9`、metadata-only payload、CI `29186179985` | リスク自動算定・業務別ルール全体ではない |
| `C03-039` | `CI_VERIFIED` | 却下済みContentAssetをCASで再申請可能にする限定経路 | PR #18 `dd54ce9`、request rollback/retry独立oracle | 実DBでの同時再申請は未実測 |
| `C03-048` | `CI_VERIFIED` | content_review申請・決定の監査へ人間actorIdを記録 | PR #18 `dd54ce9`、transaction内`auditLog.create`、独立state oracle | 全承認type・IP/UA・代理承認の証拠ではない |
| `C03-050` | `CI_VERIFIED` | content_review監査へapproval_request / approve / reject actionと対象IDを記録 | PR #18 `dd54ce9`、transaction契約test、CI `29186179985` | 変更前後diff・汎用Audit Graph全体ではない |
...
| `C28-017` | `CI_VERIFIED` | AI社員8名の3D/2D read-onlyダッシュボード、canonical profile、2D/canvas選択とURL・back/forward同期 | PR #14 `ba01244`を継承したPR #18 `dd54ce9`、CI 472/131、artifact `8258122858` | main・Preview・Production未確認。mobile topbarで通知・ユーザー操作が画面外になるP2あり |
...
| `C30-052` | `CI_VERIFIED` | 直近`AIAgentRun=FAILED`のread-only表示、FAILED記録後の安全なrethrow、秘密形状のbounded fail-closed mask | PR #14 `ba01244`、独立Grammar 1,687件＋旧thread 25件の4経路marker 0、unit 452、E2E 127 | BullMQ実queueのretry/failed telemetry統合証拠なし。未知入力を含む脆弱性ゼロは宣言しない |
...
| `USR-003` | `CI_VERIFIED` | PR #18 `dd54ce9`に継承された3D AI Office、8名canonical parity、2D/canvas URL同期、profile全体画像、NAV 67導線 | CI `29186179985`、artifact `8258122858` | main・Vercel Preview・Production未確認。mobile topbarの通知・ユーザー操作clipはP2修正待ち |
...
| `USR-007` | `EVIDENCE_GAP` | PR #14 `ba01244`で既報Grammar P1を独立1,687件＋旧thread 25件の4経路marker 0まで確認 | BullMQ実queue retry、第三者診断、未知入力、main/Preview/Productionは未確認。「脆弱性0」は断定しない |
...
| PR #18のC21 `content_review`承認ブリッジ | `UNMAPPED_CANDIDATE` | `dd54ce9`、内部ApprovalRequest、transaction契約test 12/12、独立state oracle 11/11、CI `29186179985` | C21台帳に承認workflow固有IDがない。C03既存IDの限定証拠へ接続し、C21カテゴリ完成・公開実行へ格上げしない |

...
describe('redactExecutiveFinance — finance 権限ゲート（データ整形段階）', () => {
  ...
  it('invoiceId も finance 機密として redact（null 化）', () => {
    const d0 = summarizeExecutiveDashboard([fact({ id: 'x', invoiceId: 'inv-x', receivableOverdue: true, unpaidAmount: 1000, invoiceFormalized: true, invoiceSent: true })], opts());
    expect(d0.projects[0]!.invoiceId).toBe('inv-x'); // finance可視ならそのまま
    const d1 = redactExecutiveFinance(d0, false);
    expect(d1.projects[0]!.invoiceId).toBeNull();
  });
});

describe('今月完了の判定（completedAt 優先・Phase 1-13）', () => {
  it('completedAt が当月なら今月完了（status/eventDate に依らない）', () => {
    const d = summarizeExecutiveDashboard(
      [
        fact({ id: 'a', status: 'completed', completedAt: new Date('2026-06-10T00:00:00Z'), eventDate: new Date('2026-05-01T00:00:00Z') }),
        fact({ id: 'b', status: 'completed', completedAt: new Date('2026-05-10T00:00:00Z'), eventDate: new Date('2026-06-01T00:00:00Z') }),
      ],
      opts(),
    );
    expect(d.overall.monthCompletedCount).toBe(1); // a のみ（completedAt 当月）。b は completedAt 先月
  });

  it('completedAt が null なら status=completed かつ eventDate 当月で代理（fallback）', () => {
    const d = summarizeExecutiveDashboard(
      [
        fact({ id: 'a', status: 'completed', completedAt: null, eventDate: new Date('2026-06-15T00:00:00Z') }),
        fact({ id: 'b', status: 'completed', completedAt: null, eventDate: new Date('2026-05-15T00:00:00Z') }),
        fact({ id: 'c', status: 'planned', completedAt: null, eventDate: new Date('2026-06-15T00:00:00Z') }),
      ],
      opts(),
    );
    expect(d.overall.monthCompletedCount).toBe(1); // a のみ（completed かつ eventDate 当月）
  });

  it('isCompletedInMonth: completedAt 優先、無ければ fallback', () => {
    const start = monthStart;
    const end = monthEnd;
    expect(isCompletedInMonth(fact({ completedAt: new Date('2026-06-10T00:00:00Z'), status: 'planned' }), start, end)).toBe(true);
    expect(isCompletedInMonth(fact({ completedAt: new Date('2026-05-10T00:00:00Z'), status: 'completed', eventDate: new Date('2026-06-10T00:00:00Z') }), start, end)).toBe(false);
    expect(isCompletedInMonth(fact({ completedAt: null, status: 'completed', eventDate: new Date('2026-06-10T00:00:00Z') }), start, end)).toBe(true);
    expect(isCompletedInMonth(fact({ completedAt: null, status: 'planned', eventDate: new Date('2026-06-10T00:00:00Z') }), start, end)).toBe(false);
  });
});

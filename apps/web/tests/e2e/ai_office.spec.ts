...
test('一覧から AI 社員を選択すると詳細パネルに実測状態とプロフィール（設定）が表示され、フィルタが機能する', async ({ page }) => {
  ...
  // 証拠スクリーンショット。外枠ではなくプロフィール本体だけを撮り、sticky header と余白の
  // stitching 混入を防ぐ（画像単体で人物ヘッダーから評価まで読める証拠にする）。
  const profileEl = detail.getByTestId('ai-office-profile');
  await profileEl.scrollIntoViewIfNeeded();
  await page.evaluate(() => document.querySelector('header')?.style.setProperty('visibility', 'hidden'));
  await profileEl.screenshot({ path: 'test-results/ai-office-desktop-profile.png' });
...
test('モバイルでは操作可能な簡略表示に切り替わる', async ({ page }) => {
  ...
  const profileEl = detailEl.getByTestId('ai-office-profile');
  await profileEl.scrollIntoViewIfNeeded();
  await page.evaluate(() => document.querySelector('header')?.style.setProperty('visibility', 'hidden'));
  await profileEl.screenshot({ path: 'test-results/ai-office-mobile-profile.png' });
  await page.evaluate(() => document.querySelector('header')?.style.removeProperty('visibility'));
...
test('client-side の ?agent= 変更と back/forward で選択社員がURLに同期する', async ({ page }) => {
  await login(page, 'ceo@ikezaki.local');
  await page.goto('/ai-office');
  const buttons = page.getByTestId('ai-office-2d-list').getByRole('button');
  await expect(buttons).toHaveCount(8);
  // 初期/2番目を実際に選択して detail の id を取得する。
  await buttons.nth(0).click();
  const id1 = (await page.getByTestId('ai-office-detail').getAttribute('data-agent-id'))!;
  await buttons.nth(1).click();
  const id2 = (await page.getByTestId('ai-office-detail').getAttribute('data-agent-id'))!;
  expect(id1).not.toBe(id2);

  await page.goto(`/ai-office?agent=${id1}`);
  await expect(page.getByTestId('ai-office-detail')).toHaveAttribute('data-agent-id', id1);
  await page.evaluate((nextId) => window.history.pushState({}, '', `/ai-office?agent=${nextId}`), id2);
  await expect(page.getByTestId('ai-office-detail')).toHaveAttribute('data-agent-id', id2);
  await page.goBack();
  await expect(page.getByTestId('ai-office-detail')).toHaveAttribute('data-agent-id', id1);

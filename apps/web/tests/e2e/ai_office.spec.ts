...
test('v6.8 P2: 3D canvas の実クリック選択が URL ?agent= を更新し、back/forward で同期する', async ({ page }) => {
  ...
  const initialId = await office.getAttribute('data-agent-id'); // 先頭自動選択（URL は増やさない）

  // 別 index のネームプレートを実クリック → raycast onClick → selectAgent → router.push(?agent=)。
  const probe = await page.evaluate(() => {
    const c = document.querySelector('canvas[data-testid="ai-office-canvas"]') as HTMLCanvasElement & {
      __nameplateProbe?: (i: number) => { ndcX: number; ndcY: number; agentId: string } | null;
      __pickAt?: (x: number, y: number) => string | null;
    ...
    for (const i of [1, 2, 3, 4, 5, 6, 7]) {
      ...
      if (p && c.__pickAt!(p.ndcX, p.ndcY) === p.agentId) return p;
    }
    return null;
  });
  ...
  // canvas click で detail ID と URL の両方が更新される（2D 一覧と同じ経路）。
  ...
  // back → 直前状態（?agent= 無し=初期自動選択）へ戻り、URL の agent が消える。
  await page.goBack();
  await expect(page).not.toHaveURL(new RegExp(`agent=${probe!.agentId}`));
  if (initialId) await expect(office).toHaveAttribute('data-agent-id', initialId);
  // forward → 再び canvas 選択へ。

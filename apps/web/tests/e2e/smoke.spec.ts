...
test('顧客事例の許諾台帳に登録・取り消しできる（人間のみ・証跡は所在説明のみ）', async ({ page }) => {
  // CaseStudyConsent 台帳UI（doc86/doc87）: 事例作成 → 許諾登録 → 一覧表示 → 取り消し → 片付け（アーカイブ）の1周。
  await login(page);
  await page.goto('/brain/case-studies');
  await page.getByRole('link', { name: '新規作成' }).click();
  await page.waitForURL('**/brain/case-studies/new');
  const consentCaseTitle = `E2E許諾台帳事例-${Date.now()}`;
  await page.getByLabel('タイトル（必須・120文字まで）').fill(consentCaseTitle);
  await page.getByLabel('本文（必須・5000文字まで）').fill('E2Eテスト用の架空・匿名の社内事例です。許諾台帳の動作確認のみに使います。PII・secretを含みません。');
  await page.getByRole('button', { name: '作成する' }).click();
  await page.waitForURL('**/brain/case-studies');

  // 編集画面経由で許諾台帳へ（doc86 の導線）
  const caseRow = page.locator('tr', { hasText: consentCaseTitle });
  await caseRow.getByRole('link', { name: '編集' }).click();
  await page.waitForURL('**/edit');
  await page.getByRole('link', { name: '許諾台帳' }).click();
  await page.waitForURL('**/consents');
  await expect(page.getByRole('heading', { name: '許諾台帳' })).toBeVisible();
  await expect(page.getByText('所在説明のみ', { exact: false }).first()).toBeVisible();

  // 登録（purpose 1つ・evidence は所在説明・期限必須）
  await page.getByRole('link', { name: '許諾を登録' }).click();
  await page.waitForURL('**/consents/new');
  await expect(page.getByText('原本の本文・メール本文・個人情報', { exact: false })).toBeVisible();
  await page.getByLabel('社内閲覧（internal_view）').check();
  await page.getByLabel('証跡の所在説明（必須・1000文字まで）').fill('証跡所在: テスト用の架空メモ（E2E）');
  await page.getByLabel('許諾日（必須）').fill('2026-07-01');
  await page.getByLabel('有効期限（必須）').fill('2027-07-01');
  await page.getByRole('button', { name: '登録する' }).click();
  await page.waitForURL('**/consents');
  await expect(page.getByText('証跡所在: テスト用の架空メモ（E2E）')).toBeVisible();
  await expect(page.getByText('有効（登録済み）')).toBeVisible();

  // externalAiAllowed / publishStatus の UI が無いこと（封印の維持）
  await expect(page.locator('input[name="externalAiAllowed"]')).toHaveCount(0);
  await expect(page.locator('select[name="publishStatus"], input[name="publishStatus"]')).toHaveCount(0);
  ...
  // 片付け: テスト事例をアーカイブ（台帳の行は物理削除しない）
  await page.goto('/brain/case-studies');
  const cleanupRow = page.locator('tr', { hasText: consentCaseTitle });
  await cleanupRow.getByRole('button', { name: 'アーカイブ' }).click();
  await page.waitForURL('**/brain/case-studies');
  await expect(page.getByText(consentCaseTitle)).toHaveCount(0);
});

test('実名寄り（匿名化オフ）事例の1周ができる（台帳登録→匿名化オフ保存→バッジ表示→匿名化に戻す→片付け）', async ({ page }) => {
  // anonymized=false 本格扱い（doc94 §0・doc95）: 保存条件接続（doc92）の上に badge_only 表示が乗ることを確認。
  // 実名寄りは AI参照対象外・外部公開不可のバッジ付きで社内限定表示。最後は匿名化に戻して片付ける。
  await login(page);
  await page.goto('/brain/case-studies');
  await page.getByRole('link', { name: '新規作成' }).click();
  await page.waitForURL('**/brain/case-studies/new');
  const restrictedTitle = `E2E実名寄り事例-${Date.now()}`;
  await page.getByLabel('タイトル（必須・120文字まで）').fill(restrictedTitle);
  await page.getByLabel('本文（必須・5000文字まで）').fill('E2Eテスト用の架空・匿名の社内事例です。実名寄り表示の動作確認のみに使います。PII・secretを含みません。');
  await page.getByRole('button', { name: '作成する' }).click();
  await page.waitForURL('**/brain/case-studies');

  // 許諾台帳に internal_view の有効な行を登録（保存条件接続の前提・doc92）
  const targetRow = page.locator('tr', { hasText: restrictedTitle });
  ...
  await page.waitForURL('**/consents/new');
  await page.getByLabel('社内閲覧（internal_view）').check();
  await page.getByLabel('証跡の所在説明（必須・1000文字まで）').fill('証跡所在: テスト用の架空メモ（E2E実名寄り）');
  await page.getByLabel('許諾日（必須）').fill('2026-07-01');
  await page.getByLabel('有効期限（必須）').fill('2027-07-01');
  await page.getByRole('button', { name: '登録する' }).click();
  await page.waitForURL('**/consents');

  // 編集画面で許諾あり＋匿名化オフにして保存（突合判定を通過して保存できる）
  await page.goto('/brain/case-studies');
  await page.locator('tr', { hasText: restrictedTitle }).getByRole('link', { name: '編集' }).click();
  await page.waitForURL('**/edit');
  await page.getByLabel('許諾状態').selectOption('granted');
  await page.getByLabel('匿名化する（推奨・既定）').uncheck();
  await page.getByRole('button', { name: '保存する' }).click();
  await page.waitForURL('**/brain/case-studies');

  // badge_only: 実名寄り行に注意バッジが出る（AI参照対象外・外部公開不可）
  const restrictedRow = page.locator('tr', { hasText: restrictedTitle });
  await expect(restrictedRow.getByText('実名寄り（許諾あり）')).toBeVisible();
  await expect(restrictedRow.getByText('AI参照対象外')).toBeVisible();
  await expect(restrictedRow.getByText('外部公開不可')).toBeVisible();

  // 編集画面には手動匿名化戻しの注意（自動では戻らない・doc94 §0 require_manual_reanonymize）
  await restrictedRow.getByRole('link', { name: '編集' }).click();
  await page.waitForURL('**/edit');
  await expect(page.getByText('自動では匿名化に戻りません')).toBeVisible();

  // 片付け1: 匿名化に戻して保存（実名寄り状態を残さない）
  await page.getByLabel('匿名化する（推奨・既定）').check();
  await page.getByRole('button', { name: '保存する' }).click();
  await page.waitForURL('**/brain/case-studies');
  await expect(page.locator('tr', { hasText: restrictedTitle }).getByText('匿名化済み')).toBeVisible();
  ...
  // 片付け2: テスト事例をアーカイブ
  await page.locator('tr', { hasText: restrictedTitle }).getByRole('button', { name: 'アーカイブ' }).click();
  await page.waitForURL('**/brain/case-studies');
  await expect(page.getByText(restrictedTitle)).toHaveCount(0);
});

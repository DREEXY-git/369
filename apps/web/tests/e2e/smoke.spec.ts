...
test('顧客事例（Case Study）の一覧が閲覧専用で表示される', async ({ page }) => {
  // Phase 2-C-3: read-only 一覧＋架空 seed のみ。作成/編集/アーカイブのUIが無いことも確認する。
  await login(page);
  ...
  await expect(page.getByText('この画面は社内参照専用の閲覧のみ', { exact: false })).toBeVisible();

...
// を実測する。**Phase B 条件4 切替後**: writer は canonical（新規行の idempotencyKey が canonical 形式・
// payload.idem.enc='canonical' も assert）。reader は canonical-first＋検証付き legacy fallback で不変のため
// (a)(b) の dual-read 収束（legacy 行/canonical 行いずれへも増分 0）は writer 切替をまたいで維持される。
...
test('(c) 肯定対照: 無関係イベントは dual-read に阻害されず正常に新規作成され、保存 key は canonical 形式（Phase B writer）', async () => {
  ...
  try {
    ...
    // Phase B 条件4 の証拠: 新規行の保存 key は canonical 形式（legacy FNV ではない）。
    const row = await prisma.domainEvent.findUnique({
      where: { id: r.eventId },
      select: { idempotencyKey: true, payload: true },
    });
    expect(row!.idempotencyKey, '保存 key は canonical 形式（Phase B writer 切替）').toBe(makeCanonicalIdempotencyKey(fresh));
    expect(row!.idempotencyKey).not.toBe(makeIdempotencyKey(fresh)); // legacy FNV ではない
    expect(row!.idempotencyKey).toMatch(/^DEAL_CREATED:[^:]*:[^:]*:[^:]*$/); // 4 セグメント canonical
    // writer は payload.idem に identity を無損失保存し enc='canonical' を刻む（ID-2・切替後 encoding の物的証拠）。
    expect(readIdemMetadata(row!.payload), '新規行は payload.idem を持つ（dedupe なし emit は空文字）').toEqual({ dedupe: '' });
    expect((row!.payload as { idem?: { enc?: string } }).idem?.enc, 'enc は canonical').toBe('canonical');
  } finally {
    await cleanupByEventIds(created);
  }
});

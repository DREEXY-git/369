...
L2 は approval packet（`369-l2-approval-packet-v1`: fixed head / evidence / CI run /
Vercel preview / rollback 手順 / 要約）を Issue に残して**停止**する。merge・schema・
secrets・外部送信・課金は GitHub UI 上で人間だけが実行する。

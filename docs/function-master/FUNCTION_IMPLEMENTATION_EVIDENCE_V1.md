...
| `C28-017` | `CODEX_VERIFIED` | AI社員8名の3D/2D read-onlyダッシュボード、canonical profile、2D/canvas選択とURL・back/forward同期、mobile NAV 67導線とtheme永続化 | PR #18 `fa04e74`、CI 472/146、artifact `8260681537`、21 PNG独立目視 | exact-head Human Preview、main、Production未確認 |
...
| `C30-041` | `CODEX_VERIFIED` | `AIApprovalGate(PENDING)`の人間approve/reject、approve後`QUEUED`、24時間超stale再確認、AI取得前拒否 | PR #20 `9080df1d4cafcee225775003700b219ac0522d64`、CI 493/159、Codex unit 116/116、comment `4951708593` | 実worker requeue、Phase 4固有Preview、main、Production未確認 |
...
| `USR-003` | `CODEX_VERIFIED` | PR #18 `fa04e74`の3D AI Office、8名canonical parity、2D/canvas URL同期、profile全体画像、NAV 67導線、mobile theme | CI `29194789992`、artifact `8260681537`、21 PNG独立目視 | exact-head Human Preview、main、Production未確認 |
...
| PR #18のC21 `content_review`承認ブリッジ | `UNMAPPED_CANDIDATE` | `fa04e74`、内部ApprovalRequest、実PostgreSQL証拠、CI `29194789992`、Codex pass `4951699871` | 限定Release Pathは合格。C21台帳に承認workflow固有IDがなく、C21カテゴリ完成・公開実行へ格上げしない。Human Preview/main/Production未確認 |
...
| Phase 4 `ai_run_resume` bridge / BullMQ実queue候補 | `UNMAPPED_CANDIDATE` | PR #20 `9080df1`、CI `29196387933`、Codex unit 116/116、ローカルBullMQ 9/9 | approve後QUEUEDとstale/AI拒否/rollbackは確認。実queue requeue、CI Redis、production worker、固有PreviewはEvidence Gap。正式IDを創作しない |
| AI Inbox / Execution Receipt / Workflow Dry Run | `UNMAPPED_CANDIDATE` | `PHASE_READINESS_MATRIX_V3.md` | Scout時点で固定実装headなし。正式IDへ格上げせず`ROADMAP_ONLY / WAITING_FOR_CLAUDE_HEAD` |

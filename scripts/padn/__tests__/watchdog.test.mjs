...
test('freeze 中の head 移動を FREEZE 検出（Codex R11 P2）', async () => {
  const world = standardWorld();
  // B2 を FROZEN_FOR_REVIEW のままにし（PASS/READY コメント除去）、PR head を動かす
  ...
  const snap = await snapshotOf(world);
  const { findings, action } = runWatchdog(snap, policy, {}, snap.now);
  assert.ok(findings.some((f) => f.id === 'frozen_head_moved' && f.severity === 'freeze'));
  assert.equal(action, 'INCIDENT_FREEZE');
});

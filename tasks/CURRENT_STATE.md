...
1. **Phase 2-C-2-PROD 記録（doc73）の push（push-only・別承認。feature＋main）**。
2. **Phase 2-C-3（CaseStudy read-only 画面＋架空 seed デモデータ・doc74 候補）の承認判断**（doc73 §5 に推奨プロンプト案）。
3. または **Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・2-C-3 と並行可）**。

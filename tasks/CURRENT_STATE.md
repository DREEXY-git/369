...
- Phase 2-C-4 基準 commit（本番確認 GO 済み基準）: `11e8f51`（※現在 HEAD ではなく基準 commit。現在位置は git を参照。昇格記録: doc77＋doc14 §54・2026-07-05）
...
1. **Phase 2-C-5 実装 commit（doc79・CaseStudy AI参照）の push（push-only・別承認。feature＋main）**。
2. **push 後の本番確認（利用者実測・§0。架空事例1件作成→検索→参照元表示→ai_reference（CaseStudy）確認→アーカイブ片付け）→ GO なら 2-C-5-PROD（doc80 候補）**。
3. その後 **ConsentRecord 連携設計 / Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・並行可）**。

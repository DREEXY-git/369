...
test('会議中核の原子性: 取込みで 会議＋文字起こし＋議事録＋決定＋タスク＋監査 が揃って作られる', async ({ page }) => {
  ...
  try {
    ...
    // P3-MEETING: 単一transaction化で追加された行も削除（DomainEvent/Outbox/AIOutput/UsageEvent）。
    const events = await prisma.domainEvent.findMany({ where: { aggregateType: 'Meeting', aggregateId: meetingId }, select: { id: true } });
    ...
    const outs = await prisma.aIOutput.findMany({ where: { entityType: 'Meeting', entityId: meetingId }, select: { id: true } });

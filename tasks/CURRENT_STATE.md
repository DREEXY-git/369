...
- 最新の本番確認 GO 済みプロダクト基準: **CaseStudyConsent 保存条件接続**
- 内容: **匿名化を外す保存（anonymized=false）に許諾台帳の有効な行（internal_view・期限内・取り消しなし・証跡・人間登録・非抑止）との突合を必須化（CONNECT_ONLY）した保存条件接続の本番確認 GO 記録。新規作成では匿名化を外せない（作成→台帳登録→編集で外す運用）。anonymized=false は未解禁（保存できるだけ・AI/公開に使われない）・AI参照条件変更なし・CaseStudyConsent は AI 文脈へ注入しない**
...
1. **CaseStudyConsent anonymized=false 本格扱い 最小実装（doc95）の push（push-only・別承認。feature＋main）**。
2. **anonymized=false 本格扱いの本番確認（利用者実測・§0・別承認。台帳登録→匿名化オフ保存→バッジ表示→匿名化に戻す→片付けの1周）**。
3. **AI参照条件の扱い判断（ai_reference purpose の使用・別承認）／公開活用判断（ApprovalRequest・表現審査・別承認）**。
4. **Customer Pain の扱い判断（高機密ラベル対応が先・別承認）**。

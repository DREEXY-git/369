...
test('v6.8 P2: 3D canvas の実クリック選択が URL ?agent= を更新し、back/forward で同期する', async ({ page }) => {
  test.setTimeout(120_000);
  await login(page, 'ceo@ikezaki.local');
  // 決定論のため明示 deep link（?agent=A）から開始する（query 無し開始だと back 後の選択は
  // 「query 無しは手動選択を上書きしない」規約により直前の手動選択が残る＝履歴比較にならないため）。
  await page.goto('/ai-agents');
  const cards = page.locator('[data-testid^="ai-agent-card-"]');
  await expect(cards.first()).toBeVisible();
  const idA = (await cards.first().getAttribute('data-testid'))!.replace('ai-agent-card-', '');
  ...
  // A と異なる agent のネームプレートを実クリック → raycast onClick → selectAgent → router.push(?agent=B)。
  const probe = await page.evaluate((excludeId) => {
    const c = document.querySelector('canvas[data-testid="ai-office-canvas"]') as HTMLCanvasElement & {
      __nameplateProbe?: (i: number) => { ndcX: number; ndcY: number; agentId: string } | null;
      __pickAt?: (x: number, y: number) => string | null;
    };
    for (const i of [0, 1, 2, 3, 4, 5, 6, 7]) {
      const p = c.__nameplateProbe!(i);
      if (p && p.agentId !== excludeId && c.__pickAt!(p.ndcX, p.ndcY) === p.agentId) return p;
    }
    return null;
  }, idA);
  expect(probe, 'clickable nameplate with deterministic pick').not.toBeNull();
  const box = (await canvas.boundingBox())!;
  await page.mouse.click(box.x + ((probe!.ndcX + 1) / 2) * box.width, box.y + ((1 - probe!.ndcY) / 2) * box.height);

  // canvas click で detail ID と URL の両方が更新される（2D 一覧と同じ selectAgent 経路）。
  await expect(office).toHaveAttribute('data-agent-id', probe!.agentId);
  await expect(page).toHaveURL(new RegExp(`agent=${probe!.agentId}`));

  // back → ?agent=A へ戻り選択も A（useSearchParams 反応 effect）。forward → B。
  await page.goBack();
  await expect(page).toHaveURL(new RegExp(`agent=${idA}`));
  await expect(office).toHaveAttribute('data-agent-id', idA);

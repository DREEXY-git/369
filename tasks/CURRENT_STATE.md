...
- Phase 2-C-2 基準 commit（本番確認 GO 済み基準）: `b012bd0`（※現在 HEAD ではなく基準 commit。現在位置は git を参照。昇格記録: doc73＋doc14 §52・2026-07-05）
...
1. **Phase 2-C-3 実装 commit（doc74・read-only 画面＋架空 seed）の push（push-only・別承認。feature＋main）**。
2. **push 後の本番確認（利用者実測・§0 テンプレート。ナビ「顧客事例」・架空4件表示・作成/編集ボタンなし・既存画面無回帰）**。
3. その後 **Phase 2-C-4（CaseStudy 人間書き込み＋writeAudit＋許諾/匿名化制御・別承認）の承認判断**。または **Stage 2 / Stage 3 / ★2 / UX改善 の人間選択（いずれも別承認・並行可）**。

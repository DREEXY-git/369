...
test('approve 経路: 下書き→承認申請→/approvals承認→approved表示・deep link往復・重複PENDINGなし', async ({ page }) => {
  ...
  // decideApprovalAction は revalidate 後に同一 URL（/approvals）へ redirect するため、
  // waitForURL('**/approvals') は「既に /approvals にいる」だけで即解決し、決定 commit を待たない
  // （RC CI で reject 経路が flake した根因）。決定の確定シグナル＝当該 PENDING 項目が一覧から
  // 消える（toHaveCount(0)）を待つことで、後続の goto が決定 commit を追い越さないようにする。
  await expect(page.getByTestId(`approval-content-deeplink-${id}`)).toHaveCount(0);
...
test('reject 経路: 却下後は再申請可能（rejected は requestable）', async ({ page }) => {
  ...
  // approve 経路と同じ同一 URL 決定 race の根本対処: 決定が確定して PENDING 一覧から当該項目が
  // 消えるのを待つ（waitForURL('**/approvals') は即解決し decideApprovalAction の commit を待たない）。
  await expect(page.getByTestId(`approval-content-deeplink-${id}`)).toHaveCount(0);
...
test.describe('AI ロールは承認権限が付与されていても決定できない（action 境界の不変条件）', () => {
  ...
  test('AI＋OWNER role fixture は /approvals をデータ取得前に遮断・ApprovalRequest/asset は不変（v7.0 R2）', async ({ page }) => {
    // v7.0 R2（Codex P2-1 comment 4951050657）: v6.9 時点は「閲覧はできるが決定 submit は denied」だったが、
    // R2 で閲覧自体をデータ取得前に遮断する三重防御（page/action/core）へ強化した。承認項目・判断フォームは
    // AI には一切描画されない（action 境界の user.isAi 拒否と core の forbidden 契約は unit/契約テストが担保）。
    await login(page, AI_EMAIL);
    await page.goto('/approvals');
    await expect(page.getByText('承認一覧は人間の承認者のみ閲覧できます', { exact: false })).toBeVisible();
    await expect(page.getByTestId(`approval-content-deeplink-${assetId}`)).toHaveCount(0);
    expect(await page.getByRole('button', { name: '承認' }).count()).toBe(0);

    // DB 実測: 決定も状態遷移も起きていない。

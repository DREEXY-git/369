...
- v7.0 FINAL（2026-07-12）: PR #18 = R1 mobile topbar（<sm優先順位表示＋4幅probe）＋R2 実PostgreSQL transaction証拠12件で head `c8b6065`・CI `29190155634`・E2E 145/0・freeze・CLAUDE_FIXED_V70。別branch `claude/p4-human-gate-resume-v1` = roadmap82 P4安全実行Bridge（schema変更不要・gate CAS→run遷移→ApprovalRequest 1:1→監査の単一transaction・実PG/実UI 7件green・フルE2E 152/0ローカル）＋P4Q BullMQ実queue証拠7/7（loopback Redis・secret 0・CI未収載GAPは維持）＋roadmap83 C19 schema案A推奨/C22判定。RCはCODEX_RELEASE_PASS_V70待ち。詳細は tasks/CURRENT_STATE.md が正。
- Phase 3.5 承認ブリッジ（C21完了・C19停止）P35_READY（2026-07-12）: roadmap81 §2/§3 実装サイクル（branch `claude/p35-approval-bridges-v1`・PR #18・PR #14 非変更）。**C21**: `content_review`＋`content-approval.ts` 状態機械（unit 7）＋`requestContentApprovalAction`（`ContentAsset.status` 条件付き updateMany の**原子的 CAS で重複 PENDING 防止**・勝者のみ ApprovalRequest 作成・メタのみ）＋`decideApprovalAction` PENDING 限定 CAS 化（冪等・content_review 分岐は社内状態遷移のみ＝外部作用ゼロ）＋UI/deep link＋E2E 3本。**C19**: `MarketingSuggestion` に status 列なし＋`ApprovalRequest` に unique 制約なし→原子的重複防止が既存 schema で不可能のため **`SCHEMA_CHANGE_APPROVAL_REQUIRED` で停止**（案A/B を roadmap81 §11 に固定・完成主張なし）。unit **452/0**・tsc/lint/build/safety 0・CI `29184051254` success・E2E **130/0**（ログ本文）・artifact 16 files。詳細は tasks/CURRENT_STATE.md が正。
- Grammar P1(object/array文法)封鎖・3D canvas URL同期 v6.8 進行中（2026-07-12）: Codex 新規 Grammar P1（`{"password":"abc","O0F144S68"}` の key 無し value 受理）＋P2（3D raycast click が URL 未更新）。**WIP-1**: tail 検証を container stack 付き single-pass parser（object=KEY/COLON/VALUE、array=VALUE を区別・object key は quoted string 必須）へ置換。keyless quoted/bare/nested/array・key without colon 等を fail-closed。旧 84/1152/1764/V67 全回帰＋positive oracle 保持＋direct/worker 4経路 sentinel 0＋線形。**WIP-2**: raycast click を `selectAgentRef` 経由で 2D と同じ selectAgent（router.push）へ統一・scene 再構築なし。実 canvas click→URL→back/forward E2E。unit **444/0**・tsc/lint/build/safety 0。Codex `CODEX_PASS_V68` 判定待ち・branch freeze。詳細は tasks/CURRENT_STATE.md が正。
- 残存Whitespace P1封鎖・tenant逆参照・完全Parity(直接照合)・真client nav v6.7 進行中（2026-07-12）: Codex 新規 space/LF P1（`{"password":"abc" O0F144S66"x"}`）。**WIP-1**: v6.6 の tail 判定が値後の空白/LF を無条件 prose 化し漏洩（現 head 420/588 実測再現）→`isInsideJsonContainer` で container 文脈を判定し、container 内は空白後 bare も構造検証（正規の次 field/array/終端だけ保持）、container 外の純プローズは quote 無し純テキストのみ保持。1764 matrix＋旧 1152/6.6＋worker 3経路 sentinel 0＋benign 回帰＋線形。**WIP-2**: 一覧 run を own-tenant agent 集合で制限＋表示名を read model 由来へ。foreign 参照 fixture で run/agent 名 0＋404。**WIP-3**: 8名を getAiCharacter 期待値へ直接照合＋portrait 決定論 fingerprint。AiOffice に選択→router.push を足し画面上選択で query 変更→back/forward。プロフィール element screenshot artifact。unit **439/0**・tsc/lint/build/safety 0。Codex `CODEX_PASS_V67` 判定待ち・branch freeze。詳細は tasks/CURRENT_STATE.md が正。
- 新規P1(balanced-but-invalid tail)封鎖・AI社員完全Parity v6.6 進行中（2026-07-12）: Codex 新規 P1（comment 3564933388）。**WIP-1**: v6.5 の tail 検証が quote 均衡だけを見るため `{"password":"abc",O0F144S64"x"}` の偽 closer を信頼し漏洩（現 head 30/42 broken-matrix＋exact P1 実測再現）→`scanValueEnd` の tail 検証を**文法的 bounded state machine**（EXPECT_SEP/EXPECT_VALUE＋depth＋JSON プリミティブ検証・container 出切りは prose 委譲）へ強化。候補 closer は後続が正規次フィールド/配列/container終端/record終端として解析可能な場合のみ採用、それ以外は末尾まで fail-closed。independent 破壊 matrix を direct/保存/再throw/Action要約 4経路で sentinel 0＋正規複数field/primitive/nested/prose 保持＋線形。**WIP-2**: 8名の全プロフィール項目（name/epithet/personality/skills/traits/mistakes/eval）を詳細⇔3D の画面上実値で field 単位比較＋fingerprint 補助。**WIP-3**: v6.5 非退行を既存 suite で再確認。unit **433/0**・tsc/lint/build/safety 0。Codex `CODEX_PASS_V66` 判定待ち・branch freeze。詳細は tasks/CURRENT_STATE.md が正。
- 汎用マスク・tenant境界・人物同期・モバイルNAV受入 v6.5 進行中（2026-07-12）: Codex v6.4 独立再監査（High·P1 2）。**WIP-1**: 「同 depth 引用符1個=一意 closer」を破る生成 matrix（偽 closer 同 depth＋別 depth terminal quote）で 84/84 sentinel 残存を、現 head で 576/1152 実測再現→`scanValueEnd` を **clean close＋tail 構造検証**（dangling/未均衡は末尾まで fail-closed・健全時のみ最小マスク）へ全面変更。matrix 1152 件（direct/worker 保存/再throw/Action要約）＋decoy 内元秘密 0 残存を直接 assert・benign 保持・全回帰。**WIP-2**: `/ai-agents` 子（run/action/memory）に tenantId 明示＋最小 select＋writeDataAccess（メタのみ）・越境子 fixture 否定 E2E。**WIP-3**: 8名 portrait/profile 値一致＋AiOffice `useSearchParams` で ?agent= client-nav/back-forward 同期。**WIP-4**: drawer role=dialog/aria-modal/focus 移動・復帰・aria-expanded＋67導線 DOM/overlay close/背景復元 E2E。unit **428/0**・tsc/lint/build/safety 0。Codex `CODEX_PASS_V65` 判定待ち・branch freeze。詳細は tasks/CURRENT_STATE.md が正。
- 残存P1・モバイルNAV・AI社員表示・Evidence語彙 v6.4 進行中（2026-07-12）: Codex `CODEX_REVIEW_RESULT_V63`＝CHANGES_REQUIRED。残存P1（内部 escaped quote depth3 の直後 delimiter で秘密残存）を実測再現→scanner を parity から**実 backslash depth 一致＋曖昧時 fail-closed**へ修正（depth-matrix 否定テスト・保存/再throw/Action要約）。モバイルNAV drawer を portal で viewport 全体へ（backdrop-blur containing block 回避・E2E で高さ/深い導線/Escape）。AI社員カードの長い permission を短い自律ラベル＋折返し行に分離＋overflow 検査＋8名 canonical key/fullName/state の3画面値一致 parity。roadmap80 の定義外 Evidence 語彙を統一。unit 423/0。Codex 再監査待ち。詳細は tasks/CURRENT_STATE.md が正。
- P1 mask 2件クローズ v6.3 進行中（2026-07-12）: v6.2 の bounded scanner に Codex が新規 P1 2件（P1-1 escaped-quoted 値の内部 delimiter で suffix 残存・P1-2 quoted 値の改行で2行目残存）を提起。両方を現 head で実測再現→scanValueEnd を「同 escape レベルの対応する閉じ引用符まで消費（内部 delimiter/改行を読み飛ばす・fail-closed）」へ修正。sentinel 非残存を出力/保存/再throw で直接 assert＋100KB bounded＋v6.2 全ケース回帰。unit 411/0。Critical/High=0 は Codex 判定待ち・branch freeze。詳細は tasks/CURRENT_STATE.md が正。
- Codex Track B 指摘クローズ v6.2 進行中（2026-07-12）: Track B=CHANGES REQUIRED（High 1/P2 2/Evidence Gap・thread 5）。High 秘密マスクを bounded scanner へ全面再設計（値内部 escaped quote suffix 漏れを実測再現→修正・否定テスト）・stale 判定を isStaleActiveRun 単一正本化（RUNNING/QUEUED/null・pre/post 一致・表形式網羅）・build badge を OWNER/ADMIN role 本体へ限定（肯定/否定テスト）・AI社員 8名 parity＋実在別tenant fixture 404・NAV 明示契約67＋RBAC 非表示≠消失・8 named 視覚証拠＋DOM/overflow/canvas・doc 証拠段階厳密化＋MVP先行/Wave1-5 方針正本化（NOT_PLANNED→DEFERRED）。unit 398/0。RECOVERY_READY は撤回し CHANGES_REQUIRED/CODEX_REVIEW_PENDING。Codex 再監査まで GO 提案なし。詳細は tasks/CURRENT_STATE.md・audit176・roadmap80 が正。
- 完全復旧・4軸ロードマップ・競合台帳 v6.1 進行中（2026-07-11）: 根本原因=deployment lineage（利用者画面=main 63 NAV／欠落4機能は integration 7ef2d9f にコード存在・削除0）。recovery `claude/full-recovery-v61`（PR #14）で P1/P2 再監査（mask escaped-quote/改行直後値/裸session・stale rival・NAV gate を red先行で修正）・AI社員⇔3D Office 人物正本統一（getAiCharacter＋deriveAgentState 単一正本・双方向 deep link）・NAV 静的契約67＋build識別。unit 365/0。roadmap80 で 4軸Phase＋workstream＋AI社員開発環境/Salesforce/MoneyForward台帳を正本化。main/Production は人間GO後のみ。詳細は tasks/CURRENT_STATE.md・audit176・roadmap80 が正。
...
- CRM Customer / Contact 閲覧統制の追加監査「docs/roadmap/26 Candidate（docs-only・369-vault非編集）」: `docs/audit/125_customer_contact_confidential_view_control_audit.md`（doc125）新規＋`docs/roadmap/26_customer_contact_confidential_view_control_audit_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（Customer閲覧統制監査入口1行追記）＋本ファイル。doc124 の未確認点を追加 read-only 監査。**重要訂正**: doc124 は canAccessLabel 直 grep で見落としたが、**Customer 詳細（customers/[id]/page.tsx:30）は `assertCanViewConfidential(user,{label: customer.label})`（ABAC・security/policy.ts→packages/shared policy.ts:156 canAccessLabel）を実行し DataAccessLog(confidential_view) を記録**（meetings/finance/invoices も同型）＝**Customer 詳細閲覧の高機密ラベル runtime 統制は閉じている**。tenantId スコープ・変更系は hasPermission('customer')＋writeAudit。**残るズレ**: Customer 一覧(customers/page.tsx)は行ごとの assert なし（LabelBadge のみ）・Contact 単体経路の統制未確認・LocalBusinessLead は leadmap 権限系（別体系）・doc110 標準閲覧式(customer-pain-access.ts の canViewCustomerPainDetail/isHumanUser)は apps 未接続。**明確な無統制経路は発見せず（C ではない）**。安全側 **判定 B: HOLD**。**Phase 3 進入は引き続き HOLD・人間 Phase Gate 承認事項**。roadmap26 に現在地10項目明示見出し（§12-1〜12-10）・50カテゴリ Matrix 全行（§13・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§18）。**runtime 解禁なし・canAccessLabel/writeDataAccess 実装なし・schema変更なし・migrationなし・RBAC変更なし・company-brain-reference/leadmap/customers 変更なし・externalAiAllowed true 解禁なし・外部送信/実LLM/AIコスト/本番なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。次は **doc125 push-only**（別承認）または Customer 一覧・Contact 単体閲覧統制の追加監査。詳細 `docs/audit/125_customer_contact_confidential_view_control_audit.md`。反映状態は git refs を正とする。
- 高機密ラベル runtime 統制の現状監査「docs/roadmap/25 Candidate（docs-only・369-vault非編集）」: `docs/audit/124_high_confidential_runtime_control_audit.md`（doc124）新規＋`docs/roadmap/25_high_confidential_runtime_control_audit_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（runtime統制監査入口1行追記）＋本ファイル。Phase 3 移行残件②。doc108〜114 設計と実コードのズレを read-only 監査。**AI参照経路（Company Brain）の高機密ラベル封印は閉じている**（`AI_READABLE_LABELS=['NORMAL','INTERNAL']` DBフィルタ＋`canAccessLabel` 二重ガード・外部LLM時 externalAiAllowed＋maskText・CaseStudy anonymized+private のみ）＝CUSTOMER_CONFIDENTIAL/顧客PIIは AI 非注入。`writeDataAccess`(db.ts:62)/`DataAccessLog`(schema:331)/ABAC(security/policy.ts) 実装済み・`canAccessLabel`(labels.ts:45) 純粋関数存在。**未確認/ズレ**: CRM `Customer`(既定CUSTOMER_CONFIDENTIAL schema:506)/`Contact`/`LocalBusinessLead` の人間 UI 閲覧経路で canAccessLabel＋writeDataAccess(confidential_view) 配線が未確認・doc110 標準閲覧式(customer-pain-access.ts)は apps 未接続・runtime 解禁は doc112 停止条件のまま。安全側 **判定 B: HOLD**。**Phase 3 進入は引き続き HOLD・人間 Phase Gate 承認事項**。roadmap25 に現在地10項目明示見出し（§12-1〜12-10）・50カテゴリ Matrix 全行（§13・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§18）。**runtime 解禁なし・canAccessLabel/writeDataAccess 実装なし・schema変更なし・migrationなし・RBAC変更なし・company-brain-reference変更なし・externalAiAllowed true 解禁なし・外部送信/実LLM/AIコスト/本番なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。次は **doc124 push-only**（別承認）または CRM Customer/Contact 閲覧統制の追加監査。詳細 `docs/audit/124_high_confidential_runtime_control_audit.md`。反映状態は git refs を正とする。
- Phase 2 正式完了記録「docs/roadmap/24 Candidate（docs-only・369-vault非編集）」: `docs/audit/123_phase2_formal_completion_record.md`（doc123）新規＋`docs/roadmap/24_phase2_formal_completion_record_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（正式完了記録入口1行追記）＋本ファイル。doc48・doc121・doc122 を受け Phase 2 正式完了記録を整理。**Phase 2＝CONDITIONAL COMPLETE**（Company Brain foundation 正式完了 `85f1bf3`/doc48・CRM/SFA 既存実装 doc121案A・ただし CRM/SFA の本番確認GO/回帰ゲート green の正式記録が未整備・高機密ラベル runtime 統制は設計のみ・不足補強 archivedAt/internalNote 未着手 → **正式完了宣言は人間 Phase Gate 承認事項**）。**Phase 3 進入は HOLD**（残件6条件=①Phase 2完了正式記録②高機密ラベル runtime 統制監査③外部送信 Human Certification Gate 運用確認④Consent・SuppressionList 運用確認⑤AI境界再確認⑥回帰ゲート green 確認・GO＝①〜⑥＋人間承認）。roadmap24 に現在地10項目明示見出し（§12-1〜12-10）・50カテゴリ Complete Function Coverage Matrix 全行（§13・直接C08/C09/C46＋Phase 3予告C18・間接21・後続24・禁止C45）・Global AI Rules（§18・AIができること/無承認禁止/Human Certification Gate/Consent Gate/Security Gate）を記録。5本柱（Human Certification Gate/Business Event Ledger 直接扱い）。**実装なし・schema変更なし・migrationなし・RBAC変更なし・company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc123 push-only**（別承認）または 高機密ラベル runtime 統制の現状監査 docs-only。詳細 `docs/audit/123_phase2_formal_completion_record.md`。反映状態は git refs を正とする。
- Phase 2 完了判定・Phase 3 移行条件整理「docs/roadmap/23 Candidate（docs-only・369-vault非編集）」: `docs/audit/122_phase2_completion_and_phase3_gate.md`（doc122）新規＋`docs/roadmap/23_phase2_completion_and_phase3_gate_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（Phase Gate入口1行追記）＋本ファイル。doc117〜121・既存 LeadMap/CRM 実測を前提に Phase 2 完了扱いと Phase 3 進入可否を整理。**Phase 2＝部分完了**（Company Brain foundation は Phase 2-A 正式完了 `85f1bf3`/doc48・CRM/SFA は既存実装あり doc121案A・ただし CRM/SFA の Phase 2 完了正式記録は未作成・高機密ラベル runtime 統制は設計のみ・CRM 不足補強 archivedAt/internalNote 未着手）。**Phase 3 AI Growth Engine 進入は HOLD 推奨**（外部発信リスク＝広告/PR/SEO/MA/メール・Human Certification Gate/Consent/SuppressionList/高機密ラベル統制の運用確認が前提）。移行条件6項目（①Phase 2完了記録②高機密ラベル runtime 統制監査③外部送信 Human Certification Gate 運用確認④Consent・SuppressionList 運用確認⑤AI境界再確認⑥回帰ゲート green 確認）。GO条件＝①〜⑥充足＋人間 Phase Gate 承認。**判定は人間承認事項**。50カテゴリ（直接C08/C09/C46＋Phase 3予告C18）・Phase対応（事業2→3/PDF 2.5→3/Ledger R4+R0→R5相当候補）・5本柱（Human Certification Gate/Business Event Ledger）。**実装なし・schema変更なし・migrationなし・RBAC変更なし・company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc122 push-only**（別承認）または Phase 2 正式完了記録の作成 docs-only。詳細 `docs/audit/122_phase2_completion_and_phase3_gate.md`。反映状態は git refs を正とする。
- 既存LeadMap/CRM実装の追認・差分監査「docs/roadmap/22 Candidate（docs-only・369-vault非編集）」: `docs/audit/121_crm_leadmap_existing_implementation_reconciliation.md`（doc121）新規＋`docs/roadmap/22_crm_leadmap_existing_implementation_reconciliation_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（差分監査入口1行追記）＋本ファイル。doc121 実装ミッションで既存に LeadMap/CRM 実装済み（`LocalBusinessLead`＝実リード＝tenantId/PII/placeId/`LeadStage`16値/customerId/dealId 連携・`LocalBusinessContact`/`Customer`（label既定CUSTOMER_CONFIDENTIAL）/`Contact`/`Deal`（source manual|leadmap・leadId）/`Account`・`/leadmap/*` UI・`leadmap/actions.ts` の requireUser/hasPermission('leadmap')/tenantId/writeAudit/OutreachApproval(PENDING)・rbac の `leadmap`/`leadmap:external_send`）と判明し**実装せず停止**、read-only 監査で doc118〜120（greenfield 前提 Candidate）とのズレを整理。**判定=案A: 既存 `LocalBusinessLead`/`/leadmap` を CRM Lead 正本候補として追認・新規 `Lead`/`/crm/leads` は作らない・doc118〜120 は削除せず温存**。ズレ表（Lead実体/Contact PII保持/LeadStage16値 vs doc5値/専用leadmap権限が既存で crm:*・knowledge論点解消/UI既存/CRM配線済み/Customer既定CUSTOMER_CONFIDENTIAL）。不足補強候補（archivedAt/internalNote 追加は別承認）。**実装なし・schema変更なし・migrationなし・UI変更なし・Server Action変更なし・RBAC変更なし・crm:*新設なし・Contact PII追加なし・company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc121 push-only**（別承認）または 既存 LocalBusinessLead 薄い補強の実装可否判断。詳細 `docs/audit/121_crm_leadmap_existing_implementation_reconciliation.md`。反映状態は git refs を正とする。
- CRM Lead-only 実装スプリント可否 最終判断「docs/roadmap/21 Candidate（docs-only・369-vault非編集）」: `docs/audit/120_crm_lead_only_implementation_gate.md`（doc120）新規＋`docs/roadmap/21_crm_lead_only_implementation_gate_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（実装Gate入口1行追記）＋本ファイル。doc118/doc119 を前提に CRM Lead-only を実装スプリントへ進める条件が揃っているかを最終判断。**判定 READY / GO＝条件は揃っている・ただし実装着手は §30 別承認文言に基づく別の重い人間承認**。判断項目・schema要否（追加1）・migration要否（1本）・Lead候補列/status最終確認・tenantId/RBAC/AIロール mutation禁止/writeAudit/writeDataAccess/Data Classification（NORMAL/INTERNALのみ）・Contact PII非採用・company-brain-reference非変更・AI下書きのみ・外部送信なし・各Gate・Business Event Ledger将来接続・実装案（案X/Y/Z）・**推奨=案X（Lead 1テーブル＋最小CRUD＋一覧・既存knowledge権限暫定・crm:*新設は別承認・schema1＋migration1・本番確認は別承認）**・停止条件・別承認文言（案）を記録。50カテゴリ（直接C08/C09/C46）・20大/追加19/5本柱（Human Certification Gate・Business Event Ledger を直接扱う）・Phase対応（事業Phase 2/PDF 2.5/Ledger R4+R0）。**今回は実装しない・schema変更なし・migrationなし・RBAC変更なし・crm:*新設なし・company-brain-reference変更なし・Contact PII保存なし・外部連携/OAuth/外部送信/実LLM/AIコスト/本番なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc120 push-only**（別承認）または実装スプリント着手（§30 別承認）。詳細 `docs/audit/120_crm_lead_only_implementation_gate.md`。反映状態は git refs を正とする。
- CRM Lead-only 最小設計「docs/roadmap/20 Candidate（docs-only・369-vault非編集）」: `docs/audit/119_crm_lead_only_minimum_design.md`（doc119）新規＋`docs/roadmap/20_crm_lead_only_minimum_design_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（Lead-only入口1行追記）＋本ファイル。doc118 推奨案A（Lead-only）を実装判断できる粒度まで最小設計。**Lead 候補列（tenantId/displayName=会社名・拠点名/source/placeId/fetchedAt/expiresAt/status/assignedUserId/internalNote/archivedAt・Contact PII非採用）・status enum 候補（NEW/REVIEWING/QUALIFIED/EXCLUDED/ARCHIVED）・archivedAt ソフトアーカイブ・tenantId 必須・RBAC方針・`crm:*`新設 vs 既存knowledge権限代替の比較（本書ではRBAC変更しない）・writeAudit対象・writeDataAccessはLead-onlyでは原則不要（PII段階で必須＝停止条件）・Data Classification=NORMAL/INTERNALのみ・company-brain-reference非注入・AIは下書きのみ・外部送信なし・Human Certification Gate/Consent Gate/Security Gate・Business Event Ledger将来接続・Phase対応・50/20/19/5本柱・初期MVP非対象・停止条件・推奨判断**。**今回は実装しない・実装は別承認・schema変更なし・migrationなし・画面/Server Actionなし・RBAC変更なし・company-brain-reference変更なし・外部連携/OAuth/外部送信/実LLM/AIコスト/本番なし・Contact PII保存なし・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc119 push-only**（別承認）または CRM Lead-only 実装スプリント可否の最終判断。詳細 `docs/audit/119_crm_lead_only_minimum_design.md`。反映状態は git refs を正とする。
- CRM最小縦切り 実装可否判断「docs/roadmap/19 Candidate（docs-only・369-vault非編集）」: `docs/audit/118_crm_minimum_vertical_slice_decision.md`（doc118）新規＋`docs/roadmap/19_crm_minimum_vertical_slice_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（CRM最小縦切り入口1行追記）＋本ファイル。doc117 CRM/SFA Lineage を受け、Phase 2 CRM/SFA の最初の実装範囲を **案A Lead-only / 案B Lead+Deal / 案C Lead+Deal+Pipeline** の3案で比較（目的・入れる/入れないデータ・schema要否・migration要否・PIIリスク・tenantId/RBAC/writeAudit/writeDataAccess・Human Certification Gate/Consent Gate/Security Gate・Company Brain接続・AI参照・外部送信・Phase対応・収益/moat・停止条件）。**推奨=案A（Lead-only）**（Contact PII最小・Security Gate負荷最小・薄い縦切り優先）。50カテゴリ（直接C08/C09/C46）・4軸Phase（事業Phase 2 主対象）・5本柱（Human Certification Gate/Business Event Ledger 将来接続）記録。**今回は実装しない・推奨案Aも実装は別承認・schema/migration/画面/Server Action/外部連携/OAuth/外部送信/実LLM/AIコスト/本番なし・Contact PIIは初期範囲に入れない・company-brain-reference 非注入・GitHubが正本・Obsidian閲覧・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc118 push-only**（別承認）または 案A Lead-only 実装可否の最小設計 docs-only。詳細 `docs/audit/118_crm_minimum_vertical_slice_decision.md`。反映状態は git refs を正とする。
- CRM / SFA / Salesforce Mini Lineage「docs/roadmap/18 Candidate（docs-only・369-vault非編集）」: `docs/audit/117_crm_sfa_salesforce_lineage_candidate.md`（doc117）新規＋`docs/roadmap/18_crm_sfa_salesforce_lineage_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（CRM/SFA Lineage入口1行追記）＋本ファイル。doc116 SaaSカタログの最初の個別深掘り。**内包・代替機能候補（Lead/Account/Contact/Deal/Opportunity/Pipeline/Activity/Task/Note/Customer 360/Sales Playbook/Quote・Contract・Invoice handoff）・外部連携候補（CRM/MA/広告/メール/カレンダー・例示・契約判断なし）・AI営業提案は下書きのみ・外部送信は Human Certification Gate 必須・AI単独の契約/値引き/請求/送信は禁止・tenantId/RBAC/writeAudit/writeDataAccess/Data Classification/PII・Company Brain/AI Growth Engine/CPQ/Contract/Invoice/ERP 接続候補・Service/Marketing/Data Cloud 境界・Phase対応・収益・moat・やらないこと・承認ゲート**。**Candidate（実装/契約/API連携/DB化/schema化/正式採用ではない）・schema/画面/外部連携は各々別承認・顧客PIIは company-brain-reference 非注入・高機密ラベル runtime 解禁は別承認・GitHubが正本・Obsidian閲覧・369-vault非編集**。**実装なし・schema/migration/RBAC/label定義/company-brain-reference変更なし・外部SaaS連携実装なし・OAuthなし・外部送信/実LLM/AIコスト/本番なし・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。**Phase対応を補強済み**（主対象=事業Phase 2 Salesforce Mini/CRM基盤・前提=Phase 0/1・隣接=Phase 3/4/5/12/13/14/19・PDF系 OS本体 2.5-9/10・戦略 Phase 20/22/23/24/26 は将来接続候補で今回実装しない）。次は **doc117＋Phase補強 push-only**（別承認）または CRM 最小縦切り実装可否判断。詳細 `docs/audit/117_crm_sfa_salesforce_lineage_candidate.md`。反映状態は git refs を正とする。
- 369組み込み予定SaaSカタログ「docs/roadmap/17 Candidate（docs-only・369-vault非編集）」: `docs/audit/116_embedded_saas_catalog_candidate.md`（doc116）新規＋`docs/roadmap/17_embedded_saas_catalog_candidate.md` 新規＋CURRENT_STATE 更新＋`docs/10_obsidian/OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md`（SaaSカタログ入口1行追記）＋本ファイル。`docs/roadmap/09-16` に不足していた「組み込み予定SaaS一覧」を補完 Candidate カタログとして整理。**内包・代替 / 外部連携 / Marketplace / PLUG / Employee App / Candidate保留 / Do Not Build の6区分・内包対象カテゴリ（Core OS/IAM〜Docs/Audit）網羅・外部連携対象・収益源・moat（IP moat）・禁止と安全代替（Human Certification Gate）・Phase対応**。**特定企業名は例示に留め契約判断なし・Candidate（実装/契約/API連携/DB化/正式採用ではない）・1カテゴリずつ別承認・231-252はCandidate扱いのまま・GitHubが正本・Obsidian閲覧・369-vault非編集**。**実装なし・schema/migration/RBAC/label定義/company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc116 push-only**（別承認）または 1カテゴリ個別深掘り。詳細 `docs/audit/116_embedded_saas_catalog_candidate.md`。反映状態は git refs を正とする。
- Obsidian 関係設計「docs/10_obsidian Candidate（docs-only・369-vault非編集）」: `docs/audit/115_obsidian_relationship_design.md`（doc115）新規＋`docs/10_obsidian/OBSIDIAN_369_VAULT_RELATIONSHIP_CANDIDATE.md`／`OBSIDIAN_SYNC_RULES_CANDIDATE.md`／`OBSIDIAN_DASHBOARD_INDEX_CANDIDATE.md` 新規＋CURRENT_STATE 更新＋本ファイル。`docs/roadmap/16` の役割分担 Candidate を運用粒度で `docs/10_obsidian/` に具体化。**GitHubが正本・Obsidianは閲覧/思考整理/経営ダッシュボード・369-vaultを正本として扱わない・直接編集しない・Obsidianだけで正式判断しない・secrets/個人情報/本番ログ生データ/実顧客データは同期しない・同期実行と自動同期実装は別承認・Candidate note は GitHub反映後に official**。**369-vault差分0（非編集）・実同期なし・実装なし・schema/migration/RBAC/label定義/company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。次は **doc115 push-only**（別承認）。詳細 `docs/audit/115_obsidian_relationship_design.md`。反映状態は git refs を正とする。
...
- 記録: doc156（監査）＋`docs/roadmap/57`（Candidate）。次は roadmap57/doc156 push-only（別承認）→ P3-CT-5 設計＋実装前 Gate 統合ミッション（別承認・roadmap57 §19）。

## P3-CT-4 push 前 敵対的レビュー＋redaction 修正 — fix・判定 high 1件/low 2件を push 前に修正 / e2e 74→77 / 外部影響ゼロ

- **roadmap56 §18 追補＋page.tsx/e2e 修正**（2026-07-10）。未 push commit `83fd4bc` に 6視点×独立検証の敵対的レビュー（15エージェント・9指摘→8確定）を実施。**high**: メモ表示クエリが閲覧者 canViewFinance 未考慮で、社長生成の finance カード由来メモ（reason＋FakeLLM echo に finance 件数）が担当者に表示される redaction 迂回（6視点が独立検出）。**修正**: メモクエリに entityId notIn（financeGated keys）フィルタ／生成ボタンを isHumanUser＋leadmap:create でゲート／denied・blocked の安全通知表示／e2e「担当者に finance 由来メモ非表示」追加＋社長テストに unpaid_risk 生成追加（74→77）。actions.ts 安全境界・schema/RBAC/seed 不変。再検証 278/0・typecheck×2・lint・safety・secret NONE 全緑。push 前修正のため外部影響ゼロ。
- 記録: roadmap56 §18・audit155 §7。次は P3-CT-4 実装＋fix の push-only（別承認）→ CI で **77/0** 確認。

## P3-CT-4 Control Tower FakeLLM 下書き生成 実装 — Candidate・実装あり(コード3ファイル)・判定 実装完了 / AI は提案のみ・下書きのみ / redaction 不変 / 単体278 passed

- **doc155 / `docs/roadmap/56_p3_ct4_control_tower_fake_llm_draft_implementation_candidate.md`＋コード3ファイル**（2026-07-10）。実装前 Gate PASS（roadmap55）と CI **run 29116334142 = success・74/0**（ログ本文直接確認）を前提に、roadmap55 §7 の実装計画どおり最小実装。**変更コード3ファイル**: ①`growth/control-tower/actions.ts` 新規（`generateControlTowerNextStepAction`: requireUser→isHumanUser で AI ロール拒否→hasPermission(leadmap,create)→card key 9件 allowlist→getControlTowerData 再取得・存在確認→**redacted 拒否（二重防御）**→FakeLLM 入力を安全項目のみに制限（金額実値・生PII・placeId なし）→safeAiInput→getLLMProvider().chat（fake）＋deterministic fallback→saveAIOutputStandard（AIOutput 下書き保存・ai.output.generated 自動計測）→writeAudit(ai_run, AIOutput)→revalidatePath）②`page.tsx`（非 redacted カードに「AI 下書きメモを作る」ボタン・redacted は安全な別文言のみ（既存 redaction 文言と非重複・staff e2e toHaveCount(2) 不変）・「AI 下書きメモ（最新・下書きのみ）」read-only セクション・送信/承認/削除ボタンなし）③e2e 2件追加（**74→76**: 社長生成→メモ表示／担当者は redacted finance 2カードに生成ボタンなし・testid strict-mode 安全・送信テストなし・既存2件不変）。**任意の shared 純ロジック変更は不使用**（最小差分）。**監査**: writeAudit=ai_run・DataAccessLog=P3-CT-3 方針維持・UsageEvent=ai.output.generated のみ（新 eventType/billable_candidate なし）。**検証**: `pnpm test` **278 passed / 0 failed**・web/shared typecheck exit 0・lint exit 0・safety exit 0（ui 157→158）・diff-check OK・secret NONE・禁止領域差分 0・artifact なし・vault 差分 0。**e2e 実走はサンドボックス不可→push 後 CI で 76 passed / 0 failed を確認**。**STOP 非該当**（新 schema/RBAC/seed/package/eventType/状態永続化/外部送信/実LLM 不要のまま完了）。**AI は送信/承認/削除しない・生成物は必ず下書き・重要操作は /approvals 委譲・redaction 不変（担当者に原価・粗利・未回収の実値なし）・Customer/Contact 生 PII 非増加・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・369-vault非編集・push なし（commit-only）**。判定 **実装完了 / STOP 非該当 / e2e 実緑は push 後 CI（76/0 見込み）**。
- 記録: doc155（監査）＋ `docs/roadmap/56`（Candidate）＋コード3ファイル。次は P3-CT-4 実装 push-only（別承認）→ CI で 76/0 確認 → P3-CT-5 設計（別承認）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## P3-CT-4 Control Tower FakeLLM 下書き生成 実装前 Gate — Candidate・docs-only・実装なし・判定 Gate PASS / 既存 schema・RBAC・seed のみで成立 / STOP 非該当

- **doc154 / `docs/roadmap/55_p3_ct4_control_tower_fake_llm_draft_preimplementation_gate_candidate.md`**（2026-07-10）。P3-CT-4 設計 docs push 後 CI **run 29114587876 = success・74 passed / 0 failed**（ログ本文で env fake/log/false・Running 74 tests・74 passed・growth_control_tower 2件 ✓ を直接確認・stage3_e2e 11 run 連続 green）を前提に、roadmap54 §5.2/§6 の実装候補を **read-only 実査で最終判定**（推定で断定しない）。**Gate 判定 A〜I 全項目 YES = PASS**: A schema（AIOutput/OutreachDraft/ApprovalRequest 既存）／B RBAC（hasPermission(leadmap,create) 前例＋isHumanUser・AI に送信/承認/削除を追加しない）／C seed（CI 74/0 が既存 seed での CT 表示を実測証明）／D FakeLLM（getLLMProvider はキー未設定で必ず Fake・providers/index.ts:21-24・実LLM/AIコスト不要）／E redaction/PII（redacted 生成拒否の二重防御・入力はカード key/redact 済み件数/優先度のみ）／F HCG/Consent/Suppression（AI は送信/承認/削除しない・下書きのみ・実送信は既存 approvals 委譲・isSuppressed 維持）／G Audit/UsageEvent（writeAudit=ai_run 前例 leadmap/actions.ts:117・DataAccessLog=P3-CT-3 維持・UsageEvent=ai.output.generated のみ・新 eventType/billable_candidate 不要）／H 実装対象確定（**actions.ts 新規＋page.tsx ボタン＋e2e 1〜2件・@hokko/ai タスク追加は不採用**・db.ts/audit.ts/usage-events.ts/schema/seed/RBAC/ci.yml/package は触らず成立）／I STOP 非該当。**実装計画確定**（変更3ファイル＋任意 shared 純ロジック・触らないファイル明記・e2e 追加方針=生成 smoke＋redacted 生成不可・74→75〜76）。**検証**: `git diff --check` OK・secret NONE・safety exit 0・禁止領域差分なし・artifact なし・369-vault 差分なし（docs/tasks 5ファイルのみ）。**実装なし・schema変更なし・migrationなし・seed変更なし・RBAC変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・状態永続化なし・redaction 不変・Customer/Contact 生 PII 非増加・369-vault非編集・push なし（commit-only）**。判定 **Gate PASS / STOP 非該当 / 実装は別承認**。
- 記録: doc154（監査）＋ `docs/roadmap/55`（Candidate）。次は doc154/roadmap55 push-only（別承認）→ CI で 74/0 確認 → P3-CT-4 実装ミッション（別承認・roadmap55 §7）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## P3-CT-4 Control Tower FakeLLM 下書き生成 設計 — Candidate・docs-only・実装なし・判定 設計完了 / 既存 schema のみで成立 / redaction 不変

- **doc153 / `docs/roadmap/54_p3_ct4_control_tower_fake_llm_draft_design_candidate.md`**（2026-07-08）。P3-CT-3 実装 push 後 CI **run 28952843194 = success**（stage3_e2e ログ本文で `74 passed (1.1m)`・env fake/log/false・growth_control_tower 2件 ✓ を直接確認・10 run 連続 green）を前提に、CT カードから「次の一手ドラフト」を生成する P3-CT-4 を **docs-only 設計**（コード差分ゼロ）。**大原則**: AI は提案のみ・人間が判断/承認/送信・AI は外部送信/承認/削除しない・生成物は必ず下書き・重要操作は ApprovalRequest／`/approvals` 経由。**設計判断**: ①汎用「次の一手ドラフト」= 人間ボタン起点 → FakeLLM/deterministic → **saveAIOutputStandard で AIOutput 保存（採用・本命）**＝UsageEvent `ai.output.generated` 自動計測・PII 自動フラグ・mutation は AIOutput 1 create のみ ②リード営業文 = **既存 OutreachDraft 導線へ deep link 委譲（採用）**＝isSuppressed/OutreachApproval/HCG の既存統制がそのまま効く ③deterministic 単独=不採用（fallback） ④CT から ApprovalRequest 直作成=不採用。**最重要判定 = 既存 schema のみで成立（YES・実査根拠: generateOutreachForLead の完成形パターン・AIOutput/OutreachDraft/ApprovalRequest schema・assertAiToolAllowed 多重防御が全て既存）**＝新規 table/column/enum/migration・新 RBAC・新 seed・新 package・新 UsageEvent eventType・状態永続化すべて不要。**redaction**: redacted カード（finance 系×canViewFinance=false）は生成不可（ボタン非表示＋Server Action 拒否の二重防御）・FakeLLM 入力はカード key／redact 済み件数／優先度のみ（金額実値・生 PII・placeId を渡さない）。**監査**: writeAudit=既存 `ai_run`／DataAccessLog=P3-CT-3 方針維持／UsageEvent=既存 `ai.output.generated` のみ（usage_only・新 eventType は STOP）。**実装候補（別承認）**: `growth/control-tower/actions.ts` 新規＋page.tsx ボタン＋（必要なら）@hokko/ai タスク＋e2e 1〜2件。**検証**: `git diff --check` OK・secret NONE・safety exit 0・禁止領域差分なし・artifact なし・369-vault 差分なし（docs/tasks 5ファイルのみ）。**実装なし・schema変更なし・migrationなし・seed変更なし・RBAC変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・状態永続化なし・redaction 不変・Customer/Contact 生 PII 非増加・369-vault非編集・push なし（commit-only）**。判定 **設計完了 / 既存 schema のみで成立 / STOP 非該当**。
- 記録: doc153（監査）＋ `docs/roadmap/54`（Candidate）。次は doc153/roadmap54 push-only（別承認）→ CI で 74/0 確認 → P3-CT-4 実装前 Gate（別承認・docs-only）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## P3-CT-3 Control Tower 監査ログ配線 実装 — Candidate・実装あり(1ファイルのみ)・判定 実装完了 / 業務 mutation なし / redaction 不変 / 単体278 passed

- **doc152 / `docs/roadmap/53_p3_ct3_control_tower_audit_logging_implementation_candidate.md`＋control-tower.ts**（2026-07-08）。P3-CT-3 docs push 後 CI **run 28949692213 = success・74 passed / 0 failed**（env fake/log/false・growth_control_tower 2件 green をログ本文で直接確認・stage3_e2e 9 run 連続 green）を前提に、roadmap52 §6.2/§8 の設計どおり `apps/web/lib/domains/growth/control-tower.ts` の **1ファイルのみ**を最小変更。**変更点**: `getControlTowerData` 末尾で `actionableCount` を先に1回算出し、metadata を **allowlist 3項目（financeVisible/cardCount/actionableCount）のみ**に固定。**canViewFinance=true** → 既存 `confidential_view`＋`policyDecision:'allow'`＋metadata。**canViewFinance=false（redacted 閲覧）** → 新規に `action:'read'`・entityType=GrowthControlTower・entityId=null・label=INTERNAL・purpose=growth_control_tower_view・policyDecision:'allow'・metadata を **1閲覧=1件**記録（従来無記録→担当者に finance 実値が出ていない証跡を DataAccessLog で完結）。**1閲覧=1件**（カード別9件・レコード別にしない）・error handling は既存 `await` 直呼び・返却値不変。**writeAudit 不採用**（AuditLog は mutation 専用・read-only 画面には配線せず）・**UsageEvent 不採用**（画面閲覧は資源消費でない・既存 8 emit 不変）・**既存 DataAccessAction（read/confidential_view）のみ**（新 action 値なし）。**metadata denylist**（金額/原価/粗利/未回収額/請求額/カード別件数/finance系件数/顧客名/メール/電話/住所/担当者名/placeId/Google由来/secret/env値/token/AIプロンプト/生成文/本文/score/scoreBreakdown/URLクエリ/IP以外の端末詳細）をコードコメントで明記し実際に入れていない。**画面 page.tsx・純ロジック growth-control-tower.ts・e2e spec・db.ts・audit.ts・usage-events.ts は不変**。**STOP 非該当**（新規 schema/table/column/enum/migration なし・新 DataAccessAction 値なし・新 RBAC/seed なし・状態永続化なし・redaction で塞げない機密表示なし・ci.yml/playwright.config.ts/package/lock 非変更）。**redaction 方針不変**（担当者に原価・粗利・未回収・請求金額の実値を渡さない・lib 段階で件数 null・追加 read ログにも finance 値なし）。**Customer/Contact の生 PII 非増加**（metadata denylist で構造的に防止・既存顧客は件数のみ）。**検証**: `pnpm test` **278 passed / 0 failed**（回帰なし）・`pnpm --filter @hokko/web typecheck` exit 0・`pnpm --filter @hokko/shared typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0・`git diff --check` OK・secret NONE・禁止領域差分なし（コード変更 control-tower.ts 1ファイルのみ）・artifact なし・369-vault 差分なし。**e2e 実緑は push 後 CI（stage3_e2e）で 74 passed / 0 failed（growth_control_tower 2件 green）を確認**（DataAccessLog 記録は画面表示に出ないため既存 e2e 緑維持見込み）。**業務 mutation なし・schema変更なし・migrationなし・seed変更なし・RBAC変更なし・labels変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・状態永続化なし・369-vault非編集・push なし（commit-only）**。判定 **実装完了 / コード変更1ファイルのみ / 業務 mutation なし / redaction 不変 / 単体278 passed・型/lint/safety緑 / e2e実緑はpush後CI（74/0見込み）/ STOP非該当**。
- 記録: doc152（監査）＋ `docs/roadmap/53`（Candidate）＋実装1ファイル。次は P3-CT-3 実装 push-only（別承認）→ CI で 74/0 確認 → P3-CT-4 設計（FakeLLM 下書き生成・別承認）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## P3-CT-3 Control Tower 監査ログ配線設計 — Candidate・docs-only・実装なし・判定 設計完了 / Gate PASS（既存基盤のみで成立） / redaction 不変

- **doc151 / `docs/roadmap/52_p3_ct3_control_tower_audit_logging_design_candidate.md`**（2026-07-08）。P3-CT-2 push 後 CI **run 28946738844（head_sha `83bd185`）= success・stage1/stage3_e2e success・Run E2E 74 passed / 0 failed**（stage3_e2e 8 run 連続 green）を前提に、Control Tower の監査ログ配線（writeAudit / writeDataAccess / UsageEvent の境界）を**実装前に docs-only で確定**（コード差分ゼロ）。**現状実査**: `getControlTowerData` は canViewFinance=true のときのみ `writeDataAccess(confidential_view, GrowthControlTower, INTERNAL, growth_control_tower_view)` を1閲覧=1件記録・redacted 閲覧は無記録・writeAudit/UsageEvent 未配線。**方針確定**: ①writeAudit **不採用**（AuditLog は mutation・重要操作専用の運用で閲覧の前例ゼロ・CT v0 は read-only。P3-CT-4 の mutation 段で別途配線）②writeDataAccess **採用・拡張案確定**（confidential_view 据え置き＋policyDecision/metadata 付与、**redacted 閲覧に `action:'read'` を1閲覧=1件追加**＝担当者に finance 実値が出ていない証跡を帳簿で完結・カード別9件にしない）③UsageEvent **不採用**（既存 emit 8種は資源消費のみ・画面閲覧の前例なし・CT 起点 AI 生成は既存 `ai.output.generated` で自動計測）④metadata 境界固定（allowlist=financeVisible/cardCount/actionableCount のみ・denylist=金額/カード別件数/生PII/placeId/secret/本文/scoreBreakdown）。**Gate 判定 = PASS（STOP 非該当）**: 3テーブル・3関数・action 値 read/confidential_view すべて既存＝新規 schema/enum/migration/RBAC/seed 不要。実装候補（別承認）は `apps/web/lib/domains/growth/control-tower.ts` **1ファイルのみ**（page/shared/e2e/db.ts/audit.ts/usage-events.ts/schema/seed/RBAC は触らない）。テスト方針＝単体追加なし（DB依存・278 passed 体制不変）・表示不変で e2e 74/0 維持を push 後 CI で確認。**検証**: `git diff --check` OK・secret NONE・safety script exit 0・禁止領域差分なし・artifact なし・369-vault 差分なし（docs/tasks 5ファイルのみ）。**実装なし・schema変更なし・migrationなし・seed変更なし・RBAC変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・redaction 不変（担当者に原価・粗利・未回収の実値なし）・Customer/Contact 生 PII 非増加・状態永続化なし・369-vault非編集・push なし（commit-only）**。判定 **設計完了 / Gate PASS / STOP 非該当**。
- 記録: doc151（監査）＋ `docs/roadmap/52`（Candidate）。次は doc151/roadmap52 push-only（別承認）→ CI で 74/0 確認 → P3-CT-3 実装ミッション（別承認・roadmap52 §6.2/§8 のとおり）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## P3-CT-2 Control Tower 優先度ロジック精緻化 — Candidate・実装あり(純ロジック中心)・判定 精緻化完了 / 業務 mutation なし / redaction 不変 / 単体278 passed

- **doc150 / `docs/roadmap/51_p3_ct2_control_tower_priority_logic_candidate.md`＋純ロジック**（2026-07-08）。P3-CT-1 CI green（run 28944487139=success・stage1/stage3_e2e success・Run E2E 74 passed / 0 failed・Upload report on failure=skipped・env fake/log/false・growth_control_tower 2件 green）を前提に、`packages/shared/src/growth-control-tower.ts` の DB非依存 純ロジックを精緻化。**旧 v0**: `score=baseWeight+min(count,20)`（redacted=baseWeight×0.6）で重要度/信頼度/緊急度が未分離。**新 P3-CT-2**: **score=businessImpact×(BASE_SHARE 0.7＋URGENCY_SHARE 0.3×urgency)×confidence**（deterministic・上限 `CONTROL_TOWER_SCORE_MAX=100`・`REDACTED_FACTOR=0.6`）。urgency=`min(count/urgencyCap,1)`（件数飽和・過剰に釣り上げない）。businessImpact/urgencyCap/confidence をカードごとに設定（ceo_attention 92/5/1.0・unpaid_risk[finance] 88/5/1.0・stalled_deals 80/8/0.95・low_margin[finance] 76/6/1.0・uncontacted 70/20/0.9・high_opp 66/15/0.9・next_contact 58/15/0.85・company_brain 46/10/0.7・existing_customer 40/30/0.6）。**empty（count=0・非redacted）は score=0**、**redacted（financeGated かつ canViewFinance=false）は「要確認」中位**＝`round(businessImpact×0.6×confidence)`・count=null・urgency=0・reason=redaction 文言。各カードに `scoreBreakdown`(businessImpact/urgency/confidence/redactedFloor) を付与し**説明可能**化（乱数・時刻・PII・金額なし）。同点は重要度降順で安定ソート。priority 閾値 high>=70/medium>=40/low。件数がある社長ビューは ceo_attention→unpaid_risk→stalled_deals→low_margin が上位に来やすく、count=0 は上位に来ない。**redaction 方針不変**（担当者に原価・粗利・未回収・請求金額の実値を渡さない・pure logic のデータ構造でも count なし）。**Customer/Contact の生 PII 非増加**（既存顧客は件数のみ）。**変更ファイル**: `packages/shared/src/growth-control-tower.ts`（純ロジック・型に `ControlTowerScoreBreakdown`/`scoreBreakdown`・定数 `CONTROL_TOWER_SCORE_MAX` 追加）＋`packages/shared/src/__tests__/growth-control-tower.test.ts`（6→13 テスト：score 上限・urgency 飽和・単調非減少・上位順・redacted 中位・priority ラベル・scoreBreakdown・deterministic）。**未変更（最小差分）**: `apps/web/lib/domains/growth/control-tower.ts`（signals interface 不変）・`apps/web/app/(app)/growth/control-tower/page.tsx`（card 消費のみ）・`apps/web/tests/e2e/growth_control_tower.spec.ts`（見出し・redaction 不変）。**STOP 条件非該当**（新規 schema/table/column/enum/migration なし・状態永続化なし・新規 RBAC action/role なし・新規 seed なし・redaction で塞げない機密表示なし・ci.yml/playwright.config.ts/package/lock 非変更）。**検証**: `pnpm test` **278 passed / 0 failed**（従来271＋新規7・growth-control-tower 6→13・回帰なし）・`pnpm --filter @hokko/web typecheck` exit 0・`pnpm --filter @hokko/shared typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0（ui 157・page 不変）・`git diff --check` OK・secret NONE・禁止領域差分なし・artifact なし。**e2e 実緑は本サンドボックスで実走不能→push 後の CI（stage3_e2e）で 74 passed / 0 failed（担当者 redaction 2件・社長 0件）を確認**（純ロジック変更は見出し・redaction 表示不変で既存 e2e 緑維持見込み）。**app変更なし（page/lib/e2e 不変）・schema変更なし・migrationなし・seed変更なし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・業務データ mutation なし・状態永続化なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **精緻化完了 / 業務 mutation なし / redaction 不変 / 単体278 passed・型/lint/safety緑 / e2e実緑はpush後CI（74/0見込み）/ STOP非該当**。
- 記録: doc150（監査）＋ `docs/roadmap/51`（Candidate）＋純ロジック2ファイル。次は P3-CT-2 push-only（別承認）→ CI で 74/0 確認 → P3-CT-3 設計（状態管理が要れば schema 影響を実装前 Gate で判定・別承認）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## P3-CT-1 E2E selector hardening — Candidate・tests-only・判定 selector 安定化完了 / 実装本体変更なし / redaction 方針不変

- **doc149 / `docs/roadmap/50_p3_ct1_e2e_selector_hardening_candidate.md`＋e2e spec 1本**（2026-07-08）。P3-CT-1 実装 commit `6cc9d2c` を push する前に、`apps/web/tests/e2e/growth_control_tower.spec.ts` の担当者 redaction assertion を **Playwright strict mode 安全化**（tests-only）。**strict-mode risk**: Control Tower v0 の finance-gated カードは「未回収リスク」「低粗利改善候補」の**2枚**（`growth-control-tower.ts` の CARD_SPECS で financeGated:true が2件）。担当者（canViewFinance=false）では同一 redaction 文言 `原価・粗利は財務閲覧権限が必要です（機密情報）。`（`CONTROL_TOWER_REDACTION_NOTICE`）が **2要素一致** → 旧 `await expect(page.getByText('原価・粗利は財務閲覧権限が必要です',{exact:false})).toBeVisible();` は Playwright strict-mode violation（単一要素前提）で CI 赤化リスク。ページ脚注は実装時に当該文言を含まない文へ変更済みのため、担当者=**2件**・社長（canViewFinance=true）=**0件**。**修正（最小・tests-only）**: 担当者テスト＝`const redactionMessages = page.getByText('原価・粗利は財務閲覧権限が必要です',{exact:false}); await expect(redactionMessages).toHaveCount(2); await expect(redactionMessages.first()).toBeVisible();`＋`非表示` Stat に `.first()`／社長テスト＝`await expect(...redaction...).toHaveCount(0);`＋`表示可` Stat に `.first()`。**意図不変・むしろ「finance 2枚とも redacted・社長は 0」を件数で厳密化**＝担当者に原価・粗利・未回収の実値が出ないことを担保強化。**実装本体（page.tsx・lib/domains/growth/control-tower.ts・packages/shared/src/growth-control-tower.ts・nav.ts・index.ts）は一切変更なし**（変更は e2e spec 1本＋docs/tasks）。**検証**: `pnpm test` **271 passed / 0 failed**（回帰なし）・`pnpm --filter @hokko/web typecheck` exit 0・`pnpm --filter @hokko/shared typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0・`git diff --check` OK・secret NONE・禁止領域差分なし・artifact なし。**e2e 実緑は本サンドボックスで実走不能→push 後の CI（stage3_e2e）で growth_control_tower 2件を含む 74 passed / 0 failed を確認**。**schema変更なし・migrationなし・seed変更なし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・redaction 方針不変・PII 非増加・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・状態永続化追加なし・機能拡張なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **selector 安定化完了 / 実装本体変更なし / redaction 方針不変（担保強化）/ e2e実緑はpush後CI（74/0見込み）**。
- 記録: doc149（監査）＋ `docs/roadmap/50`（Candidate）＋ e2e spec 1本（tests-only）。次は P3-CT-1 push-only（別承認）→ CI で 74/0 確認 → P3-CT-2（優先度ロジック精緻化・別承認）。

## P3-CT-1 AI Growth Control Tower v0 read-only 画面 実装 — Candidate・実装あり(read-only)・判定 実装完了 / 業務 mutation なし / 単体271 passed / e2e実緑はpush後CI

- **doc148 / `docs/roadmap/49_p3_ct1_control_tower_readonly_implementation_candidate.md`＋実装**（2026-07-08）。Gate PASS（doc147/48）と回帰ゲート緑（run 28940565283=72/0・6run連続）を受け、`/growth/control-tower` の **read-only 成長機会管制塔** を最小実装（**業務データ mutation なし・read-only**）。**実装ファイル**: 新規 `packages/shared/src/growth-control-tower.ts`（DB非依存 純ロジック・`ControlTowerCard` 型・`buildControlTowerCards`/`countActionableCards`・`CONTROL_TOWER_REDACTION_NOTICE`・優先度スコアリング・deterministic・状態永続化なし）＋`packages/shared/src/__tests__/growth-control-tower.test.ts`（単体6件：優先度降順/finance redaction/空データ/deterministic/actionable）＋`packages/shared/src/index.ts`（barrel export 1行）／新規 `apps/web/lib/domains/growth/control-tower.ts`（tenantId スコープ read-only `prisma.*.count`＋既存 `getGoldenPathExecutiveDashboardData(tenantId,canViewFinance)` 再利用＝ceo_attention=overall.attentionCount・lowMargin=overall.lowMarginCount(finance)・unpaid=projects filter(finance)＋finance 参照時のみ `writeDataAccess(action=confidential_view,entityType=GrowthControlTower,label=INTERNAL,purpose=growth_control_tower_view)` 最小1件・金額/PII/secret を metadata に入れない・v0 しきい値 停滞14日/次回接触30日/高機会priority>=70）／新規 `apps/web/app/(app)/growth/control-tower/page.tsx`（RSC・force-dynamic・requireUser・canViewFinance=hasPermission(user,'finance','read')・Card/Badge/Button/Stat 再利用・**mutation/Server Action/form/外部送信導線なし**・重要操作は既存導線への deep link）／変更 `apps/web/components/shell/nav.ts`（`Radar` import＋「Growth・DX OS」に `Growthコントロールタワー → /growth/control-tower` 1行）／新規 `apps/web/tests/e2e/growth_control_tower.spec.ts`（read-only smoke＋redaction：CEO 閲覧／sales は finance 実値なし＝redaction 文言＋非表示 Stat）。**初期カード9枚**: 社長が見るべき成長機会(Golden Path attentionCount→/planning-hokko)・未回収リスク(finance→/planning-hokko)・停滞商談(Deal 更新14日超かつ未失注→/deals)・低粗利改善候補(finance lowMarginCount→/planning-hokko)・未追客リード(LocalBusinessLead lastContactAt null→/leadmap)・高機会リード(priority>=70→/leadmap)・次回接触推奨(lastContactAt 30日超→/leadmap)・Company Brain 改善候補(ProductCatalogItem active かつ targetPain 欠落→/brain/catalog)・既存顧客追加提案候補(Customer 件数のみ・PII 非表示→/customers)。**redaction**: finance 系は canViewFinance=false のとき **lib 段階で件数を集計せず null**・UI は `原価・粗利は財務閲覧権限が必要です（機密情報）。`（app 既存文言）表示＝**担当者に金額実値を渡さない**。**Customer 一覧行レベル・Contact 単体は据え置き・生 PII 列を増やさない**。**AI 境界**: FakeLLM・実LLM/AIコストなし・AI は外部送信/承認/削除なし・v0 は LLM 未呼び出しの deterministic 文言のみ・重要操作は ApprovalRequest/既存 /approvals へ deep link。**STOP 条件非該当**（新規 schema/table/column/enum/migration なし・状態永続化なし・新規 RBAC action/role なし・新規 seed なし・redaction で塞げない機密表示なし・ci.yml/playwright.config.ts/package/lock 非変更）。**検証**: `pnpm test` **271 passed / 0 failed**（従来265＋新規6・回帰なし）・`pnpm --filter @hokko/web typecheck` exit 0・`pnpm --filter @hokko/shared typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0（ui files 156→157）・`git diff --check` OK・secret NONE・禁止領域差分なし・artifact なし。**e2e 実緑は本サンドボックスで実走不能→push 後の CI（stage3_e2e）で 74 passed / 0 failed（既存72＋新規2）を確認**。**Phase 3 GO 済み・Control Tower v0 P3-CT-1 実装完了・以降 P3-CT-2〜7 は別承認**。**schema変更なし・migrationなし・seed変更なし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・業務データ mutation なし・状態永続化なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **read-only 実装完了 / 業務 mutation なし / 単体271 passed・型/lint/safety緑 / e2e実緑はpush後CI（74/0見込み） / STOP非該当**。
- 記録: doc148（監査）＋ `docs/roadmap/49`（Candidate）＋実装7ファイル。次は P3-CT-1 push-only（別承認）→ CI で 74/0 確認 → P3-CT-2（優先度ロジック精緻化・別承認）。**外部送信・実LLM・課金・本番・状態永続化は個別承認制を維持**。

## AI Growth Control Tower v0 実装前 Gate / schema影響判定 / 最小実装計画 — Candidate・docs-only・判定 Gate PASS / 既存 schema・RBAC・seed のみで成立 / 実装未着手

- **doc147 / `docs/roadmap/48_ai_growth_control_tower_v0_pre_implementation_gate_candidate.md`**（2026-07-08）。roadmap47/doc146 の設計を受け、実装前に v0 が既存 schema・既存 RBAC・既存 seed のみで成立するかを **read-only 実査**で最終判定（**実装コードは書かない**）。**前提 CI**: doc146/roadmap47 push で起動した **run 28939408568（run_number 146・event push・head_sha 39edd19）= completed / success・stage1 success・stage3_e2e success・Run E2E step success・Upload report on failure=skipped**＝stage3_e2e は **run 28930122157(142)/28934614261(143)/28937029131(144)/28938318122(145)/28939408568(146) の 5 run 連続で完全 green（72/0）**。**read-only 実査（証跡）**: 既存 `/growth`（`apps/web/app/(app)/growth/page.tsx`）は `requireUser()`＋`user.tenantId` スコープの prisma 読み取りで成立・mutation のみ `hasPermission(user,'marketing','create')`＝**read-only ページは認証のみで成立**／`packages/shared/src/rbac.ts` は RESOURCES に `deal`/`finance`/`marketing`/`leadmap`/`customer` を持ち `READ_ONLY=[...grant(RESOURCES,['read','ai_read'])]`＝**既存 read/ai_read 権限あり**／財務 redaction は `apps/web/lib/domains/operations/golden-path-dashboard.ts` の `getGoldenPathExecutiveDashboardData(...,canViewFinance)`＋`redactExecutiveFinance`/`visibleGoldenPathActions`(@hokko/shared・canViewFinance=false で金額 null 化) で **再利用可**／`apps/web/lib/db.ts` に `writeAudit`・`writeDataAccess(input: DataAccessInput)`(DataAccessLog/confidential_view) 実在／`packages/db/prisma/seed.ts` に `prisma.deal.create`(seed:237・deals＋dealStageHistory・deal 出現19)・`LocalBusinessLead`(3 refs)・`CompanyPolicy`(2 refs)＋既存 e2e(planning_hokko/operations/finance/golden_path が 72/0 green)＝**対象データ seed 済み**／`apps/web/components/shell/nav.ts`「Growth・DX OS」グループ(成長ダッシュボード/growth・成長イベント台帳/growth/events・Marketing OS・DX OS)に **1行追加で成立**。**判定10項目**: ①既存 schema のみで成立=YES(read-only 集約)／②新規 table/column=NO／③状態永続化(dismiss/snooze/read/unread/pin/priority override)=**v0 では不要・非目標**(採用時は新規 schema/migration→**STOP・別承認**)／④新規 RBAC action/role=NO(requireUser＋既存 read・canViewFinance・canAccessLabel)／⑤新規 seed=NO／⑥初期カード9案は既存データで表示可(既存顧客追加提案候補は件数/匿名指標中心)／⑦PII/Contact を増やさず成立=YES(Customer 一覧行レベル・Contact 単体は据え置き)／⑧財務値は canViewFinance/redaction で制御=YES／⑨writeAudit/writeDataAccess 配線可／⑩e2e 72/0 を壊さない(追加は read-only smoke＋redaction・既存 spec 不変・playwright.config.ts/ci.yml 変更しない)。**Gate 判定 = PASS(STOP 非該当)／v0 は既存 schema・既存 RBAC・既存 seed のみで成立**。**P3-CT-1 最小実装計画**を確定: 候補 URL `/growth/control-tower`・追加候補=`apps/web/app/(app)/growth/control-tower/page.tsx`＋`packages/shared/src/growth-control-tower.ts`(純ロジック・DB非依存・単体テスト)＋`apps/web/lib/domains/growth/control-tower.ts`(データ整形＋finance redact)・変更候補=nav.ts 1行(実装は別承認)・触らない=schema/migrations/seed/rbac.ts/playwright.config.ts/ci.yml/package/lock/369-vault・再利用=requireUser/hasPermission/getGoldenPathExecutiveDashboardData/redactExecutiveFinance/visibleGoldenPathActions/summarizeGrowthEvents/writeAudit/writeDataAccess・権限/redaction/監査/AI 境界(FakeLLM・下書きのみ・重要操作は ApprovalRequest)・テスト追加=`growth_control_tower.spec.ts`(read-only smoke＋redaction)・STOP 条件・実装プロンプト案。**Control Tower v0 は実装未着手(本書は Gate と計画のみ・コードは書いていない)**。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・Control Tower v0 実装開始なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **Gate PASS / 既存 schema・RBAC・seed のみで成立 / 実装未着手 / 将来 状態永続化・新規権限・新規 seed が要れば実装前 STOP・別承認**。
- 記録: doc147（監査）＋ `docs/roadmap/48`（Candidate）。次は doc147/roadmap48 push-only（別承認）→ P3-CT-1 Control Tower v0 read-only 画面 実装ミッション（別承認・実装あり）。**外部送信・実LLM・課金・本番 deploy は個別承認制を維持**。

## AI Growth Opportunity Control Tower v0 設計 — Candidate・docs-only・判定 設計完了 / 実装未着手 / schema 影響があれば別承認

- **doc146 / `docs/roadmap/47_ai_growth_control_tower_v0_design_candidate.md`**（2026-07-08）。Phase 3 GO（doc145/roadmap46）を受けた最初の縦切り **AI Growth Opportunity Control Tower v0** を **docs-only で設計**（実装はしない）。**設計着手の前提 CI**: doc145/roadmap46 push で起動した **run 28938318122（run_number 145・event push・head_sha d36887d）= completed / success・stage1 success・stage3_e2e success・Run E2E step success・Upload report on failure=skipped**＝これで stage3_e2e は **run 28930122157（142）/28934614261（143）/28937029131（144）/28938318122（145）の 4 run 連続で完全 green（72/0）**。**設計要点**: 目的＝既存データ（LeadMap リード `LocalBusinessLead`(schema:2756)・商談 `Deal`(622)・Company Brain・Golden Path・Finance Bridge・Usage/Audit）から成長機会を1画面（候補 URL `/growth/control-tower`・既存 `apps/web/app/(app)/growth/page.tsx`・`/growth/events`・nav.ts「Growth・DX OS」の隣）に **read-only 集約**し社長が次の一手を判断／非目標＝外部送信自動化・実LLM・課金・本番 deploy・runtime 解禁・externalAiAllowed true・EXTERNAL_SEND_ENABLED true・新規 schema 先行追加・AI の自動承認/削除/送信は**しない**／初期カード9案（未追客リード・停滞商談・高機会リード・Company Brain 改善提案・低粗利改善候補・未回収リスク・次回接触推奨・既存顧客追加提案候補・社長が見るべき成長機会）／権限＝tenantId スコープ・requireUser・hasPermission(`packages/shared/rbac.ts`)・canViewFinance・canAccessLabel/assertCanViewConfidential(`packages/shared/src/policy.ts`)・**Customer 一覧行レベル/Contact 単体は据え置き・生 PII 列を足さない**／監査＝writeAudit・機密参照 writeDataAccess(`apps/web/lib/db.ts`)・ai_reference・UsageEvent 既存方針（非課金・raw/secret/金額/PII を入れない）／AI 境界＝FakeLLM 決定論・AI は外部送信/承認/削除/重要操作直接実行なし・生成物は下書き・重要操作は ApprovalRequest／redaction＝既存 `events/[id]/page.tsx` の `canViewFinance ? 金額 : redaction` 踏襲（スタッフに原価・粗利の実値露出なし・件数中心）。**schema 影響判定**: **v0 は既存 schema のみで read-only 集約が成立する見込み**（新規テーブル/カラム想定なし）。ただし「カード dismiss/スヌーズ/既読 の状態永続化」「Control Tower 専用の新規権限 action/ロール」「新規デモデータ」が必要になった時点で**実装前に STOP して別承認**（schema/migration/RBAC/seed）。**実装フェーズ案**: P3-CT-0(docs設計)→CT-1(read-only 画面)→CT-2(優先度ロジック・純粋関数＋単体テスト)→CT-3(監査配線)→CT-4(AI 下書き・FakeLLM)→CT-5(承認導線・OutreachDraft(2931)→/approvals へ deep link・新規送信なし)→CT-6(e2e・read-only smoke＋redaction・既存 72/0 を壊さない)→CT-7(push/CI/Gate)・各段別承認・stop 条件つき。**テスト方針**: 既存 e2e 72/0 を壊さない・追加は read-only smoke 相当＋redaction・security/HCG/Consent/Suppression 回帰なし・playwright.config.ts/ci.yml 変更しない方針。roadmap47 に目的（§1）・非目標（§2）・対象データ（§3）・カード案（§4）・画面（§5）・権限（§6）・監査（§7）・AI 境界（§8）・schema 影響判定（§9）・テスト（§10）・実装フェーズ（§11）・50カテゴリ Matrix（§12・C41-44 直接Phase3開始＋C03/C06/C08/C46 直接）・Evidence/Assumption/Unknowns/Risk（§13）・次回プロンプト（§14）を記録。**Phase 3 GO 済みだが Control Tower v0 は設計のみ・実装未着手**。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・Control Tower v0 実装開始なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **設計完了 / 実装未着手 / schema 影響があれば別承認**。
- 記録: doc146（監査）＋ `docs/roadmap/47`（Candidate）。次は Control Tower v0 実装前 Gate / schema 影響判定 / 最小実装計画（docs-only）、および doc146/roadmap47 push-only（別承認）。**外部送信・実LLM・課金・本番 deploy・schema/RBAC/seed 変更・実装着手は個別承認制を維持**。

## Phase 3 GO 記録 — Candidate・docs-only・判定 Phase 3 GO（人間判断・正式記録）/ 実装は未着手 / 次は AI Growth Opportunity Control Tower v0 設計

- **doc145 / `docs/roadmap/46_phase3_go_record_candidate.md`**（2026-07-08）。doc144/roadmap45 の判断材料を受け、人間が Phase 3（AI Growth Engine）進入を **GO** と判断した事実を正式記録（**Phase 3 GO は実装開始ではない**・設計は次ミッション）。**GO の CI 実測根拠**: doc144/roadmap45 push で起動した **run 28937029131（run_number 144・event push・head_sha 18d246f）= completed / success・stage1 success（safety/unit/typecheck/lint 全 success）・stage3_e2e success（migrate/seed/build/chromium/Run E2E 全 success）・Run E2E ログ「72 passed (1.0m)」（0 failed）・Upload report on failure=skipped・operations:44 ✓(2.3s)・planning_hokko:45 ✓(1.3s)**。これで stage3_e2e は **run 28930122157（142）/28934614261（143）/28937029131（144）の 3 run 連続で完全 green（72/0）**。**C=SEED_DATA_DRIFT=0／D=TRUE_APP_BUG=0／F=INSUFFICIENT_EVIDENCE=0・F3 seed 不要・schema/migration 不要**（tests-only 修正のみで到達）。**redaction 健全**（operations:44・planning_hokko:45 green＝スタッフが実案件詳細に到達も `原価・粗利は財務閲覧権限が必要です（機密情報）。`（events/[id]/page.tsx line173）のみ描画・原価・粗利の実値露出なし）。**安全封印の現状値（GO 後も維持）**: EXTERNAL_SEND_ENABLED=false・LLM_PROVIDER=fake・MAIL_PROVIDER=log・externalAiAllowed 既定 false・SuppressionList 強制・Human Certification Gate 維持・Consent 用途別分離・outreach opt-out 正式方針。**人間 GO 判断6件**: ①高機密 runtime 統制②（Customer 一覧行レベル・Contact 単体）据え置きで進める=GO（生PII列を足さない不変条件・格上げ別承認）／②outreach opt-out 正式化=GO／③positive Consent 用途別分離=GO／④回帰ゲートは CI 実測で充足=GO（3 run 連続 72/0）／⑤外部送信・実LLM・課金・本番 deploy は個別承認制維持=GO／⑥Phase 3 最初の縦切り＝**AI Growth Opportunity Control Tower v0**。**Phase 3 進入を GO として正式記録**するが、**runtime 解禁・externalAiAllowed true・EXTERNAL_SEND_ENABLED true・外部送信・実LLM・AIコスト・課金・本番 deploy には進まず**、次は AI Growth Opportunity Control Tower v0 の設計（docs-only）から着手。**Control Tower v0**＝既存データ（LeadMap リード・商談・Company Brain・監査/Usage）から成長機会を1画面に可視化し人間承認で動く導線へつなぐ read-only 中心ダッシュボード v0（非目標=外部送信自動化/実LLM/課金/本番/新規 schema 先行追加は各々別承認・封印境界=FakeLLM/EXTERNAL_SEND_ENABLED=false/externalAiAllowed 既定 false/read-only 中心/AI は外部送信・承認・削除なし/生成物は下書き/重要操作は ApprovalRequest/tenantId スコープ/hasPermission/writeAudit/writeDataAccess/redaction 踏襲）。roadmap46 に Phase 2-A 完了（§2）・CI green 3 run（§3）・C/D/F 確定（§4）・F3/schema 不要（§5）・redaction 健全（§6）・封印（§7）・人間 GO 判断（§8）・Phase 3 進入（§9）・Control Tower v0（§10）・個別承認境界（§11）・やってはいけないこと（§12）・50カテゴリ Matrix（§13・C41-44 を直接Phase3開始へ格上げ）・Evidence Map（§17）・次回プロンプト（§20）を記録。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/課金/本番なし・runtime 解禁なし・Phase 3 実装開始なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **Phase 3 GO（人間判断・正式記録）/ 実装は未着手 / 次は AI Growth Opportunity Control Tower v0 設計**。
- 記録: doc145（監査）＋ `docs/roadmap/46`（Candidate）。次は AI Growth Opportunity Control Tower v0 設計ミッション（docs-only）、および doc145/roadmap46 push-only（別承認）。**外部送信・実LLM・課金・本番 deploy は個別承認制を維持**。

## Phase 3 最終 Phase Gate 判断シート — Candidate・docs-only・判定 技術ゲート大部分充足 / 最終 Phase Gate 人間承認前のため HOLD

- **doc144 / `docs/roadmap/45_phase3_final_gate_decision_sheet_candidate.md`**（2026-07-08）。人間が Phase 3 GO/HOLD を正式判断するための最終判断材料を1枚に集約（**本書は Phase 3 GO そのものではない・最終判断は人間**）。**技術ゲート充足状況**: ①Phase 2-A 完了記録=充足（doc48／doc14 §45・基準 85f1bf3）／②高機密ラベル runtime 統制=Customer 詳細/meetings/finance/invoices は assertCanViewConfidential+canAccessLabel+DataAccessLog で閉・Customer 一覧行レベル/Contact 単体は据え置き（判定 B・doc124-127・格上げ別承認）＝条件付き充足／③外部送信 Human Certification Gate=充足（requiresApproval+EXTERNAL_SEND_ENABLED 既定 false+decideApprovalAction・AI は assertAiToolAllowed で external_send 直接不可）／④Consent/SuppressionList=充足（isSuppressed 強制・unsubscribe 検知）／⑤AI 境界=充足（AI_READABLE_LABELS=NORMAL/INTERNAL・FakeLLM 決定論・externalAiAllowed 既定 false）／⑥**回帰ゲート緑（e2e 含む）=充足（run 28930122157〔run_number 142・head_sha a6447b9〕と run 28934614261〔run_number 143・head_sha 75af6e8＝docs-only push〕の 2 run 連続で stage1 success・stage3_e2e success・Run E2E 72 passed / 0 failed・Upload report on failure=skipped／docs-only push でも ci.yml 定義どおり stage1・stage3_e2e が起動し両方 green）**。**C=SEED_DATA_DRIFT=0／D=TRUE_APP_BUG=0／F=INSUFFICIENT_EVIDENCE=0 最終確定・F3 seed 不要・schema/migration 不要**。**redaction 健全**（operations:44・planning_hokko:45 green＝スタッフが実案件詳細に到達も `原価・粗利は財務閲覧権限が必要です（機密情報）。`（events/[id]/page.tsx line173）のみ描画・原価・粗利の実値露出なし）。**Global AI Rules 現状値**: LLM_PROVIDER=fake・MAIL_PROVIDER=log・EXTERNAL_SEND_ENABLED=false・externalAiAllowed 既定 false（CI 実行時も維持）。**まだ GO でない理由=最終 Phase Gate 人間承認が未実施**（技術条件⑥は充足・②据え置き方針の承認と最終意思決定は人間）。**人間が判断すべき GO/HOLD 論点6件**: 高機密 runtime 統制②の据え置き可否・outreach opt-out（Suppression 強制）正式化・positive Consent 用途別分離/outreach 非必須・回帰ゲートは CI 実測（72/0）で可か・Phase 3 最初の縦切りスコープ（runtime 解禁/外部送信/実LLM/課金を伴わない設計か）・GO 後も EXTERNAL_SEND_ENABLED/externalAiAllowed true 解禁/実LLM/実課金/本番 deploy は個別承認必須の原則維持。**GO の初手**=Phase 3 GO 記録ミッション（docs-only・封印維持）→Phase 3 最小縦切り設計（docs-only）。**HOLD の初手**=高機密 runtime 統制②格上げ設計 or Phase 3 縦切り候補棚卸し（docs-only）。roadmap45 に Phase 2-A 完了（§1）・CI green 証跡（§2）・C/D/F 確定（§3）・F3/schema 不要（§4-5）・redaction 健全（§6）・4ゲート状態（§7）・Global AI Rules（§8）・GO 条件充足状況（§9）・まだ GO でない理由（§10）・GO/HOLD 論点（§11）・GO の初手（§12）・HOLD の保留（§13）・やってはいけないこと（§14）・50カテゴリ Matrix（§15・直接C03/C06/C08/C37/C38/C39/C46）・20大/追加19/5本柱接続（§16-18）・Evidence Map（§19）・次回プロンプト（§20）を記録。**Phase 3 進入は引き続き HOLD**。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・runtime 解禁なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **技術ゲート大部分充足 / 最終 Phase Gate 人間承認前のため HOLD**。
- 記録: doc144（監査）＋ `docs/roadmap/45`（Candidate）。次は人間による **最終 Phase 3 Phase Gate 承認（GO/HOLD 正式判断）**、および doc144/roadmap45 push-only（別承認）。GO の場合のみ Phase 3 GO 記録ミッションへ。**F3 seed / schema は不要**。

## CI Stage 3 E2E F1e green result — Candidate・docs-only・判定 F1e完全成功 / stage3_e2e green / 72 passed 0 failed / Phase 3 HOLD（最終 Phase Gate 承認前）

- **doc143 / `docs/roadmap/44_ci_stage3_e2e_f1e_green_result_candidate.md`**（2026-07-08）。F1e tests-only 修正コミット `a6447b9` の push 後 CI **run 28930122157（run_number 142・event push・head_sha a6447b9・workflow ci.yml・branch `claude/ci-stage3-e2e-f1d-selectors-hikwbg`）= completed / success** を read-only 実測で記録。**stage1（job 85826925313）= success**（Set up〜checkout〜pnpm/setup-node〜Install〜Generate Prisma〜**Company Brain safety checks**〜**Unit tests**〜**Typecheck**〜**Lint** 全 success）。**stage3_e2e（job 85827191326）= success**（Initialize containers[ephemeral Postgres pgvector/pgvector:pg16]〜checkout〜Install〜**Write CI .env**〜Generate Prisma〜**Apply migrations**〜**Seed demo data**〜**Build web**〜**Install Playwright browser(chromium)**〜**Run E2E(playwright) success**〜Stop containers 全 success）。**「Upload Playwright report and traces on failure」= skipped**（`if: failure()` のため＝失敗ゼロの傍証）。**Run E2E ログ実測**: 「Running 72 tests using 2 workers」→ 全 ✓ →「**72 passed (1.1m)**」（failed/flaky/skipped 表示なし）。前回 run 28927924922（run_number 141・F1e前 HEAD 84d2a78）= failure・66 passed / 6 failed → 今回 **+6 passed / −6 failed・退行ゼロ・72/72**。**Run E2E env（ログ実測）**: `LLM_PROVIDER=fake`・`MAIL_PROVIDER=log`・`EXTERNAL_SEND_ENABLED=false`（封印維持）。**F1e対象6件すべて ✓**: operations.spec.ts:44:1「スタッフはイベント原価・粗利の機密情報を閲覧できない」(2.5s・A/setup)・planning_hokko_golden_path.spec.ts:16:1(1.6s・B)・:24:1(1.5s・A)・:35:1(1.5s・A)・:45:1「スタッフ（財務権限なし）は案件詳細で原価・粗利を見られない」(1.5s・A・redaction)・golden_path_actions.spec.ts:15:1(1.4s・B)。**redaction 2件（operations:44・planning_hokko:45）green＝スタッフが実案件詳細に到達し `原価・粗利は財務閲覧権限が必要です（機密情報）。`（events/[id]/page.tsx line173・canViewFinance 偽）が描画・原価・粗利の実値は非表示＝機密漏えいなし**。他の「スタッフは機密を閲覧できない」系（executive_dashboard:47・finance_bridge:40・finance_formalize:30・invoice_payment:32・operations_exec:50・dunning:70/81 等）も ✓。**C=SEED_DATA_DRIFT=0／D=TRUE_APP_BUG=0／F=INSUFFICIENT_EVIDENCE=0 を CI 実測で最終確定**（tests-only 修正のみで 72/0＝seed/schema/app 無変更）。**F3 seed / schema 変更は不要**（確定）。**regression なし**（F1d 緑化 4件維持・既存 66 件維持）。**safety/HCG/Consent/Security/Marketplace Gate 影響なし・Global AI Rules 維持**。roadmap44 に run 概要（§3）・stage1/stage3_e2e 詳細（§4-5）・72/0（§6）・F1e 6件 green（§7）・redaction green（§8）・機密漏えいなし（§9）・C=0/D=0/F=0 確定（§10）・F3/schema 不要（§11-12）・regression なし（§13）・Phase 3 Gate 影響（§16）・現在地10項目（§17-1〜17-10）・50カテゴリ Matrix（§18・直接C03/C06/C08/C37/C38/C39/C46）・Evidence Map（§19）を記録。**Phase 3 GO 条件⑥（回帰ゲート緑＝e2e含む）は技術的に充足したが、Phase 3 進入は最終 Phase Gate 人間承認が必須のため HOLD 継続**（本記録は「Phase 3 GO」ではない）。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・runtime 解禁なし・artifact download/git add なし・network policy回避なし・force push/amend/rebase/reset/rerun/cancel/send_later なし・369-vault非編集・push なし（commit-only）**。判定 **F1e完全成功 / stage3_e2e green / 72 passed 0 failed / Phase 3 HOLD（最終 Phase Gate 承認前）**。
- 記録: doc143（監査）＋ `docs/roadmap/44`（Candidate）。次は doc143/roadmap44 push-only（別承認）→ 人間による **最終 Phase 3 Phase Gate 承認（GO/HOLD 正式判断）**、または **Phase 3 GO 記録ミッション**（人間が GO を選択した場合）。**F3 seed / schema は不要**。

## CI Stage 3 E2E F1e tests-only fix — Candidate・tests-only・判定 F1e適用完了 / CI_STAGE3_E2E_RED継続（push→CI緑化まで）

- **doc142 / `docs/roadmap/43_ci_stage3_e2e_f1e_tests_only_fix_candidate.md`＋e2e spec 3本**（2026-07-08）。doc141/roadmap42 の C6 artifact 分析で確定した残6件（**C=0/D=0・真因 tests-only**）を **e2e spec のみ**で最小修正。**app read-only 確認**（app は変更せず）: `operations/events/[id]/page.tsx` に `Golden Path — 現在地と次の一手`（CardTitle line96）・`粗利率` Stat（line170）・`原価・粗利は財務閲覧権限が必要です（機密情報）。`（line173）が実在し、財務欄は `canViewFinance ? 金額 : redaction`（line165-174）で**権限どおり出し分け**（社長=金額／スタッフ=redaction・機密値非表示）＝app 健全・**D 証跡なし**。一覧 `operations/events/page.tsx` は先頭に「イベント案件を作成」→/new リンク（line30）・以降に実案件リンク `/operations/events/${e.id}`（line50）。作成フォームに `name="revenue"`（`events/new/page.tsx` line58）。/dashboard/ceo に `推奨アクション`（line83）・`今すぐ見るべき案件（優先度順）`（line231）、/planning-hokko に `今すぐ見るべき案件（優先度順）`（line42）・`案件詳細・次の一手 →`（line95）が実在。**修正6件**: golden_path_actions:15＝`getByText('対処:')`→`getByRole('heading',{name:/推奨アクション/})`（B）／planning_hokko:16＝`getByText('Golden Path — 現在地と次の一手')`→`getByRole('heading',{name:/今すぐ見るべき案件/})`（B）／planning_hokko:24/35/45＝一覧セレクタ `a[href^="/operations/events/"]:not([href$="/new"])').first()`（A・/new 除外／:24 assertion は `getByRole('heading',{name:/Golden Path — 現在地と次の一手/})`）／operations:44＝`input[name="revenue"]` 入力＋`Promise.all([page.waitForURL(/\/operations\/events\/(?!new(?:[/?#]|$))[^/?#]+/), 「案件を作成」click])` で実 detail URL を race-safe 捕捉（A/setup）。**redaction/finance assertion（原価・粗利は財務閲覧権限が必要です・粗利率・Golden Path — 現在地と次の一手）は app 実在のため維持**。**検証**: `pnpm --filter @hokko/web typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0・`git diff --check` OK・secret NONE・旧パターン grep=0・新パターン grep=存在。e2e 実緑は ローカル DB/ブラウザ不在で未実行＝**push 後の CI（stage3_e2e）で確認**（**66 passed / 6 failed → 72/0** 見込み）。**F3 seed / schema は不要**（C=0）。**残 F 論点**: redaction 2件の detail 到達後の描画は push 後 CI で確認（出ない or 機密露出なら D=TRUE_APP_BUG として停止・人間承認）。**失敗は表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap43 に app read-only 確認（§3）・修正内容（§4）・redaction 再検証方針（§7）・現在地10項目（§9-1〜9-10）・50カテゴリ Matrix（§10・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・artifact 非 git add・369-vault非編集・push なし（commit-only）**。判定 **F1e適用完了 / CI_STAGE3_E2E_RED継続（push→CI緑化まで）/ Phase 3 HOLD**。
- 記録: doc142（監査）＋ `docs/roadmap/43`（Candidate）＋ e2e spec 3本（tests-only）。次は F1e push-only（別承認）→ CI stage3_e2e で 66/6→72/0＋redaction 2件緑を確認 → stage3_e2e green → 最終 Phase Gate 承認。**F3 seed / schema は不要**。

## CI Stage 3 E2E C6 artifact analysis — Candidate・docs-only・判定 C=0/D=0/F=0（真因 tests-only）/ CI_STAGE3_E2E_RED継続

- **doc141 / `docs/roadmap/42_ci_stage3_e2e_c6_artifact_analysis_candidate.md`**（2026-07-08）。ローカル到達 artifact（**playwright-report・ID 8158827253・run 28919165959**・チャット添付・18MB・54 files・unzip -t OK）を read-only で `/tmp/369-artifact-8158827253` へ展開し、残 **6 failed** の **error-context.md（ARIA スナップショット）＋test-failed-1.png** を目視分析（trace は存在確認のみ・zip/展開物/screenshot/trace は **git add せず**・repo 内に展開せず）。**重要訂正**: 前分類「C暫定6件」は誤りで、**artifact 目視により C=SEED_DATA_DRIFT=0／D=TRUE_APP_BUG=0／F=INSUFFICIENT_EVIDENCE=0**（データ存在・アプリ健全）と確定。**真因は tests-only の A=TEST_SELECTOR_DRIFT/setup 4件＋B=TEXT_EXPECTATION_DRIFT 2件**: golden_path_actions:15＝**B**（/dashboard/ceo 完全描画・`:18`「今すぐ見るべき案件」通過・`:20` `対処:` 不在＝`対処:` は /planning-hokko 側描画）／planning_hokko:16＝**B**（/planning-hokko に3案件・KPI・`対処:`・`次の一手`・財務値（社長は正当閲覧）まで描画・期待 literal `Golden Path — 現在地と次の一手` のみ不在）／planning_hokko:24/35/45＝**A**（`page.locator('a[href^="/operations/events/"]').first()` が一覧先頭の「イベント案件を作成」(/operations/events/new) リンクを掴み作成フォームへ・案件は seed に3件存在＝ID確認済）／operations:44＝**A/setup**（社長作成ステップ想定売上未入力＋`waitForURL('**/operations/events/**')` race で url=/new 捕捉→スタッフが /new を開く）。**redaction 2件（operations:44・planning_hokko:45）はスタッフが /operations/events/new 作成フォームに留まり、原価・粗利・金額いずれの機密値も露出なし＝D重大（機密漏えい）なし**。社長は /planning-hokko で 売上¥1,980,000/原価¥1,150,000/粗利¥830,000 を権限どおり正当閲覧（過剰表示ではない）。**F3 seed / schema 変更は不要**（C=0）。緑化は **tests-only（F1e）で 66/6→72/0 到達可能**。残 F 論点: redaction メッセージの描画正当性は detail 未到達のため未検証（漏えいは無い・F1e 後に再確認）。**失敗は表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap42 に artifact展開（§3）・6件個別分析（§6-11）・C/D/F最終分類（§12）・redaction/Security/HCG（§13）・F3 seed引き継ぎ（§14・不要）・現在地10項目（§16-1〜16-10）・50カテゴリ Matrix（§17・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・artifact blob URL curl なし・network policy回避なし・369-vault非編集・push なし（commit-only）**。判定 **C=0/D=0/F=0（真因 tests-only A=4/B=2）/ CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc141（監査）＋ `docs/roadmap/42`（Candidate）。次は F1e tests-only 修正（別承認）→ CI stage3_e2e で 66/6→72/0 を確認 → redaction 2件の detail 到達後再検証 → stage3_e2e green → 最終 Phase Gate 承認。**F3 seed / schema は不要**。

## CI Stage 3 E2E F1d result — Candidate・docs-only・判定 F1d完全成功 / CI_STAGE3_E2E_RED継続

- **doc140 / `docs/roadmap/41_ci_stage3_e2e_f1d_result_candidate.md`**（2026-07-08）。**F1d** push 後の CI run **28917520358**（branch `claude/ci-stage3-e2e-f1d-selectors-hikwbg`・head_sha `5dda39d8…`・run_number 139・push）実測を **docs-only** で記録＝**stage1 success**／**stage3_e2e failure**（基盤ステップ migrate/seed/build/browser install は全 success・"Run E2E (playwright)" step のみ failure）／**Run E2E** = **66 passed / 6 failed**（**F1d前** run 28885319767 = **62 passed / 10 failed**・差分 **+4 passed / -4 failed**・退行ゼロ・env `LLM_PROVIDER=fake`/`MAIL_PROVIDER=log`/`EXTERNAL_SEND_ENABLED=false`）。**F1d完全成功**: dunning:15・dunning:50・executive_dashboard:15・executive_dashboard:37 が**失敗一覧から消滅**＝**A=TEST_SELECTOR_DRIFT（strict-mode）完全消化**（残 A=0）。**executive_dashboard:15 の後続 Golden Path KPI 文言は C 転移なし**（後続アサーションまで全通過）。**regression なし**。**残りは C暫定6件のみ**（golden_path_actions:15・operations:44・planning_hokko:16/24/35/45＝element-not-found・**SEED_DATA_DRIFT**）。**D=TRUE_APP_BUG は証跡なし（0）**（redaction 2件は機密値露出の証跡なし）。artifact 生成済み（**playwright-report・ID 8158244025・17725314 bytes・54 files**・本環境 proxy 403 で**バイナリ download せず**）。**失敗は表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap41 に CI結果（§3-4）・F1d対象4件（§5）・残6件（§6）・66/6 の意味（§7）・regression（§8）・artifact（§9）・現在地10項目（§16-1〜16-10）・50カテゴリ Matrix（§17・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・network policy回避なし・369-vault非編集・push なし（commit-only）**。判定 **F1d完全成功 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc140（監査）＋ `docs/roadmap/41`（Candidate）。次は doc140/roadmap41 push-only（別承認）→ 残 C暫定6件の C/D 確定（artifact screenshot 人手取得 or network 許可）→ F3（C=6件 seed・schema影響事前停止条件・別承認）。

## CI Stage 3 E2E F1d test selector fix — Candidate・tests-only・判定 F1d適用完了 / CI_STAGE3_E2E_RED継続

- **doc139 / `docs/roadmap/40_ci_stage3_e2e_f1d_test_selector_fix_candidate.md`＋e2e spec 2本**（2026-07-08）。**F2 log-based** 分析（doc138/roadmap39）で **job log 確定**した **strict-mode** 4件（A=**TEST_SELECTOR_DRIFT**）を **e2e spec のみ**で最小修正。**修正**: dunning:15（実落:23）`page.locator('#dunning').getByText(/督促/)`→`getByRole('heading',{name:/入金確認・督促/})`（CardTitle=`<h3>`・`invoices/[id]/page.tsx:177`）／dunning:50（:53）`getByText('承認待ち')`→`getByRole('heading',{name:'承認待ち'})`（PageHeader=`<h1>`・`approvals/page.tsx:31`）／executive_dashboard:15（:19）`getByText('社長コックピット',{exact:false})`→`getByRole('heading',{name:'社長コックピット'})`（PageHeader=`<h1>`・`dashboard/ceo/page.tsx:67`）／executive_dashboard:37（:43）`getByRole('link',{name:/プランニングホッコー/})`→`getByRole('link',{name:'プランニングホッコー →'})`（本文リンク・`operations/events/[id]/page.tsx:155`）＋**同一テスト内の連続する戻り導線で次に露見する同型 strict-mode を予防した最小追加**として :44 `getByRole('link',{name:/社長コックピット/})`→`getByRole('link',{name:'社長コックピット →'})`（本文リンク・`operations/events/[id]/page.tsx:156`）。要素はいずれも app に**描画済み**（**SEED_DATA_DRIFT** ではない・read-only で heading/本文リンク実在確認）。**検証**: `pnpm --filter @hokko/web typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0・`git diff --check` OK・secret NONE・旧曖昧セレクタ4件 grep=0・新セレクタ4件 grep=存在。e2e 実緑は ローカル DB/ブラウザ不在で未実行＝**push 後の CI（stage3_e2e）で確認**（**62 passed / 10 failed → 66/6** 見込み）。**C暫定6件は未修正**（golden_path_actions:15・operations:44・planning_hokko:16/24/35/45＝**SEED_DATA_DRIFT**・F3 別承認・schema 影響事前停止条件）。**D=TRUE_APP_BUG は0のまま**。**失敗は表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap40 に F1d 4件一覧（§6）・個別修正（§7-10）・現在地10項目（§14-1〜14-10）・50カテゴリ Matrix（§15・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。判定 **F1d適用完了 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc139（監査）＋ `docs/roadmap/40`（Candidate）＋ e2e spec 2本（tests-only）。次は F1d push-only（別承認）→ CI stage3_e2e 再実行で 62/10→66/6 見込みを確認 → artifact screenshot 確定（人手download or network許可）→ F3（C=6件 seed・schema影響事前停止条件・別承認）。

## CI Stage 3 E2E F2 log-based artifact-blocked analysis — Candidate・docs-only・判定 log-based分類完了 / CI_STAGE3_E2E_RED継続

- **doc138 / `docs/roadmap/39_ci_stage3_e2e_f2_log_based_artifact_blocked_analysis_candidate.md`**（2026-07-07）。F2 **artifact**（playwright-report・id 8145981012・29933595 bytes・expired=false・expires 2026-07-14）はメタデータ取得成功も、**Azure blob storage への download が proxy 403（CONNECT tunnel failed）でブロック＝artifact取得不能**（network policy 回避せず）。代替として run 28885319767（**stage1 success**／**stage3_e2e failure**／**Run E2E 62 passed / 10 failed**）の job logs（read-only）の error/locator/**strict-mode**/element-not-found を一次証跡に **log-based 分類**。**重大訂正**: doc136/roadmap37 の「残10件は全て C=**SEED_DATA_DRIFT**」は誤りで、フルログにより **dunning:15（`#dunning`内 `/督促/` が h3＋未回収額p の2要素）・dunning:50（`承認待ち` link＋h1）・executive_dashboard:15（`社長コックピット` link＋h1）・executive_dashboard:37（`プランニングホッコー` link×2）の4件は strict-mode（A=**TEST_SELECTOR_DRIFT**）＝要素は描画済み**と判明。**最終分類: A=4（log確定・tests-only可）／C暫定=6（golden_path_actions:15・operations:44・planning_hokko:16/24/35/45＝element-not-found・screenshot未確認でC暫定＋F=**INSUFFICIENT_EVIDENCE**）／D=**TRUE_APP_BUG**=0（証跡なし・security.spec全passed）／E=0**。redaction系2件（operations:44・planning_hokko:45）は機密値露出の証跡なし＝D断定せず。**Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap39 に artifact取得不能（§4-5）・log-based（§6）・10件個別分析（§9-18）・A/C/D 範囲（§19-21）・現在地10項目（§26-1〜26-10）・50カテゴリ Matrix（§27・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/playwright.config.ts/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・network policy回避なし・369-vault非編集・push なし（commit-only）**。判定 **log-based分類完了 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc138（監査）＋ `docs/roadmap/39`（Candidate）。次は F1d（A=4件 tests-only・別承認）→ artifact screenshot 確定（人手download or network許可）→ F3（C=6件 seed・schema影響事前停止条件・別承認）。

## CI Stage 3 E2E F2 diagnostics — Candidate・config-only・判定 F2診断基盤追加完了 / CI_STAGE3_E2E_RED継続

- **doc137 / `docs/roadmap/38_ci_stage3_e2e_f2_diagnostics_candidate.md`＋config 2本**（2026-07-07）。残 C10件の C/D 最終確定に向け、CI 失敗時に **html report / trace / screenshot** を **artifact** 取得できる **F2 診断基盤**を **config-only** で追加。**原因**: `reporter:'list'`（html 未生成）＋`use.trace:'on-first-retry'` だが `retries:0`（trace 永久未取得）＝直近 run 28882890123 で `No files were found with the provided path: apps/web/playwright-report`。**変更（config-only）**: `apps/web/playwright.config.ts`（`reporter:[['list'],['html',{outputFolder:'playwright-report',open:'never'}]]`／`use.trace:'retain-on-failure'`／`use.screenshot:'only-on-failure'`・trace/screenshot は既定 `test-results/`・gitignore 済み）・`.github/workflows/ci.yml`（Upload artifact の `path` に `apps/web/playwright-report`＋`apps/web/test-results` を追加＝**env/service/実行ステップは不変**・`EXTERNAL_SEND_ENABLED=false`/`LLM_PROVIDER=fake`/`MAIL_PROVIDER=log`/ephemeral Postgres 維持）。**html reporter は `@playwright/test ^1.49.1` 組込のため package.json/lockfile 変更なし**。**検証**: `pnpm --filter @hokko/web typecheck` exit 0・`pnpm lint` exit 0・`node scripts/check-company-brain-safety.mjs` exit 0・diff --check OK・secret NONE。**テスト合否は不変（62 passed / 10 failed・診断基盤のみ）**。**CI_STAGE3_E2E_RED は C=SEED_DATA_DRIFT 10件残存のため継続**（dunning:15/50・executive_dashboard:15/37・golden_path_actions:15・operations:44・planning_hokko:16/24/35/45）。**D=TRUE_APP_BUG は0のまま**（F2 artifact で最終確定）。**失敗は表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。roadmap38 に F2 設定（§5）・変更/非変更（§6-7）・現在地10項目（§10-1〜10-10）・50カテゴリ Matrix（§11・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・tests期待値変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals変更なし・package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。判定 **F2診断基盤追加完了 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc137（監査）＋ `docs/roadmap/38`（Candidate）＋ config 2本（playwright.config.ts / ci.yml artifact path）。次は F2 push-only（別承認）→ CI で artifact 取得 → 残 C10件を C/D 最終確定（F3 データ整合は別承認）。

## CI Stage 3 E2E F1c result — Candidate・docs-only・判定 CI_STAGE3_E2E_RED / C10件へ収束

- **doc136 / `docs/roadmap/37_ci_stage3_e2e_f1c_result_candidate.md`**（2026-07-07）。**F1c** push 後の CI run 28879821278（HEAD c2d07c5・run_number 135）実測を **docs-only** で記録＝**stage1 success**／**stage3_e2e failure**（基盤ステップ migrate/seed/build/browser は全 success・**Run E2E** のみ failure）／**Run E2E** = **62 passed** / **10 failed**（**F1c** 前 run 28876766413 = **60 passed** / **12 failed**・差分 +2 passed / −2 failed・退行ゼロ・env `LLM_PROVIDER: fake`/`MAIL_PROVIDER: log`/`EXTERNAL_SEND_ENABLED: false` 維持）。**F1c対象2件はいずれも緑化**: finance_bridge:15 **FinanceEvent**（3要素 **strict-mode**→`getByRole('heading',{name:'直近の FinanceEvent'})`）・invoice_payment:25 **入金実績**（2要素 **strict-mode**→`getByText('入金実績',{exact:true})`）＝失敗一覧から消滅。**A=TEST_SELECTOR_DRIFT（strict-mode・計7件）/ B=文言ドリフト（1件）は全消化**。**残10件は全て C=SEED_DATA_DRIFT へ収束**（dunning:15/50・executive_dashboard:15/37・golden_path_actions:15・operations:44・planning_hokko:16/24/35/45＝element-not-found・前提データ不足）。**D=TRUE_APP_BUG は0のまま**（security.spec passed・redaction 失敗は前提イベント未生成で機密漏えいではない）。**失敗は operations/planning/finance/dunning/executive の表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap37 に CI結果（§3-4）・残10件一覧/分類（§5-6）・収束（§7-9）・F2/F3 前提（§10-11）・現在地10項目（§13-1〜13-10）・50カテゴリ Matrix（§14・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・tests変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。判定 **CI_STAGE3_E2E_RED継続 / C10件へ収束 / Phase 3 HOLD**。
- 記録: doc136（監査）＋ `docs/roadmap/37`（Candidate）。次は F2 診断（別承認・playwright html reporter+trace を ci.yml/config に追加→残 C10件の screenshot/trace 取得で C/D 最終確定）。

## CI Stage 3 E2E F1b result and F1c — Candidate・tests-only・判定 F1c適用完了 / CI_STAGE3_E2E_RED継続

- **doc135 / `docs/roadmap/36_ci_stage3_e2e_f1b_result_and_f1c_candidate.md`＋e2e spec 2本**（2026-07-07）。**F1b** push 後の CI run 28876766413（HEAD 4069fb6・run_number 134）実測を記録＝**stage1 success**／**stage3_e2e failure**（基盤ステップ migrate/seed/build/browser は全 success・**Run E2E** のみ failure）／**Run E2E** = **60 passed** / **12 failed**（**F1b** 前 run 28873089407 = **59 passed** / **13 failed**・**F1** 前 = **57 passed** / **15 failed**・**F1b** 差分 +1 passed / −1 failed・退行ゼロ）。**F1b の効果**: operations_exec:39 リスク **strict-mode** 緑化＝**operations_exec spec 全緑**（:15 棚卸・:22 案件名・:39 人員配置/リスク すべて緑・失敗一覧に operations_exec 系 0件）。**残12件の分類精緻化**（CI ログ実エラー）: finance_bridge:15 `getByText('FinanceEvent')`（3要素）・invoice_payment:25 `getByText('入金実績')`（2要素）が**新規 strict-mode（A=TEST_SELECTOR_DRIFT）**と判明。残10件は element-not-found の C=**SEED_DATA_DRIFT**（dunning:15/50・executive_dashboard:15/37・golden_path_actions:15・operations:44・planning_hokko:16/24/35/45）。**F1c（tests-only 2件）**: finance_bridge:19 `getByText('FinanceEvent',{exact:false})`→`getByRole('heading',{name:'直近の FinanceEvent'})`／invoice_payment:29 `getByText('入金実績',{exact:false})`→`getByText('入金実績',{exact:true})`（A=**TEST_SELECTOR_DRIFT**・app表示正常・見出し/完全一致で限定）。**検証**: `pnpm --filter @hokko/web typecheck` exit 0・`pnpm lint` exit 0・旧曖昧セレクタ2件 grep=0・`node scripts/check-company-brain-safety.mjs` exit 0。e2e 実緑は ローカル DB 不在で未実行＝**push 後の CI（stage3_e2e）で確認**（12→10 見込み）。**CI_STAGE3_E2E_RED は C=SEED_DATA_DRIFT 10件残存のため継続**（F2 診断→F3 データ整合へ）。**D=TRUE_APP_BUG は0のまま**。**失敗は operations/planning/finance/dunning/executive の表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap36 に CI結果（§3-5）・F1c 修正（§6-8）・現在地10項目（§12-1〜12-10）・50カテゴリ Matrix（§13・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。判定 **F1c適用完了 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc135（監査）＋ `docs/roadmap/36`（Candidate）＋ e2e spec 2本（tests-only）。次は doc135/F1c push-only（別承認）→ CI stage3_e2e 再実行で該当2行緑を確認（12→10 見込み）→ 残 C10件は F2 診断。

## CI Stage 3 E2E F1 result and F1b — Candidate・tests-only/docs-only・判定 F1b適用完了 / CI_STAGE3_E2E_RED継続

- **doc134 / `docs/roadmap/35_ci_stage3_e2e_f1_result_and_f1b_candidate.md`＋e2e spec 1本**（2026-07-07）。**F1** push 後の CI run 28873089407（HEAD 62ad994）実測を記録＝**stage1 success**／**stage3_e2e failure**／**Run E2E** = **59 passed** / **13 failed**（**F1** 前 run 28860862696 = **57 passed** / **15 failed**・差分 +2 passed / −2 failed・退行ゼロ・基盤緑）。**F1対象4件の結果**: (1) operations_exec:15 棚卸=**緑化** (2) operations:22 E2Eテスト案件=**緑化** (3) operations_exec:39 人員配置=**部分成功**（line45 heading 化は通過・line46 `getByText('リスク',{exact:false})` が4要素一致=**strict-mode** 露見） (4) planning_hokko:16 Golden Path=期待文言反映もカード未描画で **SEED_DATA_DRIFT**（C）へ転移。**F1b（tests-only 微修正）**: operations_exec:39 line46 を `getByRole('heading',{name:/リスク/}).first()` に限定（A=**TEST_SELECTOR_DRIFT**・app表示正常＝4要素存在・見出し厳密化）。物流タスク行（line47）は赤化なしのため非変更。**検証**: `pnpm --filter @hokko/web typecheck` exit 0・`pnpm lint` exit 0・旧曖昧セレクタ `getByText('リスク', { exact: false })` grep=0・`node scripts/check-company-brain-safety.mjs` exit 0。e2e 実緑は ローカル DB 不在で未実行＝**push 後の CI（stage3_e2e）で確認**。**CI_STAGE3_E2E_RED は C=SEED_DATA_DRIFT 11件残存のため継続**（F2 診断→F3 データ整合へ）。**D=TRUE_APP_BUG は0のまま**。**失敗は operations/planning/finance の表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap35 に CI結果（§3-4）・F1b 修正（§5-6）・現在地10項目（§10-1〜10-10）・50カテゴリ Matrix（§11・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。判定 **F1b適用完了 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc134（監査）＋ `docs/roadmap/35`（Candidate）＋ e2e spec 1本（tests-only）。次は doc134/F1b push-only（別承認）→ CI stage3_e2e 再実行でリスク行緑化を確認（13→12 見込み）→ 残 C11件は F2 診断。

## CI Stage 3 E2E F1 test fix — Candidate・tests-only・判定 F1適用完了 / CI_STAGE3_E2E_RED継続

- **doc133 / `docs/roadmap/34_ci_stage3_e2e_f1_test_fix_candidate.md`＋e2e spec 3本**（2026-07-07）。doc132/roadmap33 の **root-cause** 計画に基づき **F1**（A=**TEST_SELECTOR_DRIFT** 3件＋B=**TEXT_EXPECTATION_DRIFT** 1件）を **tests-only** で最小修正。**修正**: operations_exec:15 棚卸（`getByRole('heading',{name:/棚卸/})`＝h1「棚卸（実地在庫照合）」＋h3「棚卸を開始」の2要素一致=**strict-mode**→`'棚卸（実地在庫照合）'` 厳密化）／operations_exec:39 人員配置（`getByText('人員配置')`＝span＋h3「人員配置（0）」2要素→`getByRole('heading',{name:/人員配置/}).first()`）／operations:22 案件名（`getByText('E2Eテスト案件')`＝link＋h1 2要素→`getByRole('heading',{name:'E2Eテスト案件'})`＋同テスト `getByText('粗利率')`→`.first()`＝同カテゴリ最小追加）／planning_hokko:16 **expected text**（「現在地と次の一手（Golden Path）」は app 不在→実文言「Golden Path — 現在地と次の一手」へ）。**検証**: `pnpm --filter @hokko/web typecheck` exit 0・`pnpm lint` exit 0・旧文言 grep=0・`node scripts/check-company-brain-safety.mjs` exit 0。e2e 実緑は ローカル DB 不在で未実行＝**push 後の CI（stage3_e2e）で確認**。**CI_STAGE3_E2E_RED は C=SEED_DATA_DRIFT 11件残存のため継続**（F2 診断→F3 データ整合へ）。**D=TRUE_APP_BUG は0のまま**。**失敗は operations/planning/finance/dunning の表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系は緑**＝安全中核ゲート（HCG/Consent/Security）影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap34 に修正内容（§5-7）・検証（§9）・現在地10項目（§11-1〜11-10）・50カテゴリ Matrix（§12・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 進入は引き続き HOLD**（回帰ゲート緑=e2e含む 未達）。**app変更なし・seed変更なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。判定 **F1適用完了 / CI_STAGE3_E2E_RED継続 / Phase 3 HOLD**。
- 記録: doc133（監査）＋ `docs/roadmap/34`（Candidate）＋ e2e spec 3本（tests-only）。次は doc133/F1 push-only（別承認）→ CI stage3_e2e 再実行で該当4件緑を確認 → 残 C11件は F2 診断。

## CI Stage 3 E2E root-cause plan — Candidate・docs-only・判定 CI_STAGE3_E2E_RED / PLAYWRIGHT_FAIL

- **doc132 / `docs/roadmap/33_ci_stage3_e2e_root_cause_plan_candidate.md`**（2026-07-07）。CI run 28860862696（**stage1 success** / **stage3_e2e failure** / **Run E2E** = **57 passed** / **15 failed**）の15失敗を spec ごとに read-only で **root-cause** 深掘り。8 spec 実読＋期待文言を `apps/web/app`/`components` に rg 突合＋seed 確認。**最終分類**: A=**TEST_SELECTOR_DRIFT**（**strict-mode**）**3件確定**（operations_exec:15 棚卸/39 人員配置・operations:22 案件名＝ロケータ2要素一致・`.first()`/exact 化で解決・app変更不要）／B=**TEXT_EXPECTATION_DRIFT** **1件確定**（planning_hokko:16「現在地と次の一手（Golden Path）」が app に不在＝実文言「Golden Path — 現在地と次の一手」・テスト期待値更新）／C=**SEED_DATA_DRIFT** **11件（優勢・要ライブ確認）**（dunning:15/50・executive_dashboard:15/37・finance_bridge:15・golden_path_actions:15・invoice_payment:25・operations:44・planning_hokko:24/35/45＝期待文言はappに存在するが「一覧→先頭クリック→詳細アサート」型で前提データ不足が主因）／**D=TRUE_APP_BUG=0確定**（期待文言ほぼ存在＋`security.spec.ts` passed＝権限redact機能）／E=0。**最小修正計画 F1-F4**: F1=テスト側4件（A3＋B1・低リスク・app/seed非依存）→ F2=診断補強（playwright html reporter+trace・現状 report 未生成で screenshot なし）→ F3=seed/データ整合（C11件・schema影響の事前停止条件付き）→ F4=Dが出た場合のみ。**失敗は operations/planning/finance/dunning の表示系で、Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系はゼロ**＝安全中核ゲート（HCG/Consent/Security）緑・影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`LogEmailProvider`・`FakeLLM`・`externalAiAllowed` 既定false・runtime 解禁なし）。roadmap33 に最終分類表（§9）・修正順序（§10-11）・現在地10項目（§13-1〜13-10）・50カテゴリ Matrix（§14・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 GO 条件⑥（回帰ゲート緑＝e2e含む）未達→Phase 3 進入 HOLD 継続**。**実装なし・E2E/UI/seed修正なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。判定 **CI_STAGE3_E2E_RED / PLAYWRIGHT_FAIL / HOLD**。
- 記録: doc132（監査）＋ `docs/roadmap/33`（Candidate）。次は doc132 push-only（別承認）または F1（A=strict-mode 3件＋B 1件の最小テスト修正・別承認）。

## CI Stage 3 E2E failure triage — Candidate・docs-only・判定 CI_STAGE3_E2E_RED / PLAYWRIGHT_FAIL

- **doc131 / `docs/roadmap/32_ci_stage3_e2e_failure_triage_candidate.md`**（2026-07-07）。push 後の CI run 28860862696（HEAD 1e8bcd1）を read-only 分析。**stage1 success**（safety/test/typecheck/lint）／**stage3_e2e failure**だが基盤ステップは全成功（**migrate success**/**seed success**/**build success**/playwright chromium install）、**Run E2E** のみ失敗＝**57 passed** / **15 failed**（分類 **PLAYWRIGHT_FAIL**・環境/CI設定は正常＝ENV_RELATED=0）。**15失敗の切り分け**: A=TEST_SELECTOR_DRIFT（**strict-mode**）**3件確定**（operations_exec:15 棚卸／operations_exec:39 人員配置／operations:22 案件名＝ロケータが2要素一致＝**expected text** 曖昧・テスト spec のみの軽微修正・低リスク）／B=TEXT_EXPECTATION_DRIFT・C=SEED_DATA_DRIFT・D=TRUE_APP_BUG **12件（要read-only精査）**（dunning:15/50・executive_dashboard:15/37・finance_bridge:15・golden_path_actions:15・invoice_payment:25・operations:44・planning_hokko:16/24/35/45 の表示系）／E=ENV_RELATED=0。**失敗領域に Customer一覧/LocalBusinessLead/Contact/SuppressionList などの機密・同意・CRM閲覧統制系はゼロ**＝Phase 3 安全中核ゲート（HCG/Consent/Security）は緑・影響なし。封印維持（`EXTERNAL_SEND_ENABLED=false`・`LogEmailProvider`・**FakeLLM**・**externalAiAllowed** 既定false・runtime 解禁なし）。roadmap32 に15失敗一覧（§7）・分類（§8-10）・修正優先度（§12）・現在地10項目（§13-1〜13-10）・50カテゴリ Matrix（§14・直接C03/C06/C08/C37/C38/C39/C46）を記録。**Phase 3 GO 条件⑥（回帰ゲート緑＝e2e含む）未達→Phase 3 進入 HOLD 継続**。**実装なし・E2E/UI/seed修正なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/ci.yml/package.json/lockfile変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。判定 **CI_STAGE3_E2E_RED / PLAYWRIGHT_FAIL / HOLD**。
- 記録: doc131（監査）＋ `docs/roadmap/32`（Candidate）。次は doc131 push-only（別承認）または e2e 15件 root-cause 深掘り→修正計画（doc132/roadmap33）。

## CI Stage 3 E2E 実装 — Candidate・ci.yml変更・判定 実装完了（e2e実greenはpush→CIで確認）

- **doc130 / `docs/roadmap/31_ci_stage3_e2e_implementation_candidate.md`＋`.github/workflows/ci.yml`**（2026-07-07）。doc129/roadmap30 の承認済み設計に従い `ci.yml` に **stage3_e2e** ジョブを実装（`stage1` は不変・9 steps 保持）。**事前停止条件を read-only で解消**: (a) db script の `dotenv -e ../../.env` → CI 専用 `.env`（非機密・gitignore 済・未コミット）で `DATABASE_URL`/`DIRECT_URL` を ephemeral service に注入 (b) `prisma/seed.ts`(758行) は `source:'DEMO'`/`DemoMapProvider`/`fakeWebsiteAnalysis` のみで fetch/API_KEY/OPENAI/ANTHROPIC なし＝外部/secrets 依存なし (c) web は Redis/S3 起動非依存・`SESSION_SECRET` フォールバックあり＝ephemeral Postgres のみで起動可。**stage3_e2e**（needs stage1・timeout 30分）: service=`pgvector/pgvector:pg16`（本番DB非接続・app/app/ikezaki_os・health `pg_isready`）→ install→CI `.env` 生成→`db:generate`→`db:migrate:deploy`→`db:seed`→`pnpm build`→`playwright install --with-deps chromium`→`pnpm --filter @hokko/web exec playwright test`（12 spec）→失敗時 `apps/web/playwright-report` を artifact。**封印維持**: env `EXTERNAL_SEND_ENABLED=false`（`LogEmailProvider`・送信ゼロ）・`LLM_PROVIDER=fake`（`FakeLLM`・実LLMなし・AIコストなし）・`externalAiAllowed` 既定false・runtime 解禁なし・secrets 不要・本番非接触。**ローカル検証 green**: YAML パース OK（jobs=stage1/stage3_e2e）・安全env値・stage1 不変・実 secrets なし・lockfile 不変（4eac5c8b…）・`node scripts/check-company-brain-safety.mjs` exit 0。**e2e の実 green は本サンドボックスでは実走不能（Postgres/Actions なし）→ push 後の CI 実行で確認**。roadmap31 に現在地10項目（§9-1〜9-10）・50カテゴリ Matrix（§8・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§10）を記録。**判定＝CI 実装完了（ローカル検証 green）／Phase 3 進入は HOLD（残＝e2e 実 green＋最終 Phase Gate 承認）**。**schema変更なし・新規 migration作成なし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals/app コード変更なし・外部送信/実LLM/AIコスト/本番/本番DB接続 なし・369-vault非編集・push なし（commit-only）**。判定 **実装完了・e2e実greenはpush→CIで確認**。
- 記録: doc130（監査）＋ `docs/roadmap/31`（Candidate）＋ `.github/workflows/ci.yml`（stage3_e2e 追加）。次は ci.yml＋roadmap31 の push（別承認・push で CI stage3_e2e 実行）→ e2e 結果確認 → doc131。

## CI Stage 3 E2E 追加 設計 — Candidate・docs-only・判定 HOLD（設計のみ・実装別承認）

- **doc129 / `docs/roadmap/30_ci_stage3_e2e_design_candidate.md`**（2026-07-07）。CI 確認で判明した **CI_E2E_NOT_CONFIGURED**（run 28859371179=success・`stage1` の safety/test/typecheck/lint green・e2e/build は `ci.yml` で「Stage 2/3 は別承認」＝CI 未組込）を受け、Phase 3 残条件「e2e green 確認」を満たすため CI に **Stage 3 E2E（playwright・12 spec）** を安全に足す設計を docs-only で整理。**設計要点**: CI に別ジョブ `stage3_e2e` 追加（`needs: stage1` 直列 or 別ワークフロー）／**ephemeral Postgres service container（本番DB非接続・使い捨て）** に既存 migration を `db:migrate:deploy` 適用＋`db:seed`（デモ値）→ `pnpm build` → `pnpm start`（playwright webServer）→ `test:e2e` 実行／失敗時 trace/report を artifact 保存。**封印維持**: `EXTERNAL_SEND_ENABLED` 既定OFF・`LogEmailProvider`（実送信ゼロ）・`FakeLLM`（実LLMなし・AIコストなし）・`externalAiAllowed` 既定false・runtime 解禁なし・secrets 不要・`decideApprovalAction`＋`SuppressionList`＋`assertCanViewConfidential`＋`DataAccessLog` の封印は e2e 実行中も不変。**事前停止条件**: (a) db script が `dotenv -e ../../.env` を要求→CI で DATABASE_URL 注入方式を先に確定 (b) `prisma/seed.ts` の外部/secrets 依存有無を事前確認 (c) 本番DB/本番URL/実送信/実LLM/課金の兆候があれば停止 (d) `.github` 実変更・E2E 実走は人間 Phase Gate 承認後の別ミッション。roadmap30 に現在地10項目（§12-1〜12-10）・50カテゴリ Matrix（§11・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§13）を記録。**判定＝HOLD（設計 docs-only 完了・実装は別 Phase Gate 承認）**。**実装なし・.github 実変更なし・schema変更なし・新規 migration作成なし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals 変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・本番DB非接続・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。判定 **HOLD（設計のみ）**。
- 記録: doc129（監査）＋ `docs/roadmap/30`（Candidate）。次は doc129 push-only（別承認）または 本設計の人間承認 → CI Stage 3 E2E 実装（別ミッション）。

## Phase 3 Gate 移行判断の確定 — Candidate・docs-only・判定 方針GO / Phase3 HOLD

- **doc128 / `docs/roadmap/29_phase3_gate_decision_confirmation_candidate.md`**（2026-07-07）。roadmap28/doc127 の6論点を人間が「推奨どおり承認」＋回帰ゲートを実測。**§0 人間決定10項目**（VIEW_LIST_ROW_LEVEL_POLICY=DEFER_KEEP_NO_RAW_PII_NOW／LEAD_PII_VIEW_POLICY=KEEP_CURRENT_ADD_DATAACCESSLOG_LATER／CONTACT_VIEW_POLICY=PARENT_CUSTOMER_LABEL_SUBORDINATE／OUTREACH_SEND_POLICY=OPT_OUT_SUPPRESSION_ENFORCED_APPROVED／POSITIVE_CONSENT_POLICY=PURPOSE_SEPARATED_NOT_MANDATORY_FOR_OUTREACH／REGRESSION_GATE_POLICY=REQUIRED_GREEN_BEFORE_PHASE3／E2E_VERIFICATION_POLICY=VERIFY_VIA_CI_RESULT／HIGH_CONFIDENTIAL_RUNTIME_POLICY=NO_RUNTIME_ENABLEMENT_NOW／PHASE3_ENTRY_POLICY=HOLD_UNTIL_E2E_CI_GREEN_AND_PHASE_GATE／IMPLEMENTATION_POLICY=DOCS_ONLY_NOW）。**回帰ゲート実測（`pnpm install --frozen-lockfile`・lockfile不変 4eac5c8b…・main 6f4e16e）＝PARTIAL_GREEN**: safety=exit0／test=vitest 26ファイル・265 passed／typecheck=db/web/worker tsc 通過／lint=eslint exit0／build=next build exit0／e2e=ENV_BLOCKED（DATABASE_URL未設定・seed済DB/起動サーバー不在＝コード欠陥ではない）。RED=0。e2e は `assertCanViewConfidential`/`SuppressionList`/`decideApprovalAction`/`EXTERNAL_SEND_ENABLED`/`LogEmailProvider`/`FakeLLM`/`externalAiAllowed` 等の封印を維持したまま CI 実績で確認する方針。**6論点確定**: Customer一覧＝据え置き（生PII列不追加）／LocalBusinessLead・LocalBusinessContact＝当面現状維持・後日 DataAccessLog 一体化／Contact 単体＝親 Customer ラベル従属／outreach opt-out（SuppressionList 強制）＝正式承認／positive Consent（ConsentRecord）＝用途別分離／回帰ゲート green＝Phase 3 前必須（e2e は CI）。roadmap29 に §0 10項目・現在地10項目（§13-1〜13-10）・50カテゴリ Matrix（§12・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§18）を記録。**判定＝方針決定 GO／Phase 3 進入は残条件2点（e2e CI green 確認＋最終 Phase Gate 承認）まで HOLD**。**実装なし・runtime 解禁なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals 変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。判定 **方針GO / Phase3 HOLD**。
- 記録: doc128（監査）＋ `docs/roadmap/29`（Candidate）。次は doc128 push-only（別承認）または CI の e2e 結果確認 → doc129。

## Phase 3 Gate 人間判断チェックリスト — Candidate・docs-only・判定 B: HOLD

- **doc127 / `docs/roadmap/28_phase3_gate_human_decision_checklist_candidate.md`**（2026-07-07）。doc126/roadmap27 の設計判断を受け、Phase 3 AI Growth Engine 進入可否を人間が決めるための Phase Gate チェックリストを docs-only で整理。**Gate 残件6条件**（①Phase 2完了正式記録②高機密ラベル runtime 統制＝Customer 詳細は `assertCanViewConfidential`/`canAccessLabel`/`writeDataAccess`(`DataAccessLog`/`confidential_view`) で閉・一覧/lead/Contact 未接続③外部送信 Human Certification Gate＝`decideApprovalAction` が `SuppressionList` 強制・`executeApprovedAction` 二重実行防止・`EXTERNAL_SEND_ENABLED` 既定OFF・`LogEmailProvider`＝閉④Consent・SuppressionList＝Suppression 送信ゲート強制/positive Consent 用途別⑤AI境界＝NORMAL/INTERNAL のみ・`FakeLLM`・`externalAiAllowed` 既定false＝閉⑥回帰ゲート green 未実測）。**人間判断6論点（推奨）**: Customer一覧＝据え置き（生PII列不追加）／LocalBusinessLead/LocalBusinessContact＝当面現状維持・後日 DataAccessLog 一体化／Contact 単体＝親 Customer ラベル従属／outreach opt-out（SuppressionList 強制）＝正式承認可／positive Consent（ConsentRecord）＝用途別分離／回帰ゲート green＝Phase 3 前必須条件化。**GO条件**＝①〜⑥充足＋人間 Phase Gate 承認／**HOLD条件**＝②方針未決定 or ⑥未実測 or 承認未取得。格上げ選択時は schema/RBAC 変更の要否を事前停止条件として別承認化。roadmap28 に現在地10項目（§13-1〜13-10）・50カテゴリ Matrix（§14・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§19）を記録。**Phase 3 進入は引き続き HOLD**。**実装なし・runtime 解禁なし・schema変更なし・migrationなし・RBAC変更なし・labels変更なし・company-brain-reference変更なし・leadmap/customers/approvals 変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。判定 **B: HOLD**（判断材料は整備済み）。
- 記録: doc127（監査）＋ `docs/roadmap/28`（Candidate）。次は doc127 push-only（別承認）または 人間の6論点決定・回帰ゲート green 確認。

## Phase 3前残件の設計判断メモ — Candidate・docs-only・判定 B: HOLD

- **doc126 / `docs/roadmap/27_phase3_gate_remaining_controls_policy_candidate.md`**（2026-07-07）。doc125（判定 B: HOLD）＋外部送信・Consent・AI境界の read-only 追加監査を受け、Phase 3 前残件の設計判断を docs-only で正本化。**確認済み（前回未確証を解消）**: 外部送信の実ゲート `decideApprovalAction`（approvals/actions.ts）は承認時に `SuppressionList` を照会し `isSuppressed` 該当なら `suppressed` として**送信せず**ログのみ・lead stage EXCLUDED・usage 未計上／outreach UI も suppressed 時は承認ボタン disabled（多重防御）／実送信は `EXTERNAL_SEND_ENABLED`==='true' の場合のみ・既定は `LogEmailProvider`（ネットワーク送信なし）／二重実行防止は `executeApprovedAction`（原子的クレーム）／AIは `assertAiToolAllowed` で external_send 直接実行不可／AI参照は NORMAL/INTERNAL のみ・`FakeLLM` 決定論・`externalAiAllowed` 既定 false 固定。Customer 詳細は `assertCanViewConfidential`＋`canAccessLabel`＋`writeDataAccess`（`DataAccessLog`/`confidential_view`）で閉。**設計判断（人間 Phase Gate 承認事項）**: ①Customer一覧の行レベル機密統制＝据え置きを暫定既定（生PII列を足さない不変条件・高機密運用開始時に格上げ）②LocalBusinessLead/LocalBusinessContact のPII閲覧＝テナント分離＋leadmap 権限＋外部送信ゲートで当面担保③Contact 単体＝親 Customer のラベル従属④outreach＝opt-out（SuppressionList 強制）を正式方針として記録可⑤positive Consent（ConsentRecord）＝用途別分離を暫定既定。roadmap27 に現在地10項目（§12-1〜12-10）・50カテゴリ Matrix（§13・直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules（§18）を記録。**Phase 3前残件の設計判断を整理したが、Phase 3進入はHOLD継続。次は人間 Phase Gate 承認判断または回帰ゲート green 確認。****実装なし・runtime 解禁なし・Customer Pain実装なし・DB/schema/migration/RBAC/label定義変更なし・AI参照条件変更なし・company-brain-reference変更なし・externalAiAllowed true 解禁なし・EXTERNAL_SEND_ENABLED true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 `node scripts/check-company-brain-safety.mjs` exit 0。判定 **B: HOLD**（Phase 3 Gate 材料は前進）。
- 記録: doc126（監査）＋ `docs/roadmap/27`（Candidate）。次は doc126 push-only（別承認）または 人間 Phase Gate 承認判断・回帰ゲート green 確認。

## CRM Customer / Contact 閲覧統制の追加監査 — Candidate・docs-only・判定 B: HOLD

- **doc125 / `docs/roadmap/26_customer_contact_confidential_view_control_audit_candidate.md`**（2026-07-06）。doc124（判定 B: HOLD）の未確認点「CRM Customer/Contact/LocalBusinessLead の人間UI閲覧経路で canAccessLabel＋writeDataAccess(confidential_view) 配線があるか」を追加 read-only 監査。**重要訂正**: doc124 は `canAccessLabel` 直 grep で company-brain-reference.ts のみと捉えたが、実際は **ABAC 統合層 `assertCanViewConfidential`（apps/web/lib/security/policy.ts）経由**で配線。**Customer 詳細（customers/[id]/page.tsx:30）は `assertCanViewConfidential(user,{label: customer.label})` を実行**＝packages/shared/src/policy.ts:156 の `canAccessLabel` 判定＋`DataAccessLog`(confidential_view・db.ts:40/62・audit.ts:10) を記録（admin/data-access-logs・admin/policy-decisions で可視化）。meetings/[id]・finance・invoices・invoices/[id] も同型 → **Customer 詳細閲覧の高機密ラベル runtime 統制は閉じている**（doc124 の未確認を前進）。**残るズレ**: Customer 一覧(customers/page.tsx)は tenantId スコープのみで行ごとの assert なし（LabelBadge 表示のみ）・Contact 単体閲覧経路の統制未確認・LocalBusinessContact/LocalBusinessLead は `/leadmap`（leadmap 権限系・別体系・PII保持）・doc110 標準閲覧式(customer-pain-access.ts の canViewCustomerPainDetail = tenantId × knowledge:update × canAccessLabel × isHumanUser × archivedAt null)は apps 未接続（Customer Pain 本実装未着手＝仕様どおり）。**明確な無統制経路は発見せず（C ではない）**。安全側で **判定 B: HOLD**（Customer 詳細は閉・一覧/Contact 未確認・doc110 未接続）。**Phase 3 進入は引き続き HOLD・人間 Phase Gate 承認事項**。次に必要な補強=Customer 一覧の行レベル機密表示の有無・Contact 単体経路・LocalBusinessLead の高機密ラベル一体化の追加監査、doc110 標準閲覧式の実接続方針の確定（別承認）。50カテゴリ Matrix（直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules 記録。**runtime 解禁なし・実装なし・schema変更なし・migrationなし・RBAC変更なし・canAccessLabel/writeDataAccess 実装なし・company-brain-reference/leadmap/customers 変更なし・externalAiAllowed true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **B: HOLD**。次は doc125 push-only（別承認）または Customer 一覧・Contact 単体閲覧統制の追加監査。

## 高機密ラベル runtime 統制の現状監査 — Candidate・docs-only・判定 B: HOLD

- **doc124 / `docs/roadmap/25_high_confidential_runtime_control_audit_candidate.md`**（2026-07-06）。Phase 3 移行残件②の read-only 監査。doc108〜114（高機密ラベル設計・標準閲覧式・runtime 未解禁）と実コードのズレを整理。**確認できたこと**: AI参照経路（Company Brain・`company-brain-reference.ts`）は `AI_READABLE_LABELS=['NORMAL','INTERNAL']`(:33) の DB where フィルタ＋`canAccessLabel`(labels.ts:45) の二重ガードで **CUSTOMER_CONFIDENTIAL・顧客PII を AI に注入しない**（外部LLM時 externalAiAllowed=true＋maskText・CaseStudy anonymized=true＋private のみ）＝**封印は閉じている**。`writeDataAccess`(db.ts:62)/`DataAccessLog`(schema:331)/ABAC(security/policy.ts の confidential_view/ai_reference/export) 実装済み。`customer-pain-access.ts`（doc110 標準閲覧式＝tenantId × knowledge:update × canAccessLabel × isHumanUser × archivedAt null）は純粋関数存在。**未確認/ズレ**: `Customer`(既定 CUSTOMER_CONFIDENTIAL schema:506)/`Contact`/`LocalBusinessLead`(PII保持) の**人間 UI 閲覧経路で canAccessLabel＋writeDataAccess(confidential_view) 配線が確認できず**（canAccessLabel の利用は company-brain-reference.ts のみ）・doc110 標準閲覧式は apps 未接続・runtime 解禁は doc112 停止条件のまま未解禁。安全側で **判定 B: HOLD**（AI参照封印は GO 相当だが人間閲覧側の統制配線が未確認）。**Phase 3 進入は引き続き HOLD・人間 Phase Gate 承認事項**。次に必要な補強=CRM Customer/Contact 閲覧経路の canAccessLabel/writeDataAccess 配線の追加 read-only 監査、doc110 標準閲覧式の実接続方針の確定（別承認）。50カテゴリ Matrix（直接C03/C06/C08/C37/C38/C39/C46）・Global AI Rules 記録。**runtime 解禁なし・実装なし・schema変更なし・migrationなし・RBAC変更なし・company-brain-reference変更なし・externalAiAllowed true 解禁なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **B: HOLD**。次は doc124 push-only（別承認）または CRM Customer/Contact 閲覧統制の追加監査。

## Phase 2 正式完了記録 — Candidate・docs-only・READY / GO（docs整理）／Phase 2 CONDITIONAL COMPLETE／Phase 3 進入 HOLD

- **doc123 / `docs/roadmap/24_phase2_formal_completion_record_candidate.md`**（2026-07-06）。doc48（Phase 2-A 完了）・doc121（CRM/SFA 既存実装 案A）・doc122/roadmap23（移行条件）を受け Phase 2 正式完了記録を整理。Phase 2 構成＝(a) Company Brain foundation（Phase 2-A 正式完了・doc48/`85f1bf3`・本番GO）＋(b) CRM/SFA（LeadMap/Customer/Deal/Account・既存実装 doc121案A）。**判定=CONDITIONAL COMPLETE**（CRM/SFA の本番確認GO・回帰ゲート（E2E smoke）green の正式記録が未整備・高機密ラベル runtime 統制は設計のみ（doc108〜114・Customer 既定 CUSTOMER_CONFIDENTIAL）・CRM 不足補強 archivedAt/internalNote 未着手 → **Phase 2 正式完了宣言は人間 Phase Gate 承認事項**）。**Phase 3 AI Growth Engine 進入は HOLD**。残件6条件=①Phase 2完了正式記録②高機密ラベル runtime 統制監査③外部送信 Human Certification Gate 運用確認④Consent・SuppressionList 運用確認⑤AI境界再確認⑥回帰ゲート green 確認。GO＝①〜⑥充足＋人間 Phase Gate 承認。roadmap24 は §1..§25 連番＋§12-1〜12-10 現在地10項目明示見出し＋§13 50カテゴリ Matrix 全行（直接C08/C09/C46・Phase 3予告C18・間接C01/C03-07/C10-12/C15/C20/C26/C28/C30/C33/C34/C37-40/C48・後続24・禁止C45）＋§18 Global AI Rules。**実装なし・schema変更なし・migrationなし・UI変更なし・RBAC変更なし・crm:*新設なし・company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO（docs整理）／Phase 2 CONDITIONAL COMPLETE／Phase 3 HOLD**。次は doc123 push-only（別承認）または高機密ラベル runtime 統制の現状監査 docs-only。

## Phase 2 完了判定・Phase 3 移行条件整理 — Candidate・docs-only・READY / GO（docs整理）／Phase 3 進入 HOLD

- **doc122 / `docs/roadmap/23_phase2_completion_and_phase3_gate_candidate.md`**（2026-07-06）。doc117〜121・roadmap18〜22・既存 LeadMap/CRM 実測を前提に、Phase 2 を完了扱いにできるか・Phase 3 AI Growth Engine へ進んでよいかを整理。**現在地の正**: Phase 1 正式完了（doc24/`e95f887`）・Phase X 完了（doc32/`70d4d06`）・**Phase 2-A 正式完了**（doc48/`85f1bf3`＝Company Brain foundation 本番GO）・**CRM/SFA 既存実装済み**（doc121案A＝LocalBusinessLead/Customer/Deal/`/leadmap`/actions.ts）。**Phase 2＝部分完了**（CRM/SFA の Phase 2 完了正式記録が未作成・高機密ラベル runtime 統制は設計のみ・不足補強 archivedAt/internalNote 未着手）。**Phase 3 進入＝HOLD 推奨**（Phase 3 は外部発信＝広告/PR/SEO/MA/メールを含み、Human Certification Gate/Consent/SuppressionList/高機密ラベル統制の運用確認が前提）。移行条件6項目=①Phase 2完了記録②高機密ラベル runtime 統制監査③外部送信 Human Certification Gate 運用確認④Consent・SuppressionList 運用確認⑤AI境界再確認（AIロール mutation禁止/外部送信なし/実LLM・AIコスト承認制）⑥回帰ゲート（E2E smoke）green 確認。GO条件＝①〜⑥充足＋人間 Phase Gate 承認。Phase 3前禁止=Phase 2完了記録なしの Phase 3実装/外部送信/広告自動/SNS/PR/SEO/LINE・DM/実LLM/AIコスト/高機密ラベル runtime 解禁/新規Leadモデル/`/crm/leads`/crm:*新設/schema/migration。**判定=READY / GO（docs整理）／Phase 3 進入は HOLD（人間 Phase Gate 承認事項）**。**実装なし・schema変更なし・migrationなし・UI変更なし・RBAC変更なし・crm:*新設なし・company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。次は doc122 push-only（別承認）または Phase 2 正式完了記録の作成 docs-only。

## 既存LeadMap/CRM実装の追認・差分監査 — Candidate・docs-only・READY / GO（案A）

- **doc121 / `docs/roadmap/22_crm_leadmap_existing_implementation_reconciliation_candidate.md`**（2026-07-06）。doc121 実装スプリント着手ミッションで、既存リポジトリに **LeadMap/CRM が既に実装済み**であることが判明し**実装せず停止**、read-only 監査で整理した回。既存: `LocalBusinessLead`（実リード・tenantId・name・address?/phone?/email?・placeId?・source(DataSource)・fetchedAt?/expiresAt?・stage `LeadStage @default(NEW)`・ownerId?・customerId?/dealId?・campaign/insights/scores/outreachDrafts/pipelineHistory）・`LocalBusinessContact`（Contact PII）・`Customer`（label 既定 CUSTOMER_CONFIDENTIAL・PII・contacts/deals/complaints/invoices）・`Contact`・`Deal`（DealStage・amount・source manual|leadmap・leadId?・stageHistory/quotes/proposals）・`Account`・enum `LeadStage`(16値)・`/leadmap/*` UI(leads/campaigns/map/pipeline/routes/settings/actions.ts)・rbac `leadmap`/`leadmap:external_send`。安全機構: requireUser/hasPermission('leadmap')/tenantId/writeAudit/OutreachApproval(PENDING=人間承認)。**判定=案A: 既存 `LocalBusinessLead`/`/leadmap` を CRM Lead 正本候補として追認・新規 `Lead` モデル/`/crm/leads` は作らない・doc118〜120 は削除せず greenfield 前提 Candidate として温存**。doc118〜120 とのズレ: Lead実体は既存/Contact PII は保持設計/LeadStage16値 vs doc5値/専用 leadmap 権限が既存で crm:*・knowledge 論点解消/UI 既存/CRM 配線済み/Customer 既定 CUSTOMER_CONFIDENTIAL は doc108〜114 高機密ラベルに接続。不足補強候補（別承認）: `LocalBusinessLead` に archivedAt ソフトアーカイブ・internalNote が無い。**実装なし・schema変更なし・migrationなし・UI変更なし・Server Action変更なし・RBAC変更なし・crm:*新設なし・Contact PII追加なし・company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO（案A）**。次は doc121 push-only（別承認）または既存 LocalBusinessLead 薄い補強の実装可否判断。

## CRM Lead-only 実装スプリント可否 最終判断 — Candidate・docs-only・READY / GO

- **doc120 / `docs/roadmap/21_crm_lead_only_implementation_gate_candidate.md`**（2026-07-06）。doc118/doc119 を前提に、CRM Lead-only を次の実装スプリントへ進める条件が揃っているかを最終判断。**判定 READY / GO＝「条件は揃っている」であり、実装着手そのものは §30 の別承認文言に基づく別の重い人間承認**。判断項目（範囲がLead 1テーブルに収まるか・Contact PII なしで価値が出るか・権限は既存で暫定可能か・schema/migration が1テーブル1本に収まるか・安全境界を最初から組めるか・本番/外部送信/実LLMなしで閉じられるか）。schema要否=実装するなら Lead テーブル追加1・migration要否=1本（**本書ではしない**）。Lead候補列/status enum（NEW/REVIEWING/QUALIFIED/EXCLUDED/ARCHIVED）の最終確認。tenantId 必須・RBAC は hasPermission・**AIロール mutation 一律拒否**・writeAudit=作成/編集/アーカイブ/status遷移/担当変更。**writeDataAccess は Lead-only では原則不要（Contact PII/CUSTOMER_CONFIDENTIAL 段階で必須＝停止条件）**。Data Classification=NORMAL/INTERNAL のみ。**Contact PII非採用・company-brain-reference非変更・AI下書きのみ・外部送信なし**。`crm:*` 新設は妥当だが **RBAC変更は別承認**、初期は **既存 knowledge 権限で暫定を推奨**。Human Certification Gate/Consent Gate/Security Gate・Business Event Ledger 将来接続。実装案 案X（今すぐ着手）/案Y（もう1段docs）/案Z（別Lineage先行）。**推奨=案X を次の別承認で実行**（Lead 1テーブル＋最小CRUD＋一覧・既存権限暫定・schema1＋migration1・本番確認は別承認）。§30 に別承認文言（案）を記載。50カテゴリ（直接C08/C09/C46）・20大/追加19/5本柱（Human Certification Gate・Business Event Ledger を直接扱う）・Phase対応（事業Phase 2/PDF 2.5/Ledger R4+R0）。**今回は実装しない・schema変更なし・migrationなし・RBAC変更なし・crm:*新設なし・company-brain-reference変更なし・Contact PII保存なし・外部連携/OAuth/外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO（条件は揃っている・実装着手は別承認）**。次は doc120 push-only（別承認）または実装スプリント着手（§30 別承認）。

## CRM Lead-only 最小設計 — Candidate・docs-only・READY / GO

- **doc119 / `docs/roadmap/20_crm_lead_only_minimum_design_candidate.md`**（2026-07-06）。doc118 推奨案A（Lead-only）を、実装判断できる粒度まで最小設計。Lead 候補列（tenantId・displayName=会社名/拠点名レベル・source・placeId・fetchedAt・expiresAt・status・assignedUserId=userId参照のみ・internalNote=PII非依存・archivedAt・監査時刻/操作者）を設計案として提示（**schema化・migrationはしない**）。**Contact PII（氏名/電話/メール/住所）は初期範囲に入れない**（必要になったら停止・別承認／Security Gate負荷最小化・可逆性・Consent要件回避が理由）。status enum 候補=NEW/REVIEWING/QUALIFIED/EXCLUDED/ARCHIVED。物理削除なし・archivedAt ソフトアーカイブ。tenantId 必須スコープ。RBAC は `hasPermission`・AIロール mutation 不可。**`crm:*` 新設 vs 既存 knowledge 権限代替を比較し、本書では RBAC を変更しない**（実装時に別承認）。writeAudit=作成/編集/アーカイブ/status遷移/担当変更。**writeDataAccess は Lead-only では原則不要**（Contact PII/CUSTOMER_CONFIDENTIAL 段階で必須＝停止条件）。Data Classification=NORMAL/INTERNAL のみ。company-brain-reference 非注入・AIは下書き/提案のみ・外部送信なし。Human Certification Gate/Consent Gate/Security Gate・Business Event Ledger 将来接続・3案関係・Phase対応（事業Phase 2/PDF Phase 2.5）・50カテゴリ（直接C08/C09/C46）・20大/追加19/5本柱・初期MVP非対象・実装に進む停止条件・推奨判断（Lead-only を PII なし・既存権限暫定・writeAudit のみ・外部送信なしの最小範囲で実装候補／実装は別承認）を記録。**今回は実装しない・schema変更なし・migrationなし・画面/Server Actionなし・RBAC変更なし・company-brain-reference変更なし・外部連携/OAuth/外部送信/実LLM/AIコスト/本番なし・Contact PII保存なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO**。次は doc119 push-only（別承認）または実装スプリント可否の最終判断。

## CRM最小縦切り 実装可否判断 — Candidate・docs-only・READY / GO

- **doc118 / `docs/roadmap/19_crm_minimum_vertical_slice_candidate.md`**（2026-07-06）。doc117 CRM/SFA Lineage を受け、Phase 2 CRM/SFA の最初の実装範囲を3案比較。**案A Lead-only**（Lead最小CRUD・tenantId/RBAC/writeAudit中心・Contact PII非保存・営業デモ可）／**案B Lead+Deal**（金額/確度/フェーズ・金額はINTERNAL）／**案C Lead+Deal+Pipeline**（ステージ遷移/活動履歴・PII混入リスク最大でSecurity Gate負荷最大）。各案を目的・入れる/入れないデータ・schema要否・migration要否・PIIリスク・tenantId/RBAC/writeAudit/writeDataAccess・Human Certification Gate/Consent Gate/Security Gate・Company Brain接続・AI参照・外部送信・Phase対応・収益/moat・停止条件で比較。**推奨=案A（Lead-only）**（Contact PII最小化・過剰設計回避・薄い縦切り優先・次段停止条件を設計しやすい）。次段順序=案A→停止条件クリア後に案B→別承認で案C。Complete Function Coverage（直接C08/C09/C46・間接21カテゴリ・後続25カテゴリ・禁止C45）・4軸Phase（事業Phase 2 主対象/PDF 2.5-5/戦略20-26将来/Ledger R4+R0）・5本柱（Human Certification Gate/Business Event Ledger 将来接続）。**今回は実装しない・推奨案Aも実装は別承認・schema変更なし・migrationなし・画面/Server Actionなし・外部SaaS連携なし・OAuthなし・外部送信なし・実LLMなし・AIコストなし・本番確認なし・Contact PIIは初期範囲に入れない・company-brain-reference 非注入・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO**。次は doc118 push-only（別承認）または 案A Lead-only 最小設計 docs-only。

## CRM / SFA / Salesforce Mini Lineage — Candidate・docs-only・READY / GO

- **doc117 / `docs/roadmap/18_crm_sfa_salesforce_lineage_candidate.md`**（2026-07-06）。doc116 SaaSカタログ（`docs/roadmap/17`）区分①の最初の個別深掘りとして、369が内包・代替・連携するCRM/SFA/Salesforce Mini 領域を整理。**内包・代替機能候補**: Lead / Account / Contact / Deal / Opportunity / Pipeline / Activity / Task / Note / Customer 360 / Sales Playbook / Quote・Contract・Invoice handoff（物理削除なし・ソフトアーカイブ・tenantId必須）。**外部連携候補**（区分②・例示・契約判断なし）: CRM/SFA（Salesforce/HubSpot/kintone 等）・MA/広告・メール/カレンダー・チャット/通知・名刺/企業データ。**AI境界**: AI営業提案は下書きのみ（OutreachDraft）・外部送信は Human Certification Gate 必須（承認後 decideApprovalAction・isSuppressed 確認）・**AI単独の契約/値引き/請求/送信は禁止**。**権限/監査/機密**: tenantId スコープ・RBAC（hasPermission）・writeAudit・writeDataAccess・Data Classification（顧客PIIは高機密ラベル前提・runtime解禁は別承認・company-brain-reference 非注入）。**接続候補**: Company Brain（Playbook/方針参照）・AI Growth Engine（獲得ループ）・CPQ/Contract/Invoice/ERP は handoff のみ。**境界**: Service Cloud（CS）/Marketing Cloud（MA）/Data Cloud（BI/CDP）は別 Lineage。Phase対応（事業0-20 は Lead→Deal→Pipeline の薄い縦切り優先）・収益ポイント・moat・やらないこと・承認ゲートを記録。**Candidate（実装/契約/API連携/DB化/schema化/正式採用ではない）・schema/画面/外部連携は各々別承認**。**実装なし・schema/migration/RBAC/label定義/company-brain-reference変更なし・外部SaaS連携実装なし・OAuthなし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO**。**Phase対応を補強済み**（`docs/roadmap/18 §17-1〜17-5`＋`docs/audit/117 §3-1`: 主対象=事業Phase 2 Salesforce Mini/CRM基盤・前提=Phase 0 Core OS/Phase 1 Company Brain・隣接=Phase 3 AI Growth Engine/Phase 4 Human Certification Gate/Phase 5 Oracle Mini・ERP/Phase 12 Data Cloud・BI/Phase 13 Service Cloud・CS/Phase 14 Marketing Cloud・PR・SEO/Phase 19 Enterprise Governance・PDF系 OS本体 Phase 2.5/4/5/7/8/9-10・戦略 Phase 20/22/23/24/26 は将来接続候補で今回実装しない）。次は doc117＋Phase補強 push-only（別承認）または CRM 最小縦切り実装可否判断。

## 369組み込み予定SaaSカタログ — Candidate・docs-only・READY / GO

- **doc116 / `docs/roadmap/17_embedded_saas_catalog_candidate.md`**（2026-07-06）。369 / IKEZAKI OS が内包・代替・外部連携・Marketplace化・将来候補にするSaaS領域を、既存戦略docs（`docs/roadmap/09-16`）の**補完 Candidate カタログ**として整理。6区分（内包・代替 / 外部連携 / Marketplace・Developer Cloud / PLUG・Employee App / Candidate保留 / Do Not Build）。区分①の内包・代替対象SaaSカテゴリ（Core OS/IAM/SSO/RBAC/監査/承認/Trust Center、Company Brain/KB/RAG/Data Classification、CRM/SFA、CPQ/契約、Growth/MA/CDP、Human Certification Gate/Workflow、Finance/会計/請求、ERP/EPM/GRC、Procurement/3-way match、Inventory/SCM、Commerce/EC/POS、Service/CS、HCM/ATS/HRIS/LMS、Payroll/Evaluation=後半候補・AI単独確定禁止、Data Cloud/BI、Integration Hub/iPaaS/MCP、Developer Cloud/Agent SDK/Manifest、AI Employee Studio、Marketplace/Safety Review、Billing/Metering/Revenue Share、PLUG Commerce、369 Employee App、Industry Cloud、Security/Legal Ops、Docs/Audit/Evidence）を網羅。区分②の外部連携対象（Google/MS365、Slack/Teams/LINE、会計/決済/銀行、CRM/MA/広告、EC/POS/Shipping、HR/勤怠/給与、Storage/Wiki、GitHub/CI/監視、SSO/SCIM/SIEM/DLP、Payment/Tax/E-invoice）は**例示に留め契約判断なし**。収益源・moat（IP moat 含む）・禁止と安全代替（AI単独の請求/送金/契約/採用/評価/会計確定禁止・Human Certification Gate）・Phase対応（事業0-20 / OS本体2.5-18 / 戦略18.5-26）を記録。**Candidate（実装/契約/API連携/DB化/正式採用ではない）・1カテゴリずつ別承認・231-252はCandidate扱いのまま**。**実装なし・schema/migration/RBAC/label定義/company-brain-reference変更なし・外部送信/実LLM/AIコスト/本番なし・369-vault非編集・push なし（commit-only）**。安全確認 exit 0。判定 **READY / GO**。次は doc116 push-only（別承認）または 1カテゴリ個別深掘り。

...
承認者は人間だけである。Codex A〜H（B・Hを含む）は独立確認者（independent confirmer）であり、承認・PASS代行・merge判断を代行しない。B/Hの役割はTask Packetと実装を独立に再確認することであって、承認ではない。C・D・E監査はすべての実装Task PacketでREQUIRED（必須）であり、docs-only等の理由でも省略・免除しない。
...
- `PROMPT_MANIFEST.json` は各prompt本文のcontent hash（SHA-256）だけを管理する。prompt本文をGit追跡したcommit SHAはmanifest本文へ書き込まない（循環参照を作らない）。commit SHAとcontent hashの対応付けは、Task Packetと外部Human Approval Event（append-only）が併記して固定する。

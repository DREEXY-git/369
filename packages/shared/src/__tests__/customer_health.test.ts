...
describe('顧客離反予兆の判定（classifyCustomerChurn・根拠を残す）', () => {
  ...
  it('範囲外(0-100外)・非数値の churnRisk / satisfaction は無視して捏造しない（clamp して根拠化しない・F-CHURN-01）', () => {
    // churn 200 / satisfaction -1 は範囲外＝無視（clamp して「危険根拠」に化けさせない）
    const a = classifyCustomerChurn({ churnRisk: 200, satisfaction: -1, daysSinceContact: null, openComplaints: 0 });
    expect(a.score).toBe(0);
    expect(a.tier).toBe('stable');
    expect(a.reasons).toEqual([]);
    // NaN・負のクレームも無視
    const b = classifyCustomerChurn({ churnRisk: Number.NaN, satisfaction: Number.NaN, daysSinceContact: Number.NaN, openComplaints: -3 });
    expect(b).toMatchObject({ score: 0, tier: 'stable', reasons: [] });
  });

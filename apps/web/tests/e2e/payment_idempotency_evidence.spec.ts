...
// 決定論的 barrier: **この holder backend を根とする blocking graph** に属する入金 FOR UPDATE 待ち
// waiter が n 本になるまで pg_blocking_pids で観測する（Codex R5 #2: global な query 文字列 count ではなく
// holder PID へスコープし、別 test/別 request の lock 待ちを誤計上しない）。
// 注: PostgreSQL の行ロック待ちは tuple lock queue を作るため、2 本目の waiter は先頭 waiter に
// block される（pg_blocking_pids は holder を直接含まない）。そのため「holder に直接 block された waiter」
// と「既に数えた waiter に block された waiter」の閉包を数える（依然 holder 起点の一意 scope）。
async function waitForWaitersBlockedBy(holderPid: number, n: number, timeoutMs: number): Promise<void> {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    // 入金側ロック文（`SELECT status FROM "Invoice" ... FOR UPDATE`）で block 中の backend と、その blockers。
    ...
    if (Date.now() > deadline) throw new Error(`barrier: holder(pid=${holderPid}) の blocking graph に属する waiter ${n} 本を ${timeoutMs}ms 以内に観測できなかった`);
...
test('真barrier 閾値越え並行入金: holder取得確定→2 waiterのblocking観測→release後 RECEIVABLE_COLLECTED 1件（Codex R4 #2 / R5 #2）', async () => {
  const t = await tenantId();
  const uid = await ceoUserId();
  const invId = await makeIssuedInvoice(10000);
  try {
    // Codex R5 #2: holder が FOR UPDATE を**取得完了**したことを ready signal（holder backend PID 付き）で
    // 固定してから p1/p2 を起動し、pg_blocking_pids で「この holder に直接 block された 2 waiter」を
    // 観測してから release する（scheduler 依存の先行 commit や global query count の誤計上を排除）。
    let releaseBarrier!: () => void;
    const barrierReleased = new Promise<void>((res) => { releaseBarrier = res; });
    let holderReadyResolve!: (pid: number) => void;
    const holderReady = new Promise<number>((res) => { holderReadyResolve = res; });
    // T0: 対象 invoice 行を FOR UPDATE で保持。**取得後**に自 backend PID を添えて ready を resolve する。
    const holder = prisma.$transaction(
      async (tx) => {
        ...
        holderReadyResolve(pidRows[0]!.pid); // ロック取得完了を PID 付きで通知
        await barrierReleased; // 両 waiter の blocking 観測が済むまでロックを保持する
      },
      { timeout: 30000 },
    );

    // holder のロック取得完了を待ってから、それぞれ単独で閾値（10000）を越える 2 本を起動する
    //（holder 未取得のまま p1 が先に lock 取得・commit する scheduler 依存の抜け道を排除）。
    ...
    // 決定論的 barrier: **この holder PID に直接 block された**入金 waiter が 2 本になったことを観測してから release。
    // 観測が失敗しても holder を長時間保持しないよう、必ず release する（cleanup が lock で詰まるのを防ぐ）。
    try {
      await waitForWaitersBlockedBy(holderPid, 2, 20000);
    } finally {
      releaseBarrier();
    }

...
- **Phase 2-B-5: SalesPlaybookEntry AI参照追加 — 設計確認済み（Phase 2-B-5-ENTRY・判定 READY / GO・記録: doc59）／実装は未着手・別承認待ち**。結論: 2-A-3c-2 で実証済みの AI 参照ヘルパー（company-brain-reference.ts）に3テーブル目として追加する最小差分で実現可能（schema変更・migration 不要・変更は実質1ファイル＋smoke 18本目）。安全境界は既存と同一: read-only・tenantId・archivedAt:null・NORMAL/INTERNAL のみ・canAccessLabel・ai_reference をレコードごと記録・外部LLM時は externalAiAllowed=true＋maskText のみ（全件 false のため構造的にゼロ）。**Phase 2-B-5 の実装には個別人間承認なしに進まない**。
...
1. **Phase 2-B-5-ENTRY 設計記録（doc59）の push（push-only・別承認。feature＋main）**。
2. **Phase 2-B-5 実装（AI参照の最小実装: company-brain-reference.ts への3テーブル目追加＋smoke 18本目）の承認判断**。設計の正は doc59 §5〜§6。
3. 改善候補（任意・別承認）: CI 導入（test/typecheck/lint 自動実行）・安全境界の否定系テスト追加・doc49 の script 化。2-B-5 と独立に実施可能。

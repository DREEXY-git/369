...
export default async function MorningReportPage() {
  ...
  const showShortageAlert = !!shortageProj && (shortageProj.currentlyNegative || shortageProj.result.shortageDate != null || shortageProj.coverageIncomplete || shortageProj.result.minBalance < 1_500_000);
        ...
        <Card className={`mb-4 ${shortageProj.currentlyNegative || shortageProj.result.shortageDate ? 'border-2 border-red-400 bg-red-50/60 dark:bg-red-950/20' : 'border-amber-300 bg-amber-50/50 dark:bg-amber-950/20'}`}>
          <CardHeader>
            <CardTitle className="flex flex-wrap items-center gap-2">
              ...
              {shortageProj.currentlyNegative ? <Badge tone="red">現在マイナス</Badge> : shortageProj.result.shortageDate ? <Badge tone="red">{formatDate(shortageProj.result.shortageDate)} 予測</Badge> : shortageProj.coverageIncomplete ? <Badge tone="amber">判定範囲不足</Badge> : <Badge tone="amber">残高薄め</Badge>}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-sm">
            ...
            ) : shortageProj.coverageIncomplete ? (
              <p>{shortageProj.truncated ? '予定入出金が多く先頭500件までで試算しています。それ以降の予定は未反映のため、' : '一部の予定は内訳を確定できず試算から除外したため、'}<span className="font-bold">「資金ショートなし」とは断定できません</span>。大きな支払い予定がないか資金繰りページでご確認ください。</p>
            ) : (

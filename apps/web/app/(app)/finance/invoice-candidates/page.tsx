...
import { requestInvoiceSendApprovalAction, executeApprovedInvoiceSendAction } from '../bridge/actions';
...
export default async function InvoiceCandidatesPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const { candidates, approvalById } = await getInvoiceCandidateListData(user.tenantId);
  ...
  return (
    <div>
      ...
      {sp.formalized ? <div className="mb-3 rounded-md bg-emerald-50 px-3 py-2 text-sm text-emerald-700">請求候補を正式請求書（Invoice）＋売掛金（Receivable）に変換しました。</div> : null}
                  ...
                  {canRequest ? (
                    <Td>
                      {c.status === 'draft' ? (
                        ...
                        <Link href={`/invoices/${c.invoiceId}`} className="text-xs text-primary hover:underline">請求書を見る</Link>
                      ) : canFinalize ? (
                        <form action={executeApprovedInvoiceSendAction}>
                          <input type="hidden" name="approvalId" value={c.approvalId ?? ''} />
                          <Button type="submit" className="h-7 px-2 text-xs">正式請求書化</Button>
                        </form>
                      ) : (

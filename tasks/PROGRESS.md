...
- 📝 経緯: 初回報告「全てGO」→ GO記録 commit（`6c1b5e9`・feature のみ・main 未反映）→ 追加実測で NG → **main に入る前に HOLD へ訂正**（「矛盾する GO を正史に入れない」が機能）。
- 🔎 原因は未特定・断定しない: 前例=キャッシュ/反映タイミング（doc37→38）・症状の特定が先（ナビ非表示か・画面が開けないか・ハードリロード後どうか）。再確認時点の既存画面・外部送信の状態は追加実測に言及がなく包括断定しない。

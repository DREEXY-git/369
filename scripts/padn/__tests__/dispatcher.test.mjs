...
test('duplicate WIP/lease branch を検出したら fail-closed（Codex R14 P1）', async () => {
  const snap = await snapshotOf(standardWorld());
  snap.duplicateWips = [{ key: 'branch:claude/x', issues: [67, 168] }];
  const r = decide({ snapshot: snap, event: tick, ctx: ctxWith(), configs });
  assert.equal(r.status, 'FROZEN');
  assert.equal(r.decisions.length, 0);
});

test('rework 上限超過の WIP には remediate を dispatch しない（Codex R14 P2）', () => {
  const wip = {
    issueNumber: 80,
    wipId: 'WIP-PADN-X-001',
    state: 'CHANGES_REQUESTED',
    reworkCount: 3, // rework_max=2 を超過
    frozenHead: 'deadbeefcafe00112233445566778899aabbccdd',
    lease: {
      leaseId: 'LEASE-80',
      revision: 1,
    ...
    allowedPaths: ['docs/roadmap/97_x.md'],
  ...
  const snapshot = {
    ...
    control: { programId: '369-PADN-V5', directorEpoch: 1, controlRevision: 7, writeCapacity: 2, activeWriteLanes: 0, backpressure: false, incidentFreeze: false, lastDirectorActivityAt: '2026-07-17T02:51:00Z' },
    mainSha: '7e50a04df6dcc8043689958cbfd9be42e15e1af7',
    prs: [],
    wips: [wip],
    duplicateWips: [],
  };
  const r = decide({ snapshot, event: tick, ctx: ctxWith({ mode: 'rt0_pilot', writeLanesVar: 1 }), configs });
  assert.equal(r.decisions.some((d) => d.event_type === 'padn_claude_remediate'), false);
  // 上限内（reworkCount=2）なら remediate される
  const r2 = decide({ snapshot: { ...snapshot, wips: [{ ...wip, reworkCount: 2 }] }, event: tick, ctx: ctxWith({ mode: 'rt0_pilot', writeLanesVar: 1 }), configs });
  assert.equal(r2.decisions.some((d) => d.event_type === 'padn_claude_remediate'), true);
});
...
test('budget 予約: 同一 tick で残り 1 の場合、read-only 監査も 1 件しか emit しない（Codex R13 P2）', async () => {
  // B2 を FROZEN_FOR_REVIEW に巻き戻す（codex 3 監査が候補）。dispatchesToday = 上限-1。
  const world = standardWorld();
  world.commentsByIssue[68] = world.commentsByIssue[68].slice(0, 5);
  const snap = await snapshotOf(world);
  const r = decide({
    ...
    ctx: ctxWith(),
    configs,
    dispatchesToday: configs.policy.budget.max_role_dispatches_per_day - 1,
  });
  assert.equal(r.decisions.length, 1, '残り 1 なら tick 内で 1 件しか emit しない');
});

test('capacity 予約: 同一 tick で 2 WIP が eligible でも write lane 上限を超えない（Codex R12 P1）', () => {
  const mkWip = (issueNumber, wipId, paths) => ({
    issueNumber,
    wipId,
    state: 'DISPATCHED',
    reworkCount: 0,
  ...
  const snapshot = {
    ...
    prs: [],
    wips: [
      mkWip(70, 'WIP-PADN-R0A-001', ['docs/roadmap/98_a.md']),
      mkWip(71, 'WIP-PADN-R0B-001', ['docs/roadmap/99_b.md']),
    ],
    duplicateWips: [],
  };
  // rt0_pilot + PADN_WRITE_LANES=1: 2 つの RT0 DISPATCHED WIP でも implement は 1 件のみ
  const r = decide({ snapshot, event: tick, ctx: ctxWith({ mode: 'rt0_pilot', writeLanesVar: 1 }), configs });
  const implementDecisions = r.decisions.filter((d) => d.event_type === 'padn_claude_implement');
  assert.equal(implementDecisions.length, 1, '同一 tick の 2 WIP でも write lane 上限 1 を超えない');
});

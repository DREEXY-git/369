...
describe('紹介元ランキング（summarizeReferrersByName・誰が成約に繋がったか）', () => {
  ...
  it('金額・成約・総数が同率でも入力順に依らず一意順（localeCompare 同値の別文字列も決定論・Codex D-REF-02）', () => {
    const a = { referrerName: 'A', status: 'won', count: 1, wonValue: 100000 };
    const b = { referrerName: 'Ａ', status: 'won', count: 1, wonValue: 100000 }; // 全角Ａ（localeCompare('ja') では 0 になり得る）
    const forward = summarizeReferrersByName([a, b]).map((r) => r.referrerName);
    const reversed = summarizeReferrersByName([b, a]).map((r) => r.referrerName);
    expect(forward).toHaveLength(2);
    expect(forward).toEqual(reversed); // 入力順を反転しても出力順が完全一致＝一意
  });
});

describe('紹介の「要フォロー」検知（isReferralStale・放置を取りこぼさない）', () => {
  const now = new Date('2026-07-22T00:00:00Z');
  const daysAgo = (d: number) => new Date(now.getTime() - d * 24 * 60 * 60 * 1000);

  it('未決着（受領/商談中）でしきい値を超えて動きが無ければ要フォロー', () => {
    expect(isReferralStale('received', daysAgo(15), now)).toBe(true);
    expect(isReferralStale('in_progress', daysAgo(30), now)).toBe(true);
    // しきい値ちょうど・未満は放置に含めない（境界は安全側）
    expect(isReferralStale('received', daysAgo(REFERRAL_STALE_DAYS), now)).toBe(false);
    expect(isReferralStale('in_progress', daysAgo(3), now)).toBe(false);
  });

  it('終端（成約/不成立）は放置日数に関わらず常にフォロー不要', () => {
    expect(isReferralStale('won', daysAgo(365), now)).toBe(false);
    expect(isReferralStale('lost', daysAgo(365), now)).toBe(false);
    expect(isReferralStale('unknown', daysAgo(365), now)).toBe(false);
  });

  it('referralStaleBefore は now-しきい値日 の基準日時を返す（DB クエリ用）', () => {
    const before = referralStaleBefore(now);
    expect(before.getTime()).toBe(daysAgo(REFERRAL_STALE_DAYS).getTime());
    expect(referralStaleBefore(now, 7).getTime()).toBe(daysAgo(7).getTime());
  });
});

...
| `C04-003` | `PARTIAL_LOCAL` | AI社員8名の静的プロフィール（氏名・役割・性格・スキル・注意点）をread-only表示 | PR #9 `af397a52`、`packages/shared/src/ai-characters.ts`、CI #206 309 unit / 103 E2E | 永続プロフィール、編集、版管理、実績評価ではない。評価文はキャラクター設定 |
...
| `C28-017` | `VERIFIED_REPOSITORY` | AI社員の3D/2D read-onlyダッシュボードと静的プロフィールパネル | PR #5、PR #9 `af397a52`、`/ai-office`、CI #206、artifact `8248339408` | main未統合。desktop初期表示ではプロフィール未選択、CI画像はプロフィール全体を証明しない |
| `C28-036` | `PARTIAL_LOCAL` | AI社員状態・run件数に加え、生成・成功・失敗・承認待ちを証拠区分付きで表示 | PR #5、PR #6 `50259e94`、CI #200 305 unit / 103 E2E | 承認待ち合計にrun/gate二重計上リスク。成果金額・人間削減時間の実測なし |
| `C30-039` | `VERIFIED_REPOSITORY` | `AIAgentRun`に基づく期間内実行回数と成功・失敗件数の表示 | PR #6 `apps/web/lib/domains/ai-workforce/outcomes.ts`、CI #200 | AI社員別給与明細全体、稼働時間、原価ではない |
| `C30-041` | `BLOCKED` | `ApprovalRequest`と`AIApprovalGate`の待ち件数取得経路は存在 | PR #6 `inbox.ts` / `outcomes.ts` | 同一run/gateを二重計上し得る。`AIApprovalGate`を人間が決定するUI/bridgeがない |
| `C30-046` | `PARTIAL_LOCAL` | baselineなしでは削減時間を`unavailable`とし、推定値を表示しない規律とUI | PR #6 `outcome-evidence.ts`、`/ai-office?tab=outcomes`、CI #200 | baseline収集・削減時間算出・継続計測は未実装 |
| `C30-052` | `VERIFIED_REPOSITORY` | 直近`AIAgentRun=FAILED`の件数と対象AI社員をread-only表示 | PR #6 `inbox.ts` / `outcomes.ts`、CI #200 | worker例外握り潰しによりBullMQ failed/retry telemetryは未成立 |
...
| `USR-003` | `VERIFIED_REPOSITORY` | PR #9の3D AI Office、8名のオリジナル人物設定・SVG portrait・3D avatar、CI #206、artifact `8248339408` | main未統合。desktop初期画面はプロフィール未選択、nameplate非clickable、Vercel PreviewはSSO保護 |
| `USR-004` | `PARTIAL_LOCAL` | PR #6のOutcome Ledger、証拠5区分、実行/生成件数、baselineなしを`計測なし`とするUI、CI #200 | 人間baseline・成果金額・自己申告入力なし。承認待ち二重計上と異単位合算helperを修正要 |
...
| `USR-007` | `BLOCKED` | 既知Critical/High 0を出荷Gateとして定義し、Codex固定SHAレビューを実施 | PR #5系譜にHigh 2件（Bearerマスク漏れ、BullMQ例外握り潰し）が未解消。脆弱性0を断定しない |

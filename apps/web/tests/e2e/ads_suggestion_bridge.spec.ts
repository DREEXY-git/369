...
// フォーム冪等キー（deterministic PK）による1件実体化と、成功後の同一キー再送（ブラウザ再送を
// hidden input 差し替えで再現）が AIOutput ごと再生成しないことを証明する。
...
test('generate 経路: フォーム冪等キーで1件実体化・成功後の同一キー再送は AIOutput ごと再生成しない（4モデル最終件数）', async ({ page }) => {
  ...
  // フォーム描画時に発行された冪等キー（= 実体化される suggestion の deterministic PK）。
  const key = await row.locator('input[name="idempotencyKey"]').inputValue();
  expect(key, 'idempotencyKey が cuid 互換形式で発行される').toMatch(/^c[a-z0-9]{20,32}$/i);
  created.push(key);
  ...
  await row.getByTestId('ads-generate-draft').click();
  ...
  // 成功後の同一キー再送（ブラウザ再送/二重 submit 相当）: 再描画で発行された新キーを、hidden input を
  // 元のキーへ差し替えて再現する。fast-path が highlight 付きで戻し、AIOutput を含め全件数が不変。
  await page.goto('/marketing/ads');
  const rowAgain = page.locator('tr').filter({ hasText: campaignName });
  await rowAgain.locator('input[name="idempotencyKey"]').evaluate((el, k) => {
    (el as HTMLInputElement).value = k;
  }, key);
  await rowAgain.getByTestId('ads-generate-draft').click();
  await page.waitForURL(new RegExp(`/marketing/ads\\?generated=1&highlight=${key}`));
  expect(await counts()).toEqual({ aiOut: 1, sug: 1, mat: 1, run: 1, access: 1 });

  // 不正な冪等キー（形式違反）は入力エラーで拒否され、どのモデルも増えない。
  await page.goto('/marketing/ads');
  const rowBad = page.locator('tr').filter({ hasText: campaignName });
  await rowBad.locator('input[name="idempotencyKey"]').evaluate((el) => {
    (el as HTMLInputElement).value = 'not-a-valid-key';
  });
  await rowBad.getByTestId('ads-generate-draft').click();
  await page.waitForURL(/\/marketing\/ads\?error=input/);
  expect(await counts()).toEqual({ aiOut: 1, sug: 1, mat: 1, run: 1, access: 1 });
});

...
test('C 督促多段: 延滞請求の督促カードに段数バッジが表示される', async ({ page }) => {
  ...
  // silent skip 禁止（Codex #4965087729 P2-2）: 必須 seed（延滞請求 INV-2026-202）を fail-fast 検証。
  // fixture が欠けると証拠が消えるため、skip ではなく明示的に失敗させる。
  expect(overdue, 'seed の延滞請求 INV-2026-202 が必要（pnpm db:seed）').not.toBeNull();
  await login(page, 'ceo@ikezaki.local');
